...
--     writing `open scoped TotalMap` and `open scoped TotalMap` individually.
--     This wouldn't be a good sell if `→ₜ` and `→ₚ` are scoped notations, but
--     they're currently global.
