...
--     We need to explain projection reduction so that it makes
--     sense that `rfl` can prove
--     `{ toFun := fun x => 0 }.toFun n = 0`.
...
-- direction of `getElem_def` as a `simp`-lemma.
...
-- This design should minimize the need to use `getElem_def`
-- outside concrete examples (which are typically solvable with
-- `rfl` anyways).
