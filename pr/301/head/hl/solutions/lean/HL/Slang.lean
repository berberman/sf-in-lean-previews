...
-- In **Logical Foundations** (LF) we went through the basics of how to use
-- Lean to prove theorems and write functional programs. Now, we begin to
-- shift gears to using it to reason about properties of programs and
-- programming languages. We begin by looking at a language we call **Slang**
-- (for *simple language*). Despite its simplicity, Slang lets us introduce
-- key concepts for specifying the *syntax* and *semantics* of programming
-- languages and show how those concepts are realized in Lean.

-- (This chapter is shared, word for word, between two volumes: **Type
-- Systems** (TS) and **Hoare Logic** (HL). If you have already worked through
-- it in the other volume, you can safely skip ahead to the next chapter of
-- this one.)
...
-- In this chapter, we'll ignore the translation from the *concrete syntax*
-- that a programmer would actually write to these abstract syntax trees --
-- the process that, for example, would translate the string `"1 + 2 * 3"` to
-- the AST `.plus (.num 1) (.mult (.num 2) (.num 3))`.
...
-- - The BNF is more informal -- for example, it gives some suggestions about
--   the surface syntax of expressions (like the fact that the addition
--   operation is written with an infix `+`) while leaving other aspects of
--   lexical analysis and parsing (like the relative precedence of `+`, `-`, and
--   `*`, the use of parens to group subexpressions, etc.) unspecified. Some
--   additional information -- and human intelligence -- would be required to
--   turn this description into a formal definition, e.g., for implementing a
--   compiler. The Lean version consistently omits all this information and
--   concentrates on the abstract syntax only.

-- - Conversely, the BNF version is lighter and easier to read. Its informality
--   makes it flexible, a big advantage in situations like discussions at the
--   blackboard, where conveying general ideas is more important than nailing
--   down every detail precisely.

--   Indeed, there are dozens of BNF-like notations and people switch freely
--   among them -- usually without bothering to say which kind of BNF they're
--   using, because there is no need to: a rough-and-ready informal
--   understanding is all that's important.

-- It's good to be comfortable with both sorts of notations: informal ones for
-- communicating between humans and formal ones for carrying out
...
-- `Bool`, so Lean implicitly inserts a `decide` coercion. You can observe the
...
-- We can now start to get some mileage out of these definitions. Suppose we
-- define a function that takes an arithmetic expression and slightly
...
-- But if we want to be certain the optimization is correct -- that evaluating
-- an optimized expression *always* gives the same result as the original --
-- we should prove it!
...
-- `plus (num 0) e` specially, we case-split on the left operand `a₁` -- and,
-- when it is a numeral, on whether that numeral is `0` -- to line the proof
-- up with the function's own branches. Once the constructors are exposed,
-- each case is discharged by essentially the same incantation: unfold
-- `Aexp.optimize0plus`, rewrite `Aexp.eval` by its characterizing lemmas,
-- then finish with the induction hypotheses. Notice how repetitive that makes
-- the proof.
...
-- branch of `optimize0plus` -- the special `plus (num 0) e` branch included
-- -- so the nested `cases` disappear.

-- Before applying `fun_induction` to a function as complex as
-- `Aexp.optimize0plus`, let's see how it works on somthing simpler. Recall
-- the definition of `Nat.even` and `Nat.odd`:
...
-- `n' + 1` - one for each of the cases in the inductive definition of natural
-- numbers. Functional induction on `Nat.even`, however, gives us three cases
-- - `0`, `1`, and `n' + 2` - corresponding to each of the cases of its
-- definition.
...
-- this, every goal has the same shape, so we can attack them uniformly with
-- the `<;>` combinator and a single tactic, `simp_all`, which rewrites
-- `Aexp.eval` by the `@[simp]` characterizing lemmas and uses the induction
-- hypotheses -- which it picks up from the local context automatically -- to
-- close each goal. The whole proof collapses to two lines.

theorem optimize0plus_sound' (a : Aexp) :
    a.optimize0plus.eval = a.eval := by
  fun_induction Aexp.optimize0plus a with simp_all

end Aexp

-- ### Exercise (3 stars): optimize0plusB_sound ⭐⭐⭐

-- Since the `Aexp.optimize0plus` transformation doesn't change the value of
-- an `Aexp`, we should be able to apply it to all the `Aexp`s that appear in
-- a `Bexp` without changing the `Bexp`'s value. Write a function that
...
theorem Bexp.optimize0plus_sound (b : Bexp) :
  ...
  fun_induction Bexp.optimize0plus b with simp_all [Aexp.optimize0plus_sound]
...
-- more sophisticated optimizer and prove it correct. (You will probably find
-- it easiest to start small -- add just a single, simple optimization and its
-- correctness proof -- and build up incrementally to something more
-- interesting.)

-- ## Evaluation as a Relation

-- We have presented `Aexp.eval` and `Bexp.eval` as functions defined by
-- recursion. Another way to think about evaluation -- one that is often more
-- flexible -- is as a *relation* between expressions and their values. This
-- perspective leads to inductive definitions like the following.
...
-- The version above makes the rules somewhat easier to read, but gives less
-- control over naming the hypotheses during proofs involving the relation.
-- For this reason we adopt the named style.
...
-- The `notation` is declared right after the inductive. The `scoped` keyword
-- allows us to scope the notation to the `Aexp` namespace so it doesn't
-- collide with other notations we use for different evaluation relations
-- later.
...
-- *inference rules*, where the premises above the line justify the conclusion
...
-- variables, but we'll soon be adding them.) The whole collection of rules is
-- understood as being wrapped, implicitly, in an inductive declaration. In
-- informal prose, this is sometimes indicated by saying something like "Let
-- `Aexp.EvalR` be the smallest relation closed under the following rules...".
...
-- ### Exercise (1 star): beval_rules (manually graded) ⭐
...
-- Write out a corresponding definition of boolean evaluation as a relation in
-- inference rule notation.
...
theorem evalR_iff_eval' (a : Aexp) (n : Nat) :
  ...
  · subst h
    induction a with constructor <;> assumption

end Aexp

-- ### Exercise (3 stars): bevalR ⭐⭐⭐

-- Write a relation `Bexp.EvalR` in the same style as `Aexp.EvalR`, and prove
...
theorem evalR_iff_eval (b : Bexp) (bv : Bool) :
    b ⇓ bv ↔ b.eval = bv := by
  constructor <;> intro h
  · induction h with simp_all [Aexp.evalR_iff_eval]
  · subst h
    induction b with constructor <;> simp_all [Aexp.evalR_iff_eval]
...
-- For the definitions of evaluation for arithmetic and boolean expressions,
-- the choice of whether to use functional or relational definitions is mainly
-- a matter of taste. However, there are situations where relational
-- definitions work much better than functional ones.
...
-- what should we return as the result of `.div (.num 5) (.num 0)`? One option
-- would be to lift the definition of `Aexp.eval` to return an option:
...
-- *probabilistic* choice among all numbers -- we only say which results are
-- *possible*.)
...
-- points of view at will -- exactly what we did above in
