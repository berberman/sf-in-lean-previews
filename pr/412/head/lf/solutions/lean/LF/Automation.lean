...
--      (`lake env lean`). `double_injective` from `Tactics` doesn't
--      actually work here: its conclusion is the bare variable `n = m`,
--      which isn't a usable rewrite pattern (simp lemmas need a real
--      compound term on the left), so `simp [double_injective]` fails
