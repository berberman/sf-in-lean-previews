...
--     of this material. Needs a deep look. In particular:
--
--     - One feels that it takes a rather circuitous route to get
--       to formal decorated programs -- there might be a way to
--       just go straight there. More importantly, it isn't very
--       clear to me that the version of decorated programs that
--       we wound up with is really the right one -- why spend
--       all this time writing annotations that we then say are
--       unnecessary? Seems like we could define decorated
--       programs with a lighter annotation burden (as in the
--       exercise at the end) and just add optional annotations
--       when we want to, to make particular examples clearer.
--
--     - The rigidity of the Hoare rules as stated in the last
--       chapter is also annoying here at many points. Building
--       the rules of consequence into all the other rules might
--       make a lot of things smoother. Would be a big change
--       (also to Hoare.v), but definitely worth a try.
--
--     And related...
...
--     didn't have time to get to. Maybe next time if no one
--     else gets to them first.
--
--     - 1. This chapter feels largely disconnected from the style
--          of the rest of the series, which is "100% Rocq script".
--          There's a significant amount of "just comments" here
--          instead. I think we could make this much better by
--          introducing formal decorated programs right after we
--          informally define them. Then in the examples that follow
--          do each informal decorated program (to get students to
--          find the right assertions, more or less) followed
--          immediately by a formal version, instead of delaying
--          formal so far to the end. The `parity` exercise is a
--          good example of one that already almost does this
--          already. (BCP 21: Done!)
--
--     - 2. There are several places where we are verifying Imp
--          program schemas, not actual programs. We're mixing Rocq
--          variables with Imp variables. That's confusing. One
--          example is `two_loops`; I've tried to mark others as I
--          come across them. (BCP: I've added some quizzes and such
--          to try to clarify the relation between programs /
--          triples and program/triple schemas. BCP 25: I think this
--          is a non-issue now.)
--
--     - 3. Weakest preconditions show up in this chapter as
--          completely optional, then are revisited (and required)
--          in HoareAsLogic. Consider moving the entire treatment to
--          that chapter. (BCP 23: Yes, we should do that!)
--
--     - 4. It seems a shame that the SparseAnnotations section is
--          not visible in the full version, but only in a solution.
--          It's fantastic! It would be great to show it off. (BCP
--          25: Yes! Indeed, perhaps it should even replace the
--          current treatment!)

-- Note to developers:
--     HIDE: Some useful theorems about Imp are not being
--     redefined for the modified versions of Imp. We should
--     either re-prove them or add hints (or exercises!) about
--     this. BCP 20: Which ones???
...
-- The beauty of Hoare Logic is that it is *syntax directed*:
-- the structure of proofs exactly follows the structure of
-- programs.
...
-- Such a *decorated program* carries within itself an argument
...
-- Concretely, a decorated program consists of the program's
-- text interleaved with assertions (sometimes multiple
...
-- proved for that part of the program using one of the Hoare
-- Logic rules, and the structure of the program itself shows
-- how to assemble all these individual steps into a proof for
-- the whole program.

-- ### Example: Swapping

-- Consider the following program, which swaps the values of
-- two variables using addition and subtraction, instead of by
...
-- We can give a proof, in the form of decorations, that this
-- program is correct — i.e., it really swaps `X` and `Y` — as
-- follows.
...
--
--     - (Note that we are working with natural numbers rather
--       than fixed-width machine integers, so we don't need to
--       worry about the possibility of arithmetic overflow
--       anywhere in this argument. This makes life quite a bit
--       simpler!)
--
--     HIDE: A quick / optional exercise using just assignment
--     here would be good.
...
-- Note to developers:
--     NOTATION: LATER: The `~` in that paragraph will typeset
--     wrong if the space after it is removed. Maybe it's
--     better to give up on all the unicode hacks in the
--     generated HTML...?
...
-- The following Imp program calculates the integer quotient
-- and remainder of parameters `m` and `n`.
...
-- If we replace `m` and `n` by concrete numbers and execute
-- the program, it will terminate with the variable `X` set to
-- the remainder when `m` is divided by `n` and `Y` set to the
-- quotient.
...
-- From an informal proof in the form of a decorated program,
-- it is "easy in principle" to read off a formal proof using
-- the Lean theorems corresponding to the Hoare Logic rules,
-- but these proofs can be a bit long and fiddly.
...
-- This example shows that it is conceptually straightforward
-- to read off the main elements of a formal proof from a
-- decorated program. Indeed, the process is so straightforward
-- that it can be automated, as we will see next.

-- ## Formal Decorated Programs

-- With a little more work, we can *formalize* the definition
-- of well-formed decorated programs and *automate* the boring
-- mechanical steps in proving that the decorations are
-- correct.

-- ### Syntax

-- The first thing we need to do is to formalize a variant of
-- the syntax of Imp commands that includes embedded
-- assertions, which we'll call "decorations." We call the new
-- commands *decorated commands*, or `dcom`s.

-- The choice of exactly where to put assertions in the
-- definition of `dcom` is a bit subtle. The simplest thing to
-- do would be to annotate every `dcom` with a precondition and
-- postcondition — something like this...
...
-- But this would result in *very* verbose decorated programs
-- with a lot of repeated annotations: a simple program like
...
-- postconditions on each command, because a sequence of two
-- commands would contain redundant decorations — the
...
-- preconditions whenever possible and embed just
-- postconditions.
...
-- forms: each decorated command is assumed to carry its own
-- postcondition within itself but take its precondition from
-- its context in which it is used.
...
-- Similarly, inside `d1` there will also be a postcondition,
-- which additionally serves as the *precondition* for `d2`.
...
--   postcondition for the entire statement, as well as
...
--     calculate P1 and P2 later, once we know the precondition
--     of the whole loop. Indeed, we could (and perhaps should,
--     as discussed elsewhere), but we are doing something
...
-- category, `dcom`, so it can coexist with the ordinary Imp
-- command syntax.
...
-- To provide the initial precondition that goes at the very
-- top of a decorated program, we introduce a new type
-- `decorated`:
...
-- To inspect the fully elaborated term behind this notation,
-- put `set_option pp.all true in` before a `#check` or
-- `#print` command.

-- The formal definitions can use either constructors directly
-- or the decorated-command notation introduced above.
...
-- `Prop`; thus, to show that it is *valid*, we need to produce
...
-- verified to see that the decorations are locally consistent
-- and thus constitute a proof of validity of the outer triple.

-- ### Extracting Verification Conditions

-- The function `DCom.VerificationConditions` takes a decorated
-- command `d` together with a precondition `P` and returns a
-- *proposition* that, if it can be proved, implies that the
-- triple
...
--   consistent with respect to `P` if `d1` is locally consistent
--   with respect to `P` and `d2` is locally consistent with
--   respect to the postcondition of `d1`.
...
-- reads off a proposition saying that all its decorations are
-- locally consistent.

-- Formally, since a decorated command is "waiting for its
-- precondition" the main VC generator takes a `dcom` plus a
-- given precondition as arguments.

-- Note to developers:
--     HIDE: There was some discussion in 2016 about whether
--     the VC generator should should use equivalence or
--     implication in a few places. I (BCP) believe Phil
--     (Wadler) changed some instances of the former to the
--     latter.
--
--     HIDE: MRC'20: a written explanation of each part of this
--     would be quite nice. BCP 21: Agreed!! (BCP 23: But it's
--     kind of what's just above...)
...
-- `DCom.VerificationConditions` does its job correctly. Not
-- surprisingly, each of the Hoare Logic rules plays a critical
-- role at some point in the proof.
...
-- Now that all the pieces are in place, we can define what it
-- means to verify an entire program.
...
-- The propositions generated by `DCom.VerificationConditions`
-- are fairly big and contain many conjuncts that are
-- essentially trivial.
...
-- `verification_correct` to extract the verification
-- conditions, use `verify_assertion` to verify them as much as
-- it can, and finally tidy up any remaining bits by hand.
...
-- creative part in verifying programs using Hoare Logic is
-- finding the right loop invariants...

-- ### Example: Slow Subtraction

-- The following program subtracts the value of `X` from the
-- value of `Y` by repeatedly decrementing both `X` and `Y`. We
-- want to verify its correctness with respect to the pre- and
-- postconditions shown:
...
-- To verify this program, we need to find an invariant `Inv`
-- for the loop. As a first step we can leave `Inv` as an
-- unknown and build a *skeleton* for the proof by applying the
-- rules for local consistency, working from the end of the
-- program to the beginning, as usual, and without doing any
-- thinking at all yet.
...
-- - (c) it must be *preserved* by a single iteration of the
--   loop, assuming that the loop guard also evaluates to true,
--   i.e., (3) must imply (4).
...
--     for future exercise builders, together with some hints
--     on how to write programs that are easier to verify
...
--     whole section. Moreover, there is a proposal to make the
--     decorations in Hoare look more like the decorations
--     earlier in the present chapter. All three should be
--     aligned.

-- ## Weakest Preconditions (Optional)

-- Note to developers:
--     HIDE: BCP 21: We talked about moving this stuff to the
--     HoareAsLogic chapter to lighten this chapter, but it
--     fits awkwardly there, so I'm leaving it here. It's
--     optional anyway. We might consider assigning one of the
--     exercises as advanced-only though.
...
-- `X := Y + 1` with respect to postcondition `X <= 5`. Think
-- of *weakest* here as meaning "easiest to satisfy": a weakest
-- precondition is one that as many states as possible can
-- satisfy.
...
-- example, `Y <= 4` was a weakest precondition above, but so
-- are the logically equivalent assertions `Y < 5`,
-- `Y <= 2 * 2`, etc. It is easy to show that any two weakest
-- preconditions `P` and `P'` of a command `c` with respect to
-- postcondition `Q` are logically equivalent; that is,
-- `P <<->> P'`.
