...
-- failure. Partial maps are defined in terms of total maps, using None as the
-- default element.
...
-- so that we can use the `∅` notation for this empty map. We'll also declare
-- the instance
...
--     `example : ({ "foo" ↦ true })["foo"]! = true := rfl` doesn't work (the
--     collection that has Insert and GetElem is ambiguous)?
