...
-- We'll also declare the instance
...
-- which allows us to use the notation `m[a]` to access
-- elements of a map `m`.
...
-- don't need to define a `find` operation on this
-- representation of maps because it is just function
-- application!
...
-- Note to developers:
--     The Rocq source also has `apply_empty` and `update_eq`
--     as (optional) exercises; here they are worked examples,
--     since `update_eq` was already presented that way.
...
-- Note to developers:
--     xhalo32: Should we explain why
--     `example : ({ "foo" ↦ true })["foo"]! = true := rfl`
--     doesn't work (the collection that has Insert and GetElem
--     is ambiguous)?
...
--     Making this an `abbrev` is a design decision: a type
--     alias avoids duplicating all the typeclass instances
--     (`GetElem`, `EmptyCollection`, ...) for partial maps. If
...
-- Updating a partial map at a key means storing `some` value
-- there, and we introduce a similar notation for it:
...
-- Since `PartialMap.update` is *defined* as a total-map update
-- that stores `some` value, the two sides below are literally
-- the same map, and the fact holds by `rfl`. It is still worth
-- naming: `rw` matches goals *syntactically*, so a goal
-- written with `→ₚ` will not match a total-map lemma written
-- with `→ₜ` until it has been rewritten with this one. Each of
-- the partial-map lemmas that follow begins by doing exactly
-- that.

theorem totalMap_eq (m : PartialMap α β) (a : α) (b : β) : (a →ₚ b ; m) = (a →ₜ some b ; m) := rfl

-- We now straightforwardly lift all of the basic lemmas about
-- total maps to partial maps.
