...
-- Note that `try lia <;> try rw [...] <;> lia` *doesn't* work,
...
-- The lemmas we've been using for rewriting are the same ones
-- we'll give to `simp` for it to automatically solve goals
-- involving those theorems.
...
-- Instead of manually rewriting by the characterizing lemmas
-- in the example below, `simp` does it automatically.
...
-- Don't use `simp` without `only` unless you're closing a goal
...
--     Chris suggested using Mathlib's `linter.flexible` option
--     to enforce proper `simp` usage. How do we feel about
--     adding a Mathlib dependency for this?
...
-- (such as `rfl` or `contradiction`) to try to close the
...
--     make sense to keep here?

-- We connect regular expressions and strings by defining when
-- a regular expression *matches* some string.
...
-- regular expression that matches exactly the string that it
...
-- Naturally, proofs about `ExpMatch` often require induction
-- (on evidence!).
...
-- `re_chars` that lists all characters that occur in a regular
-- expression:
...
-- One potentially confusing feature of the `induction` tactic
...
-- and (2) add an equation `h : x = e` to the context. Here's
...
-- One of the first really interesting theorems in the theory
-- of regular expressions is the so-called *pumping lemma*,
-- which states, informally, that any sufficiently long string
-- `s` matching a regular expression `re` can be "pumped" by
-- repeating some middle section of `s` an arbitrary number of
...
-- be able to *calculate*, for each regular expression `re`, a
-- minimum length for strings `s` to guarantee "pumpability."
...
-- These auxiliary lemmas might also be useful in your proof of
-- the pumping lemma.
...
-- `re`, then `s` can be split into three substrings
-- `s₁ ++ s₂ ++ s₃` in such a way that `s₂` can be repeated any
-- number of times and the result, when combined with `s₁` and
-- `s₃`, will still match `re`. Since `s₂` is also guaranteed
-- not to be the empty string, this gives us a (constructive!)
-- way to generate strings matching `re` that are as long as we
-- like.

-- This proof is quite long, so to make it more tractable we've
-- broken it up into a number of sub-proofs, which we then
-- assemble to prove the main lemma.
...
-- Note to developers (Daniel Sainati @dsainati1):
--     If this exercise is going to be optional we should still
--     fill in the solution but it's lower priority.

-- Note to developers (Benjamin Pierce @bcpierce00):
--     I've made it optional, following the original, but we
--     should think/talk about it. I also reduced the rating
--     from 10 to 5 (which I think is the maximum?).
...
--     Add `gradeTheorem 5 pumping` once the proof is filled
--     in.
