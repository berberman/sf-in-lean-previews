...
--
--     - One feels that it takes a rather circuitous route to get to formal
--       decorated programs -- there might be a way to just go straight there.
--       More importantly, it isn't very clear to me that the version of
--       decorated programs that we wound up with is really the right one -- why
--       spend all this time writing annotations that we then say are
--       unnecessary? Seems like we could define decorated programs with a
--       lighter annotation burden (as in the exercise at the end) and just add
--       optional annotations when we want to, to make particular examples
--       clearer.
--
--     - The rigidity of the Hoare rules as stated in the last chapter is also
--       annoying here at many points. Building the rules of consequence into
--       all the other rules might make a lot of things smoother. Would be a big
--       change (also to Hoare.v), but definitely worth a try.
--
--     And related...

-- Note to developers (Michael Clarkson @clarksmr, before next release, 2020):
--     Here are a bunch of improvements I wanted to make but didn't have time
--     to get to. Maybe next time if no one else gets to them first.
--
--     - 1. This chapter feels largely disconnected from the style of the rest of
--          the series, which is "100% Rocq script". There's a significant amount
--          of "just comments" here instead. I think we could make this much better
--          by introducing formal decorated programs right after we informally
--          define them. Then in the examples that follow do each informal
--          decorated program (to get students to find the right assertions, more
--          or less) followed immediately by a formal version, instead of delaying
--          formal so far to the end. The `parity` exercise is a good example of
--          one that already almost does this already. (BCP 21: Done!)
--
--     - 2. There are several places where we are verifying Imp program schemas,
--          not actual programs. We're mixing Rocq variables with Imp variables.
--          That's confusing. One example is `two_loops`; I've tried to mark others
--          as I come across them. (BCP: I've added some quizzes and such to try to
--          clarify the relation between programs / triples and program/triple
--          schemas. BCP 25: I think this is a non-issue now.)
--
--     - 3. Weakest preconditions show up in this chapter as completely optional,
--          then are revisited (and required) in HoareAsLogic. Consider moving the
--          entire treatment to that chapter. (BCP 23: Yes, we should do that!)
--
--     - 4. It seems a shame that the SparseAnnotations section is not visible in
--          the full version, but only in a solution. It's fantastic! It would be
--          great to show it off. (BCP 25: Yes! Indeed, perhaps it should even
--          replace the current treatment!)

-- Note to developers:
--     HIDE: Some useful theorems about Imp are not being redefined for the
--     modified versions of Imp. We should either re-prove them or add hints
--     (or exercises!) about this. BCP 20: Which ones???

-- _Quiz:_

-- On a piece of paper (or whatever), write down a Hoare-triple specification
...
-- The beauty of Hoare Logic is that it is *syntax directed*: the structure of
-- proofs exactly follows the structure of programs.
...
-- Such a *decorated program* carries within itself an argument for its own
-- correctness.
...
-- Here is one possible specification for this program, in the form of a Hoare
-- triple:
...
-- Concretely, a decorated program consists of the program's text interleaved
-- with assertions (sometimes multiple assertions separated by ->>).

-- A decorated program can be viewed as a compact representation of a proof in
-- Hoare Logic: the assertions surrounding each command specify the Hoare
-- triple to be proved for that part of the program using one of the Hoare
-- Logic rules, and the structure of the program itself shows how to assemble
-- all these individual steps into a proof for the whole program.

-- Our goal is to verify such decorated programs "mostly automatically." But,
-- before we can verify anything, we need to be able to *find* a proof for a
-- given specification, and for this we need to discover the right assertions.
-- This can be done in an almost mechanical way, with the exception of finding
-- loop invariants. In the remainder of this section, we explain in detail how
-- to construct decorations for several short programs, all of which are loop
-- free or have simple loop invariants. We'll return to finding more
-- interesting loop invariants later in the chapter.
...
--   postcondition (5). In the precondition, we use parameters `m` and `n` to
--   remember the initial values of variables `X` and `Y` so that we can refer
--   to them in the postcondition (5).

-- - We work backwards, mechanically, starting from (5) and proceeding until we
--   get to (2). At each step, we obtain the precondition of the assignment from
--   its postcondition by substituting the assigned variable with the
--   right-hand-side of the assignment. For instance, we obtain (4) by
--   substituting `X` with `X - Y` in (5), and we obtain (3) by substituting `Y`
--   with `X - Y` in (4).

-- - Finally, we verify that (1) logically implies (2) — i.e., that the step
--   from (1) to (2) is a valid use of the law of consequence — by doing a bit
--   of high-school algebra.

-- Note to developers:
--     HIDE: BCP 21: This side comment seems too technical:
--
--     - (Note that we are working with natural numbers rather than fixed-width
--       machine integers, so we don't need to worry about the possibility of
--       arithmetic overflow anywhere in this argument. This makes life quite a
--       bit simpler!)
--
--     HIDE: A quick / optional exercise using just assignment here would be
--     good.
...
--   postcondition (8) to (4) and (7). We conjoin the precondition (1) with the
--   guard of the conditional to obtain (2). We conjoin (1) with the negated
--   guard of the conditional to obtain (5).

-- - In order to use the assignment rule and obtain (3), we substitute `Z` by
--   `Y - X` in (4). To obtain (6) we substitute `Z` by `X - Y` in (7).

-- - Finally, we verify that (2) implies (3) and (5) implies (6). Both of these
--   implications crucially depend on the ordering of `X` and `Y` obtained from
--   the guard. For instance, knowing that `X <= Y` ensures that subtracting `X`
--   from `Y` and then adding back `X` produces `Y`, as required by the first
--   disjunct of (3). Similarly, knowing that `~ (X <= Y)` ensures that
--   subtracting `Y` from `X` and then adding back `Y` produces `X`, as needed
--   by the second disjunct of (6). Note that `n - m + m = n` does *not* hold
--   for arbitrary natural numbers `n` and `m` (for example, [3 - 5 + 5 = 5]).

-- Note to developers:
--     NOTATION: LATER: The `~` in that paragraph will typeset wrong if the
...
-- The second use of consequence is trivial, while the first crucially depends
-- on the `X <= Y` condition, which ensures that subtracting `X` from `Y` and
-- then adding back `X` produces `Y`.
...
-- - Following the format dictated by the `hoare_while` rule, we copy (1) to
--   (4). We conjoin (1) with the guard to obtain (2). We also conjoin (1) with
--   the negation of the guard to obtain (5).

-- - Because the final postcondition (6) does not syntactically match (5), we
--   add an implication between them.

-- - Using the assignment rule with assertion (4), we trivially substitute and
--   obtain assertion (3).
...
-- The following Imp program calculates the integer quotient and remainder of
-- parameters `m` and `n`.
...
-- will terminate with the variable `X` set to the remainder when `m` is
-- divided by `n` and `Y` set to the quotient.

-- In order to give a specification to this program we need to remember that
-- dividing `m` by `n` produces a remainder `X` and a quotient `Y` such that
-- `n * Y + X = m /\ X < n`.

-- It turns out that we get lucky with this program and don't have to think
-- very hard about the loop invariant: the loop invariant is just the first
-- conjunct, `n * Y + X = m`, and we can use this to decorate the program.
...
-- Assertions (4), (5), (8), and (9) are derived mechanically from the loop
-- invariant and the loop's guard. Assertions (8), (7), and (6) are derived
-- using the assignment rule going backwards from (8) to (6). Assertions (4),
-- (3), and (2) are again backwards applications of the assignment rule.

-- Now that we've decorated the program it only remains to check that the uses
-- of the consequence rule are correct — i.e., that (1) implies (2), that (5)
-- implies (6), and that (9) implies (10). This is indeed the case:

-- - (1) ->> (2): trivial, by algebra.

-- - (5) ->> (6): because `n <= X`, we are guaranteed that the subtraction in
--   (6) does not get zero-truncated. We can therefore rewrite (6) as
--   `n * Y + n + X - n` and cancel the `n`s, which results in the left conjunct
--   of (5).

-- - (9) ->> (10): if `~ (n <= X)` then `X < n`. That's straightforward from
--   high-school algebra. So, we have a valid decorated program.

-- ### From Decorated Programs to Formal Proofs

-- From an informal proof in the form of a decorated program, it is "easy in
-- principle" to read off a formal proof using the Lean theorems corresponding
-- to the Hoare Logic rules, but these proofs can be a bit long and fiddly.

-- Note that we do *not* unfold the definition of `ValidHoareTriple` anywhere
-- in this proof: the point of the game we're playing now is to use the Hoare
-- rules as a self-contained logic for reasoning about programs.
...
-- The following declaration introduces a more sophisticated tactic that will
-- help with proving assertions throughout the rest of this chapter. You don't
-- need to understand the details, but briefly: it introduces the available
-- hypotheses, simplifies assertions, maps, and arithmetic expressions, and
-- then asks `lia` to solve linear arithmetic goals. What's left after
-- `verify_assertion` does its work should be just the "interesting parts" of
-- the proof (which, if we're lucky, might be nothing at all!).
...
-- This example shows that it is conceptually straightforward to read off the
-- main elements of a formal proof from a decorated program. Indeed, the
-- process is so straightforward that it can be automated, as we will see
-- next.
...
-- embedded assertions that checking the validity of a triple is reduced to
-- simple logical and algebraic calculations showing that some assertions
-- imply others.
...
-- Imp commands that includes embedded assertions, which we'll call
-- "decorations." We call the new commands *decorated commands*, or `dcom`s.

-- The choice of exactly where to put assertions in the definition of `dcom`
-- is a bit subtle. The simplest thing to do would be to annotate every `dcom`
-- with a precondition and postcondition — something like this...
...
-- But this would result in *very* verbose decorated programs with a lot of
-- repeated annotations: a simple program like `skip;skip` would be decorated
-- like this,

--   {{P}} ({{P}} skip {{P}}) ; ({{P}} skip {{P}}) {{P}}

-- with pre- and post-conditions around each `skip`, plus identical pre- and
-- post-conditions on the semicolon!

-- In other words, we don't want both preconditions and postconditions on each
-- command, because a sequence of two commands would contain redundant
-- decorations — the postcondition of the first likely being the same as the
-- precondition of the second.

-- Instead, our formal syntax of decorated commands will omit preconditions
-- whenever possible and embed just postconditions.
...
-- decorated command is assumed to carry its own postcondition within itself
-- but take its precondition from its context in which it is used.

-- - Sequences `d1 ; d2` need no additional decorations.

-- Why?

-- Because inside `d2` there will be a postcondition, which also serves as the
-- postcondition of `d1;d2`.
...
-- - A conditional `if b then d1 else d2` is decorated with a postcondition for
--   the entire statement, as well as preconditions for each branch:

--   if b then {{ P1 }} d1 else {{ P2 }} d2 end {{ Q }}

-- Note to developers (Benjamin Pierce @bcpierce00, before next release, 2025):
--     Maybe we need a note here about why we don't just calculate P1 and P2
--     later, once we know the precondition of the whole loop. Indeed, we
--     could (and perhaps should, as discussed elsewhere), but we are doing
--     something simpler for the moment.

-- - A loop `while b do d end` is decorated with its final postcondition plus a
--   precondition for the body:
...
-- context; the latter relies on the postcondition already embedded in `d`.
...
-- Lean keeps decorated-command notation in its own syntax category, `dcom`,
-- so it can coexist with the ordinary Imp command syntax.
...
-- It is also straightforward to extract the precondition and postcondition
-- from a decorated program.
...
-- We can then express what it means for a decorated program to be correct as
-- follows:
...
-- The outer Hoare triple of a decorated program is just a `Prop`; thus, to
-- show that it is *valid*, we need to produce a proof of this proposition.

-- We will do this by extracting "proof obligations" from the decorations
-- sprinkled throughout the program.

-- These obligations are often called *verification conditions*, because they
-- are the facts that must be verified to see that the decorations are locally
-- consistent and thus constitute a proof of validity of the outer triple.

-- ### Extracting Verification Conditions

-- The function `DCom.VerificationConditions` takes a decorated command `d`
-- together with a precondition `P` and returns a *proposition* that, if it
-- can be proved, implies that the triple
...
--   decorated command and the assertions imposed by the external precondition;
--   these uses correspond to applications of the consequence rule.
...
-- - The sequential composition of `d1` and `d2` is locally consistent with
--   respect to `P` if `d1` is locally consistent with respect to `P` and `d2`
--   is locally consistent with respect to the postcondition of `d1`.
...
-- With all this in mind, we can write a *verification condition generator*
-- that takes a decorated command and reads off a proposition saying that all
-- its decorations are locally consistent.

-- Formally, since a decorated command is "waiting for its precondition" the
-- main VC generator takes a `dcom` plus a given precondition as arguments.

-- Note to developers:
--     HIDE: There was some discussion in 2016 about whether the VC generator
--     should should use equivalence or implication in a few places. I (BCP)
--     believe Phil (Wadler) changed some instances of the former to the
--     latter.
--
--     HIDE: MRC'20: a written explanation of each part of this would be quite
--     nice. BCP 21: Agreed!! (BCP 23: But it's kind of what's just above...)
...
-- The following key theorem states that `DCom.VerificationConditions` does
-- its job correctly. Not surprisingly, each of the Hoare Logic rules plays a
...
-- Now that all the pieces are in place, we can define what it means to verify
-- an entire program.
...
-- The propositions generated by `DCom.VerificationConditions` are fairly big
-- and contain many conjuncts that are essentially trivial.
...
-- Fortunately, our `verify_assertion` tactic can generally take care of most
-- (or sometimes all) of them.
...
-- `verify_assertion` to verify them as much as it can, and finally tidy up
-- any remaining bits by hand.
...
-- Similarly, here is the formal decorated program for the "swapping by adding
-- and subtracting" example that we saw earlier.
...
-- Here is a skeleton of the formal decorated version of the `if_minus_plus`
-- program that we saw earlier. Replace all occurrences of `FILL_IN_HERE` with
-- appropriate assertions and fill in the proof (which should be just as
-- straightforward as in the examples above).
...
-- creative part of a verifying program using Hoare Logic is finding the right
-- loop invariants. The reason this is difficult is the same as the reason
-- that inductive mathematical proofs are:

-- - Strengthening a *loop invariant* means that you have a stronger assumption
--   to work with when trying to establish the postcondition of the loop body,
--   but it also means that the loop body's postcondition is harder to prove.

-- - Similarly, strengthening an *induction hypothesis* means that you have a
--   stronger assumption to work with when trying to complete the induction step
--   of the proof, but it also means that the statement being proved inductively
--   is harder to prove.
...
-- The following program subtracts the value of `X` from the value of `Y` by
-- repeatedly decrementing both `X` and `Y`. We want to verify its correctness
-- with respect to the pre- and postconditions shown:
...
-- To verify this program, we need to find an invariant `Inv` for the loop. As
-- a first step we can leave `Inv` as an unknown and build a *skeleton* for
-- the proof by applying the rules for local consistency, working from the end
-- of the program to the beginning, as usual, and without doing any thinking
-- at all yet.
...
-- - (b) it must be *strong* enough to imply the program's postcondition, i.e.,
--   (7) must imply (8);

-- - (c) it must be *preserved* by a single iteration of the loop, assuming that
--   the loop guard also evaluates to true, i.e., (3) must imply (4).

-- These conditions are actually independent of the particular program and
-- specification we are considering: every loop invariant has to satisfy them.

-- One way to find a loop invariant that simultaneously satisfies these three
-- conditions is by using an iterative process: start with a "candidate"
-- invariant (e.g., a guess or a heuristic choice) and check the three
-- conditions above; if any of the checks fails, try to use the information
-- that we get from the failure to produce another — hopefully better —
-- candidate invariant, and repeat.

-- For instance, in the reduce-to-zero example above, we saw that, for a very
-- simple loop, choosing `True` as a loop invariant did the job. Maybe it will
-- work here too. To find out, let's try instantiating `Inv` with `True` in
-- the skeleton above and see what we get...
...
-- While conditions (a) and (c) are trivially satisfied, (b) is wrong: it is
-- not the case that `True /\ X = 0` (7) implies `Y = n - m` (8). In fact, the
-- two assertions are completely unrelated, so it is very easy to find a
-- counterexample to the implication (say, `Y = X = m = 0` and `n = 1`).

-- If we want (b) to hold, we need to strengthen the loop invariant so that it
-- implies the postcondition (8). One simple way to do this is to let the loop
-- invariant *be* the postcondition. So let's return to our skeleton,
-- instantiate `Inv` with `Y = n - m`, and try checking conditions (a) to (c)
-- again.
...
-- Condition (a) requires that (1) `X = m /\ Y = n` implies (2) `Y = n - m`.
-- If we substitute `Y` by `n` we have to show that `n = n - m` for arbitrary
-- `m` and `n`, which is not the case (for instance, when `m = n = 1`).
-- Condition (c) requires that `n - m - 1 = n - m`, which fails, for instance,
-- for `n = 1` and `m = 0`. So, although `Y = n - m` holds at the end of the
-- loop, it does not hold from the start, and it doesn't hold on each
-- iteration; it is not a correct loop invariant.

-- This failure is not very surprising: the variable `Y` changes during the
-- loop, while `m` and `n` are constant, so the assertion we chose didn't have
-- much chance of being a loop invariant!
...
-- loop terminates, and that "fills the gap" in some appropriate way when `X`
-- is nonzero. Looking at how the loop works, we can observe that `X` and `Y`
-- are decremented together until `X` reaches `0`. So, if `X = 2` and `Y = 5`
-- initially, after one iteration of the loop we obtain `X = 1` and `Y = 4`;
-- after two iterations `X = 0` and `Y = 3`; and then the loop stops. Notice
-- that the difference between `Y` and `X` stays constant between iterations:
-- initially, `Y = n` and `X = m`, and the difference is always `n - m`. So
-- let's try instantiating `Inv` in the skeleton above with `Y - X = n - m`.
...
-- Success! Conditions (a), (b) and (c) all hold now. (To verify (c), we need
-- to check that, under the assumption that `X <> 0`, we have
...
-- variable `Y` is to start `Y` at `0`, then decrement `X` until it hits `0`,
-- incrementing `Y` at each step. Here is a program that implements this idea.
-- Fill in decorations and prove the decorated program correct. (The proof
...
-- Here is a cute way of computing the parity of a value initially stored in
-- `X`, due to Daniel Cristofani.
...
-- `m = parity m` does not hold for an arbitrary `m`, so we cannot hope to use
-- that as a loop invariant. To find a loop invariant that works, let's think
-- a bit about what this loop does. On each iteration it decrements `X` by
-- `2`, which preserves the parity of `X`. So the parity of `X` does not
-- change, i.e., it is invariant. The initial value of `X` is `m`, so the
-- parity of `X` is always equal to the parity of `m`. Using
-- `parity X = parity m` as an invariant we obtain the following decorated
-- program:
...
-- For verifying (b), we observe that, when `X < 2`, we have `parity X = X`
...
-- Hint: There are actually several possible loop invariants that all lead to
-- good proofs; one that leads to a particularly simple proof is
-- `parity X = parity m`. Ordinary Lean functions can be applied directly to
-- Imp variables inside assertions, so this can be written as
-- `{{ parity X = parity m }}`.
...
-- As we did before, we can try to use the postcondition as a candidate loop
...
-- This didn't work very well: conditions (a) and (c) both failed. Looking at
-- condition (c), we see that the second conjunct of (4) is almost the same as
-- the first conjunct of (5), except that (4) mentions `X` while (5) mentions
-- `m`. But note that `X` is never assigned in this program, so we should
-- always have `X=m`. We didn't propagate this information from (1) into the
-- loop invariant, but we could!

-- Also, we don't need the second conjunct of (8), since we can obtain it from
-- the negation of the guard — the third conjunct in (7) — again under the
-- assumption that `X=m`. This allows us to simplify a bit.
...
-- This works, since conditions (a), (b), and (c) are now all rather trivially
-- satisfied.

-- Very often, when a variable is used in a loop in a read-only fashion (i.e.,
-- it is referred to by the program or by the specification, and it is not
...
-- Hint: The loop invariant here must ensure that `Z*Z` is consistently less
-- than or equal to X.
...
-- Note to developers:
--     HIDE: CH: it might make sense to show all the variants for future
--     exercise builders, together with some hints on how to write programs
...
-- The first thing to note is that the loop reads `X` but doesn't change its
-- value. As we saw in the previous example, it can be a good idea in such
-- cases to add `X = m` to the loop invariant. The other thing that we know is
-- often useful in the loop invariant is the postcondition, so let's add that
-- too, leading to the candidate loop invariant `Z = m * m /\ X = m`.
...
-- Conditions (a) and (c) fail because of the `Z = m*m` part. While `Z` starts
-- at `0` and works itself up to `m*m`, we can't expect `Z` to be `m*m` from
-- the start. If we look at how `Z` progresses in the loop, after the 1st
-- iteration `Z = m`, after the 2nd iteration `Z = 2*m`, and at the end
-- `Z = m*m`. Since the variable `Y` tracks how many times we go through the
-- loop, this leads us to derive a new loop invariant candidate:
-- `Z = Y*m /\ X = m`.
...
-- This new loop invariant makes the proof go through: all three conditions
-- are easy to check.
...
-- variables and parameters, like `m - Y` — when going from postconditions to
-- loop invariants.
...
-- 1*2*...*n`). We
-- can define the factorial function recursively in Lean as follows:
...
-- First, write the Imp program `factorial` that calculates the factorial of
-- the number initially stored in the variable `X` and puts it in the variable
-- `Y`.

-- Using your definition `factorial` and `slowAssignmentDec` as a guide, write
-- a formal decorated program `factorialDec` that implements the factorial
-- function. Ordinary Lean functions such as `fact` can be applied directly to
-- Imp variables inside assertions.

-- Fill in the blanks and finish the proof of correctness. Bear in mind that
-- we are working with natural numbers, for which both division and
-- subtraction can behave differently than with real numbers. Excluding both
-- operations from your loop invariant is advisable!

-- Then state a theorem named `factorial_correct` that says `factorialDec` is
-- correct, and prove the theorem. If all goes well, `verify` will leave you
-- with just two subgoals, each of which requires establishing some
-- mathematical property of `fact`, rather than proving anything about your
-- program.

-- Hint: if those two subgoals become tedious to prove, give some thought to
-- how you could restate your assertions such that the mathematical operations
...
--     for an Imp program. `m` is not an Imp variable nor a constant. BCP 21:
--     But we do that all over the place, no?
--
...
-- Fill in decorations for the following program and prove them correct. As
-- with `factorial`, be careful about mathematical reasoning involving natural
-- numbers, especially subtraction.

-- Lean functions can be applied directly inside assertions. For example, the
-- minimum of `a` and `b` can be written `Nat.min a b`.
...
-- Prove that `fib` satisfies the following equation. You will need this as a
-- lemma in the next exercise.
...
-- Fill in the following definition of `dfib` and prove that it satisfies this
-- specification:

--   {{ True }} dfib {{ Y = fib n }}

-- Ordinary Lean function application can be used in the assertions. If all
-- goes well, your proof will be very brief.
...
--     Moreover, there is a proposal to make the decorations in Hoare look
...
-- requirement, we can eliminate almost all annotations, just requiring final
-- postconditions and loop invariants to be provided explicitly. Do this —
-- i.e., define a new version of `DCom` with as few annotations as possible
-- and adapt the rest of the formal development leading up to the
...
--     chapter to lighten this chapter, but it fits awkwardly there, so I'm
--     leaving it here. It's optional anyway. We might consider assigning one
--     of the exercises as advanced-only though.

-- Some preconditions are more interesting than others. For example, the Hoare
-- triple
...
-- doesn't describe any situations where we can use the command `X := Y + 1`
-- to achieve the postcondition `X <= 5`.
...
-- command will produce a state satisfying the postcondition. However, this
-- precondition is not as useful as it could be, because the `Z = 0` clause in
-- the precondition actually has nothing to do with the postcondition
-- `X <= 5`.
...
-- The assertion `Y <= 4` is called the *weakest precondition* of `X := Y + 1`
-- with respect to the postcondition `X <= 5`.

-- Assertion `Y <= 4` is a *weakest precondition* of command `X := Y + 1` with
-- respect to postcondition `X <= 5`. Think of *weakest* here as meaning
-- "easiest to satisfy": a weakest precondition is one that as many states as
...
-- Note that weakest preconditions need not be unique. For example, `Y <= 4`
-- was a weakest precondition above, but so are the logically equivalent
-- assertions `Y < 5`, `Y <= 2 * 2`, etc. It is easy to show that any two
-- weakest preconditions `P` and `P'` of a command `c` with respect to
-- postcondition `Q` are logically equivalent; that is, `P <<->> P'`.
...
--     I had to fill in the above proof for the autograder to pass. If the
--     proof is bad or unidiomatic, feel free to completely replace it. Please
--     note that I have no idea what I'm doing -- I've never worked with
--     weakest preconditions before.
