...
--     Given the description, I expected Lean complain **noisily** when I
--     clicked through true*and*wrong, but it simply fails to discharge the
--     goal. The non-proof is suppressed by sf*expect*failure. Perhaps write:
--
--     To see the error message, comment out "sf*expect*failure" temporarily.
...
-- Note to developers (David Walker @princedpw):
--     The description of `rewrite ... at` appears wrong. It should say
--     something like the following. (I suggest changing the letter associated
--     with the rule from h to something else -- perhaps r. I found that
--     easier to read.)
--
--     For example, if `hp : p` is in the context and we have a rule
--     `r : p = q`, then `rewrite [r] at hp` changes the hypothesis to
--     `hp : q`.
