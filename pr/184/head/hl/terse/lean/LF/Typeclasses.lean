...
-- should behave on a value of type `α`, but it can't find the
-- instance of the typeclass `BEq` that it needs, since not
-- every type can be checked for equality. As Lists noted when
-- we first used it, `==` on `Nat` comes from the `BEq`
-- typeclass, an interface each type has to implement for
-- itself.

-- We could sidestep typeclasses entirely and just have the
...
-- remember to supply, the right equality function. Typeclasses
-- automate this — instead of the programmer passing the
-- function explicitly, Lean searches for one and provides it
-- on its own. We specify something we want Lean to search for
-- by declaring a `class` with the needed function as a field;
-- a `class` is like an interface in Java or a trait in Rust.
-- Particular *implementations* of that interface, different
-- ones for different types, are called *instances*. Finally,
-- we add the name of that class as a *typeclass constraint* on
-- the polymorphic variable that the function applies to. This
-- directs lean to use the interface inside the function, and
-- to find and fill in the appropriate instance at call-sites
-- to that function. Here is what this looks like for
-- `List.elem_poly`:
...
-- In sum: the `[BEq α]` argument is filled in automatically,
-- based on whatever `α` the caller uses, and it's what
-- `List.elem_poly` uses internally wherever it writes `==`.
-- This is the **ad hoc polymorphism** we mentioned above:
-- `List.elem_poly` isn't generic over every type, only over
-- types that support `==`. We'll see exactly how `[BEq α]`
-- gets filled in below, starting with how to define a
-- typeclass in the first place.
