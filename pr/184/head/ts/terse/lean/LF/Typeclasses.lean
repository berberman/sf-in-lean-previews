...
-- uses internally wherever it writes `==`. The `[BEq α]`
-- constraint is saying that an instance of `BEq` must be
-- provided at call sites for the *particular* type `α` that is
-- used. In the example `[0, 1].elem_poly 0`, this type is
-- `Nat`, and the automatically chosen instance is `Nat.beq`.
