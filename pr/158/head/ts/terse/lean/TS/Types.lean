...
-- Note to developers:
--     LATER: For consistency with the STLC definitions in
--     future chapters, it would be better and simpler to
--     represent numbers by a simple nat, rather than as
--     strings of `succ` applied to `0` (which also introduces
--     subtleties that are really not the main point here). But
--     the present formulation is not a big problem either.
--
--     LATER: Harper's lecture from the Milner symposium would
--     make a good support for this lecture.
--
--     LATER: There are a bunch of slides from earlier
--     offerings of CIS500 that might be wonderful additions to
--     the TERSE notes.
...
-- - This means we can write *stuck* terms like `5 + true` and
--   `if 42 then 0 else 1`.
...
-- *Values* are `true`, `false`, and numeric values (`0`, and
-- `succ` of a numeric value).
...
-- The first interesting thing to notice about this `Tm.Step`
-- relation is that the strong progress theorem from the
-- Smallstep chapter fails here. That is, there are terms that
-- are normal forms (they can't take a step) but not values
-- (they are not included in our definition of possible
...
-- However, although values and normal forms are *not* the same
-- in this language, the set of values is a subset of the set
-- of normal forms.

-- This is important because it shows we did not accidentally
...
-- (Hint: You will reach a point in this proof where you need
-- to use an induction to reason about a term that is known to
-- be a numeric value. This induction can be performed either
-- over the term itself or over the evidence that it is a
-- numeric value. The proof goes through in either case, but
-- you will find that one way is quite a bit shorter than the
-- other. For the sake of the exercise, try to complete the
-- proof both ways.)

theorem value_is_nf (t : Tm) (h : Tm.IsValue t) : Tm.IsNormalForm t := by
  sorry

-- The "other way" mentioned in the hint proves the same fact
-- by induction on the term itself rather than on the evidence
-- that it is a numeric value. It goes through, but is a bit
-- longer than the `nvalue_is_nf` route above.
...
-- Use `value_is_nf` (here, `nvalue_is_nf`) to show that the
-- `Tm.Step` relation is also deterministic.
...
--   `pred (succ true)` is stuck for `Tm.Step` but steps under
...
-- A *functional* version computes a single `⇢` step of a term,
-- returning `none` when the term is a `⇢` normal form. This is
-- a nice chance to see a step *function*, which the chapter
-- otherwise gives only as a relation:
...
-- Typing is a *conservative* (or *static*) approximation to
-- behavior.

-- In particular, a term can be ill typed even though it steps
...
-- The following two lemmas capture the fundamental fact that
-- the definitions of boolean and numeric values agree with the
-- typing relation: a well-typed value of type `Bool` is a
-- boolean value, and of type `Nat` a numeric value.
...
--   `⊢ t2 ⦂ T` and `⊢ t3 ⦂ T`. By the IH, either `t1` is a value
...
--   - If `t1` itself can take a step, then, by `ifStep`, so can
--     `t`.

-- _Quiz:_

-- Quick review: in the language defined at the start of this
-- chapter...
...
-- (Again, make sure you understand the informal proof fragment
-- in the following exercise first.)
...
--   and remembering that `t` has the form `if ...`, we see that
--   the only ones that could have been used to prove `t ⟶ t'`
--   are `ifTrue`, `ifFalse`, or `ifStep`.

--   - If the last rule was `ifTrue`, then `t' = t2`. But we know
--     that `⊢ t2 ⦂ T`, so we are done.

--   - If the last rule was `ifFalse`, then `t' = t3`. But we know
--     that `⊢ t3 ⦂ T`, so we are done.

--   - If the last rule was `ifStep`, then
--     `t' = if t1' then t2 else t3`, where `t1 ⟶ t1'`. We know
--     `⊢ t1 ⦂ Bool` so, by the IH, `⊢ t1' ⦂
--           Bool`. The `ite`
--     rule then gives us `⊢ if t1' then t2 else t3 ⦂ T`, as
--     required.

-- ### Exercise (3 stars): preservation_alternate_proof ⭐⭐⭐

-- Now prove the same property again by induction on the
-- *evaluation* derivation instead of on the typing derivation.
...
-- Suppose, instead, that we add this new rule to the typing
-- relation:
...
-- breaks. (These are graded manually; there is no Lean code to
-- complete.)
...
-- Which of the following properties remain true in the
-- presence of this rule? For each one, write either "remains
-- true" or else "becomes false." If a property becomes false,
...
-- Suppose, instead, that we add this new rule to the `Tm.Step`
-- relation:

-- funny1 : if true then t2 else t3 ⟶ t3

-- Which of the above properties become false in the presence
-- of this rule? For each one that does, give a
-- counter-example.
...
-- The reduction rule `predZero` is a bit counter-intuitive: we
-- might feel that it makes more sense for the predecessor of
-- `0` to be undefined, rather than being defined to be `0`.
-- Can we achieve this simply by removing the rule from the
-- definition of `Tm.Step`? Would doing so create any problems
-- elsewhere?

-- ### Exercise (4 stars): prog_pres_bigstep (Advanced, manually graded) ⭐⭐⭐⭐

-- Suppose our evaluation relation is defined in the big-step
-- style. State appropriate analogs of the progress and
-- preservation properties. (You do not need to prove them.)

-- Can you see any limitations of either of your properties? Do
-- they allow for nonterminating programs? Why might we prefer
...
--     before these improvements? (I suspect yes because the
--     indentation is also wonky and I improved that too.)
--     Anyway, the `grade` block headers should be fixed,
--     throughout (and maybe in Smallstep and Imp?)... `dev`
--     block headers too, if we want to be really consistent.
