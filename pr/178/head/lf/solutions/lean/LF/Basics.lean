...
-- `mul_zero (n : Nat) : n * zero = zero`. The same statement can be written
-- using an explicit universal quantifier, as we have done before:

--   mul_zero : ∀ (n : Nat), n * zero = zero

-- These express exactly the same proposition — writing arguments as binders
-- before the colon is called *declaration-header style*. It is the
-- conventional style for Lean declarations, so we will generally use it from
-- now on.
...
-- `exact hp` closes the goal. You can also transform `hp` slightly, but we
-- will explain how when we get to an example where we need to.
