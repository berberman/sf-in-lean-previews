...
--     I don't know why but the infoview for this proof is
--     extremely confusing. How to set things up to be clearer?
...
--     The following few paragrahps are out-of-date because
--     `TotalMap` is also a structure.
...
--     The Maps chapter removed the `optionCoe` instance for
--     the duration of its partial-map development
--     (`attribute [-instance] optionCoe`, restored at
--     `end PartialMap`), on the grounds that a coercion from
--     `β` to `Option β` might be confusing at this point. It
--     also recorded two things about doing so: the removal
--     must not leak to end-of-file, or Verso's `tag`/`file`
--     metadata coercion (`Tag → Option Tag`) fails at
...
-- Note to developers:
--     `namespace TotalMap` is reopened here only because the
--     `Reflection` section below happens to sit inside it (its
--     `Nat.isEven`/`Nat.double` are really `TotalMap.Nat.*`,
--     and moving them to the root `Nat` namespace would
--     collide with `UsingLean`'s `Nat.double`). Drop the
...
theorem isEven_double_exists (n : Nat) :
    ∃ k, n = bif isEven n then double k else double k + 1 := by sorry
...
-- that `isEven_iff_Even` was reflecting a proof between
-- boolean and propositional equality. In fact, we can use this
-- theorem to directly construct a `Decidable` instance
