...
-- Rust. Particular implementations of that class are called *instance*; each
-- type can have its own instance of a class. For functions that would use
-- such instances, we specify the name of the class in a *instance implicit*
-- on the polymorphic variable that the instance's function applies to. This
-- directs Lean to rely on the class inside the function, and to find and fill
-- in the appropriate instance when the function is called.
...
-- `List.elem_poly_eq` above, here is a version that makes the default value
...
-- is to *name* the data we're after — the **default value** of a type. In
-- particular, a `structure` (chapter Lists) is a good way to give this
-- information a name; structures can also bundle together more than one piece
-- of data, which will come in handy later, though we only need a single field
-- here.

-- To address the first problem, we need to mark this particular structure as
-- one Lean should search for automatically — not every `structure`-typed
-- argument should be.

-- Let's build up to what we want in two steps: first the naming, as a plain
...
-- That means we can rewrite `List.headOr_ex` the same way we rewrote
-- `List.elem_poly_eq` into `List.elem_poly` above, replacing the explicit
...
-- Because the expression equates `DefaultValue.value` with the `Nat` `0`,
...
-- We can also create instances for polymorphic types, like `Option α`, whose
-- default is `none`, by giving the instance declaration a parameter:

instance instDefaultValueOption {α : Type} : DefaultValue (Option α) where
  value := none

-- Now, Lean can infer instances for all these types, including inside
-- `List.headOr`:
...
-- instances Lean picked. The `#synth` command runs the same search directly:

#synth DefaultValue Nat

-- instDefaultValueNat

-- For a typeclass like `DefaultValue` that carries data — a term, such as the
-- `1` above, rather than only proofs (which we will see below) — we expect at
-- most one instance per type, so this search has a unique answer.

-- We'll put `DefaultValue`'s standard-library equivalent, `Inhabited`, to
-- work later in this chapter, when we define maps that need a default value
-- for a generic type. First, though, let's go back to `List.elem_poly` and
-- see how its `[BEq α]` argument actually gets resolved.
...
-- called on a `List Nat` — no different from Lean choosing
-- `instDefaultValueNat` for `DefaultValue.value` earlier when it was equated
-- with `(1 : Nat)`.
...
-- `HasThree` that specifies a type with at least three distinct elements, and
-- give an instance of it for `Nat`.
...
-- This facility is very powerful, and is used extensively in Lean to define
-- mathematical structures that carry both operators and laws about how those
-- operators interact. As a simple example, let's use a typeclass to define a
-- *monoid*, a simple algebraic structure that includes four things:
...
-- notation for it. The `Monoid` typeclass "inherits" the fields of `OpSet`,
-- similar to how a class would in an object-oriented language.

-- As one might expect, the `+` operator over `Nat`s forms a monoid, where `0`
-- is the identity element. Note that we don't have to define `Nat`'s `OpSet`
-- instance separately, we can define a single instance that implements both
-- classes.
...
-- science, lists of any type also form a monoid, with `List.append` as the
...
-- typeclass. One simple theorem about monoids is that the identity element of
...
-- extend the definition of a `Monoid` to capture this new feature:
...
-- `∀ x, x + inv x = 0 = inv x + x`, for example. However, addition does form
-- a group over the integers:
...
-- to highlight something interesting. After the rewrites in this proof, we
-- end up with a goal that looks like `{ inner := fun x => 0 }.inner n = 0`,
-- which we can solve with `rfl`. This is because the projection `.inner` on a
-- structure of the form `{ inner := x }` is definitionally equal to `x`.
...
--     A reason I can come up with why we use a notation typeclass (in library
--     code) over a plain notation is that it makes it possible (for a
--     downstream consumer) to write `open scoped MyGetElem` instead of
--     writing `open scoped TotalMap` and `open scoped PartialMap`
--     individually. This wouldn't be a good sell if `→ₜ` and `→ₚ` are scoped
...
-- (Don't worry about the `outParam` qualifier; it is a hint to Lean that
-- helps typeclass inference.)
...
-- including an `app_unexpander` for printing `[...]`-notation lists back
-- out), or `+`/`*`/`==` for arithmetic — but always with `infixl`/`infixr` or
-- `scoped macro`; this is the first time we reach for the more general
-- `notation`/`macro_rules` forms for getting the `m[a]` syntax to work.

-- Don't worry about following the mechanism in detail — the `macro_rules` and
-- the `app_unexpander` below are minor technicalities. However, if you do
-- wish to learn more, Chapter 5 and 6 of [Metaprogramming in Lean
...
-- `m.get a` so we provide the reverse direction of `getElem_def` as a `simp`
...
-- This design minimizes the need to use `getElem_def` outside concrete
...
--     `show ("bar" == "foo") = false by simp` in some of the above sections
--     which would be good to explain in more detail in the reflection
--     section. Disclaimer: I don't actually know how Lean proves it under the
--     hood, I just assume it's relevant to reflection.
--
...
-- Next, we declare `Insert` and `Singleton` instances — the standard-library
-- typeclasses behind the `{x, y, ...}` and `{x}` collection-literal notation
-- that `List`, `Finset`, and other stdlib containers already support — so
-- that `TotalMap` can use it too.
