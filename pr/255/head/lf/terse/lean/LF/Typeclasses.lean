...
--     Students will run across universes, though. When looking
--     at List lemmas, for example, they will see things like:
--
...
-- structure and never inspect any particular `a : α`.

-- Sometimes, though, we want less freedom: rather than leaving
-- `α` completely generic, we want to partially specify its
-- behavior. In Lean, this is done through a form of "ad hoc
-- polymorphism" called **typeclasses**. The concept originated
-- in Haskell and is analogous to features you may know from
...
-- passing the function explicitly, Lean searches for one and
...
-- trait in Rust. Particular implementations of that class are
-- called *instance*; each type can have its own instance of a
-- class. For functions that would use such instances, we
-- specify the name of the class in a *instance implicit* on
-- the polymorphic variable that the instance's function
-- applies to. This directs Lean to rely on the class inside
-- the function, and to find and fill in the appropriate
...
--     which comes from `DecidableEq`, does not contain
--     `Nat.beq`. You can see in proofs of `List.elem_nat`
...
-- below, starting with how to define a typeclass in the first
-- place.
...
-- **default value** of a type. In particular, a `structure`
-- (chapter Lists) is a good way to give this information a
-- name; structures can also bundle together more than one
-- piece of data, which will come in handy later, though we
-- only need a single field here.
...
-- `DefaultValue.value` is equal to `0`.
...
-- `Option α`, whose default is `none`, by giving the instance
-- declaration a parameter:

instance instDefaultValueOption {α : Type} : DefaultValue (Option α) where
  value := none

-- Now, Lean can infer instances for all these types, including
-- inside `List.headOr`:
...
-- Let's check what `==` meant for `List.elem_nat`, with
...
-- `BEq.beq`, a field of exactly the kind of typeclass we just
-- learned to define:
...
-- This facility is very powerful, and is used extensively in
-- Lean to define mathematical structures that carry both
-- operators and laws about how those operators interact. As a
-- simple example, let's use a typeclass to define a *monoid*,
-- a simple algebraic structure that includes four things:
...
-- - a particular element `id` of type `α`, which we call the
--   "identity element", and

-- - some laws about the interaction of `⊗` and `id`, namely
--   that:
...
-- The `extends` keyword indicates that the `Monoid` typeclass
-- extends the `OpSet` typeclass, which just defines a set with
-- an operator and some notation for it. The `Monoid` typeclass
-- "inherits" the fields of `OpSet`, similar to how a class
-- would in an object-oriented language.

-- As one might expect, the `+` operator over `Nat`s forms a
-- monoid, where `0` is the identity element. Note that we
-- don't have to define `Nat`'s `OpSet` instance separately, we
-- can define a single instance that implements both classes.
...
-- There are also many monoids over other types. Most usefully
-- in computer science, lists of any type also form a monoid,
-- with `List.append` as the operator in question:
...
-- In addition to defining instances of `Monoid`, we can also
-- prove some properties about monoids in general, just based
-- on the laws defined on the typeclass. One simple theorem
-- about monoids is that the identity element of a monoid is
-- unique. That is, if we have two monoids over the same set
-- with the same operator, their identity elements must also be
-- the same:
...
-- In the above proof, we can destructure the monoid instances
-- `m₁` and `m₂` with the `obtain` tactic we saw in the Logic
-- chapter. When we do so however, because these are class
-- instances instead of normal structures, we preface our tuple
-- with the `@` symbol. When stepping through the above proof,
-- if the notation is confusing to you, remember that you can
-- set `set_option pp.all true` or
-- `set_option pp.explicit true` to make Lean show you more
-- clearly what is going on. For example, the goal is displayed
-- as `Monoid.id = Monoid.id` since the instances `m₁` and `m₂`
-- are implicit arguments to `Monoid.id`. Setting
-- `pp.explicit true` displays the goal as
...
-- Now, the monoids we described earlier are not groups: there
-- is no inverse operation on the natural numbers such that
-- `∀ x, x + inv x = 0 = inv x + x`, for example. However,
...
-- The study of groups is called *group theory* and is a rich
-- area of mathematics. Here, we will only prove a handful of
-- its simplest results:
...
-- Two groups defined with the same operation over the same set
-- must have the same inverse as well.

theorem inv_unique {α : Type} {g₁ g₂ : Group α} (h : g₁.op = g₂.op) : g₁.inv = g₂.inv := by
  sorry

-- Note to developers (Daniel Sainati @dsainati1):
--     Taking suggestions for additional simple group theory
--     theorems to prove here.
...
-- the maps return. In this section, we'll use the type
-- variable `α` for the type of keys and `β` for values. In
-- addition to `BEq`, which we have already seen, our key type
-- `α` requires instances of the `ReflBEq` and `LawfulBEq`
-- typeclasses:
...
-- inside a `structure` which we call `TotalMap`. Intuitively,
-- a total map just contains a function `inner` from a key of
-- type `α` to a value of type `β`.
...
--     I have clarified here that `MyGetElem` has nothing to do
--     with the encapsulation (`inner` vs `get`). It's simply
--     notation that expands to the public API (`get`).

-- While `TotalMap`s happen to be implemented as functions
-- under the hood, we would prefer not to expose this fact in
-- its public interface. Accordingly, we define new operations
-- for querying and updating mappings. We define a function
-- `get` for getting the value associated with a key playing
-- the role that `find` played for the Lists chapter's
-- list-based maps,
...
-- Here is an example that uses the API lemmas `empty_def` and
-- `get_def`:
...
-- `emptyNatMap.get`. We could notate `get` directly — we'll do
-- exactly that for `update` below — but here we'll instead
-- make "getting an element" its own typeclass, `MyGetElem`,
-- and notate it. Doing so means `m[a]` resolves to
-- `MyGetElem.getElem m a` for any type with a `MyGetElem`
-- instance, not just `TotalMap`.

-- Using typeclasses to define notation is typical in Lean when
-- the same notation is useful for many different types. We
-- have seen the approach already with `==`: writing `a == b`
-- is notation for `BEq.beq`, resolved by instance search for
-- whatever type `a` and `b` have. We also just saw overloaded
-- notation for `EmptyCollection` above, where `∅` is notation
-- for `EmptyCollection.emptyCollection`. Our typeclass
-- `MyGetElem` is a simpler version of the standard library's
-- `GetElem` typeclass, which has many instances such as
-- `Array`, `List`, and `Vector`. We develop it to illustrate
-- the notation-as-typeclass approach.

-- Note to developers (Niklas Halonen @xhalo32):
--     A reason I can come up with why we use a notation
--     typeclass (in library code) over a plain notation is
--     that it makes it possible (for a downstream consumer) to
--     write `open scoped MyGetElem` instead of writing
--     `open scoped TotalMap` and `open scoped PartialMap`
--     individually. This wouldn't be a good sell if `→ₜ` and
--     `→ₚ` are scoped notations, but they're currently global.
...
-- for getting the `m[a]` syntax to work.

-- Don't worry about following the mechanism in detail — the
-- `macro_rules` and the `app_unexpander` below are minor
-- technicalities. However, if you do wish to learn more,
-- Chapter 5 and 6 of [Metaprogramming in Lean
...
-- This design minimizes the need to use `getElem_def` outside
-- concrete examples (which are typically solvable with `rfl`
-- anyways).
...
-- other key is mapped to `false`, like this:
...
-- We can omit the map from the notation when we want it to be
-- empty:
...
-- Let's also see a couple of examples of working with updated
-- maps using rewrites:
...
-- Notice that in the example `exampleMap'["quux"] = false` the
-- last rewrite is effectively just `getElem_empty`.

-- Next, if we update a map `m` at a key `a` with a new value
-- `b` and then look up `a` in the map resulting from the
-- `update`, we get back `b`:
...
-- then look up a *different* key `a₂` in the resulting map, we
...
-- is implemented as a function, this is effectively the
-- functional extensionality principle (`funext`) from the
-- Logic chapter: two maps are equal when they agree at every
-- key. Recording it once, for maps, and tagging it `@[ext]`
-- lets the `ext` tactic reduce a goal `m₁ = m₂` to the
-- pointwise one in the proofs below.
...
-- To demonstrate this extensionality principle, let's look at
-- an example:
...
-- Similarly, if we update a map `m` at a key `a` with a value
-- `b₁` and then update again with the same key `a` and another
-- value `b₂`, the resulting map behaves the same (gives the
-- same result when applied to any key) as the simpler map
...
--     `Reflection` section **below** is about, so the two may
--     want to be connected rather than have one silently
--     displace the other.
--
--     Rocq then says "With the example in chapter **IndProp**
--     as a template, use `String.eqb_spec` to prove ...". That
--     cross-reference is dropped, since it is unclear what the
--     Lean `IndProp` chapter will end up containing. Revisit
--     later.

-- Note to developers (Niklas Halonen @xhalo32):
--     Regarding reflection: I have used
--     `show ("bar" == "foo") = false by simp` in some of the
--     above sections which would be good to explain in more
--     detail in the reflection section. The `BEq` instance is
--     derived from `DecidableEq` in `instBEqOfDecidableEq`.
--     All of the following proofs of the fact use `decide`
--     internally.
--
...
-- function: if we update a map `m` at two distinct keys, it
-- doesn't matter in which order we do the updates.

-- Note to developers (mwhicks1, NOW):
--     Rocq says "Similarly, use `String.eqb_spec` to prove
--     ..."; the instruction to use a specific lemma is dropped
--     here for the same reason as in the note above.
...
--     `apply_empty`) and `update_eq` as (optional) exercises;
--     here they are worked examples, since `update_eq` was
--     already presented that way. Reconsider if this section
--     is rebalanced.
...
-- Wouldn't it be nice if we could use a more natural notation
-- for concrete maps like `{ "bar" ↦ true, "foo" ↦ true }`? To
-- accomplish this we define a simple structure that consists
-- of a key and a value along with `↦` notation for it.
...
-- The reason we need to explicitly specify the type of the map
-- is that Lean doesn't know what type of collection
-- `{ "foo" ↦ true }` is without type hints, as we can see with
-- `#check`:
...
-- infer the type. A type which can't be inferred doesn't have
-- any type classes like `MyGetElem`, so typeclass resolution
-- gets stuck in the following example:
...
-- Note to developers (Niklas Halonen @xhalo32):
--     We should spend some time discussing differences between
--     the inductive approach in Lists.lean and the approach
--     here. The inductive approach could be made polymorphic
--     and proven to be equivalent with partial maps (I
--     believe), so the point is not that the maps are
--     extensionally different. A question (that I don't have
--     an answer to) is then: what makes the new partial map
--     better?

-- Lastly, we define *partial maps* on top of total maps. A
-- partial map with elements of type `β` is simply a total map
...
-- accessing the `inner` field and performing function
-- application application exposes the implementation, and
-- that's why we introduced the API `TotalMap.get`? We follow a
-- similar principle with `PartialMap`s, and define
...
-- direction of `getElem_def` as a `simp` lemma to specify that
-- the `simp` normal form is `m[a]`.

-- Updating a partial map at a key means storing a `some` value
-- there. To update, we create a new partial map from
-- `a →ₜ some b ; m.toTotal` by wrapping it in angle brackets,
-- i.e. using the anonymous constructor syntax. This is
-- equivalent to writing
-- `{ inner := a →ₜ some b ; m.toTotal }`. We also introduce a
-- similar notation for it as for total maps.
...
-- As an example, here's how we can use these on some concrete
-- maps:
...
-- rewrites in the above proof do the computation step-by-step.

example : (2 →ₚ 3)[2] = some 3 := by rfl

-- Next, we lift all of the basic lemmas about total maps to
-- partial maps. To do this we should first prove an
...
-- In this section, we will make use of some definitions and
-- theorems about natural numbers that we discussed and proved
-- in previous chapters; copy your solutions to those problems
-- here:
...
-- yield either `true` or `false` in finite time; and this, in
-- turn, means (by a standard computability argument) that
-- there is no function in `Nat → Bool` that checks whether a
-- given number is the code of a terminating Turing machine.

-- By contrast, the type `Prop` includes both decidable and
-- undecidable mathematical propositions; in particular, the
-- type `Nat → Prop` does contain functions representing
-- properties like "the nth Turing machine halts." The second
-- row in the table follows directly from this essential
-- difference. To evaluate a pattern match (or conditional) on
-- a boolean, we need to know whether the scrutinee evaluates
-- to `true` or `false`; this only works for `Bool`, not
-- `Prop`.

-- The third row highlights an important practical difference:
-- equality functions like `Nat.beq` that return a boolean
-- cannot be used directly to justify rewriting with the
-- rewrite tactic; propositional equality is required for this.
...
-- either as a boolean computation or as a function into Prop.
...
-- computation `n.even` is reflected in the truth of the
-- proposition `∃ (k : Nat), n = k.double`.
...
-- In general, both can be useful. Which we choose has to do
-- with the *computational* nature of Lean's core language,
-- which is designed so that every function it expresses is
-- total, and by default computable unless we explicit indicate
-- otherwise. As an example, consider trying to write a
-- function `α → α → Bool` checking for equality on an
-- arbitrary type:

-- Note to developers:
--     dsainati12 days ago We use regular if for the first time
--     here. It is probably necessary to explain at this point
--     what if is and how it differs from bif.
--
--     👍 1 berberman1 day ago Should we clarify the difference
--     between = and ==? Observably if and bif can possibly
--     accept both as the condition because of Coe or
--     DecidableEq instances, which IMO could be confusing.
--
--     Probably we can talk a bit about the coercion system in
--     this file as well, since Coe could be an example of
--     typeclasses, so long as if we ignore the outParam
--     thing...
--
--     chenson20181 day ago I definitely intended for this to
--     cover = versus ==. Maps uses LawfulBEq (which says = and
--     == coincide). If that will now appear here it's a good
--     place to give some more detail?
--
--     rogerburtonpatel1 day ago I think right after this part
--     on decidability is good. It's a hefty chunk of
--     information already, so keeping distinct ideas distinct
--     is more likely than not a good call.

-- Note to developers:
--     @dsainati - commenting this out for the same reason as
--     above
--
...
-- is the way that we express in Lean that a given proposition
-- is decidable. This is the generalization of our observation
-- that `even_iff_Even` was reflecting a proof between boolean
-- and propositional equality. In fact, we can use this theorem
-- to directly construct a `Decidable` instance

instance (n : Nat) : Decidable (Nat.Even n) := decidable_of_decidable_of_iff even_iff_Even

-- Now we are able to complete such proofs by computation using
-- the `decide` tactic:
...
-- In general, Lean will try to use typeclass synthesis with
-- `Decidable` in order to determine when it is appropriate to
-- use `Prop` and `Bool` interchangeably. For instance, while
-- our example `eq` failed above while trying to use
-- propositional equality `=` in the condition of an `if`
...
-- fact. If we print this definition with notation unset, we
-- would find that it is using `instDecidableEqNat`
...
-- theory enables this computation, Lean is also often used in
-- applications where we don't care about computability, such
-- as pure mathematics. In particular, it is possible to write
-- a function for arbitrary equality:
...
-- use of the axiom of choice to provide a proof that all
-- propositions are *classically* decidable. This sort of
-- definition is suitable for use with proofs, but is not
-- allowed to be used in conjunction with computational
-- features of Lean such as the `decide` tactic or the `#eval`
-- command.
...
--     only carries the proposition and not the boolean, so one
--     direction of `reflect_iff` is easily translated, but the
...
-- Note to developers:
--     Burtonpatel: Some more examples would be good. It might
--     be good to start with Nat and then move to the Indprop
--     ones. This is a short chapter, so 5-6 well-chosen,
--     informative exercises could easily fit.
