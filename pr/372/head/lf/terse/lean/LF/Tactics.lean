...
--  When generated equations do not immediately close the
--  goal, we can add `with` to name the equations to be
--  added to the context (otherwise Lean generates names for
--  us).
