...
--  how it could have been built — i.e., we can *introduce*
--  and *eliminate* `Even` evidence, in the sense of Logic.
--
...
--  Recall from the Logic chapter that equality (`Eq`) is
--  itself an inductively defined proposition, so
--  `inversion` can also be used on equality propositions.
--
