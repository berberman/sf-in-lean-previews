...
--      After `apply EvalR.seq (st' := {X ↦ 2})`, the infoview shows
--      `imp {X := 2}.EvalR ∅ {X ↦ 2}` instead of
...
--  Note to developers (Niklas Halonen @xhalo32):
--      We need to explain the `generalize` tactic. I've changed some Hoare
--      proofs from `have key` to `generalize` but the tactic hasn't been
--      explained yet.

--  Note to developers (One An @meluge):
--      At least currently, it looks like `generalize` in
--      `Automation.lean`. Are we doing anything different here with
--      `generalize` that is unexplained there?
...
--  Many programming languages include mechanisms for raising and handling
--  exceptions. In this problem (a variant of the above exercises on
--  `brk`), we'll experiment with a very simple version, with just a single
--  exception called `throw`. First, we need to enrich the language of
--  commands with an additional case for raising the `throw` exception and
--  a case for catching it.
...
--  Next, we need to define the behavior of `throw`. Informally, whenever
--  `throw` is executed, we immediately stop executing this command and
--  start executing the `catch` clause of the closest enclosing `try`.
--
--  A simple way of achieving this effect is to add another parameter to
--  the evaluation relation that specifies whether evaluation of a command
...
--  state `st`, then it terminates in state `st'` and either signals that
--  an exception has been raised (`s = sThrow`) or that execution can
--  continue normally (`s = sNormal`).
--  - If the command is `skip`, then the state doesn't change and execution
--    of any enclosing loop can continue normally.
--  - If the command is `throw`, the state stays unchanged but we signal a
--    `sThrow`.
--  - If the command is `try`, we begin by executing the first subcommand;
--    if it terminates normally, then the whole `try` terminates in the
--    same state; if it terminates with an exception, we execute the second
--    subcommand instead.
--  - If the command is an assignment, then we update the binding for that
--    variable in the state accordingly and signal that execution can
--    continue normally.
...
--    taken.
--  - If the command is a sequence `c₁ ; c₂`, we first execute `c₁`. If
--    this yields a `sThrow`, we skip the execution of `c₂` and propagate
--    the `sThrow` signal to the surrounding context; the resulting state
--    is the same as the one obtained by executing `c₁` alone. Otherwise,
--    we execute `c₂` on the state obtained after executing `c₁`, and
--    propagate the signal generated there.
--  - Finally, for a loop of the form `while (b) {c}`, when `b` evaluates
--    to `true`, we execute `c` and check the signal that it raises. If
--    that signal is `sNormal`, the execution proceeds as in the original
--    semantics. Otherwise, we stop the execution of the loop and signal
--    `sThrow`.
--
--  Based on the above description, complete the definition of the
--  `Com.EvalR` relation:
