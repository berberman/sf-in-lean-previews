...
--  Prove a specification for `subtract_slowly`, using the
--  above specification of `factCom` and the invariant below
--  as guides.
...
--  short-circuit evaluation of `Bexp.and` in this manner,
--  and prove that it is equivalent to `Bexp.eval`. (N.b.
--  This is only true because expression evaluation in Imp
--  is rather simple. In a bigger language where evaluating
--  an expression might diverge, the short-circuiting `and`
...
--  - If the command is of the form `if (b) {c₁} else {c₂}`,
--    then the state is updated as in the original semantics
--    of Imp, except that we also propagate the signal from
--    the execution of whichever branch was taken.
...
--  ### Exercise (4 stars): exn_imp (Advanced, Optional) ⭐⭐⭐⭐

--  Many programming languages include mechanisms for
--  raising and handling exceptions. In this problem (a
--  variant of the above exercises on `brk`), we'll
--  experiment with a very simple version, with just a
--  single exception called `throw`. First, we need to
--  enrich the language of commands with an additional case
--  for raising the `throw` exception and a case for
--  catching it.
...
--  Next, we need to define the behavior of `throw`.
--  Informally, whenever `throw` is executed, we immediately
--  stop executing this command and start executing the
--  `catch` clause of the closest enclosing `try`.
--
--  A simple way of achieving this effect is to add another
--  parameter to the evaluation relation that specifies
--  whether evaluation of a command executes a `throw`
--  statement:
...
--  Intuitively, `st =[ c ]=> st' // s` means that, if `c`
--  is started in state `st`, then it terminates in state
--  `st'` and either signals that an exception has been
--  raised (`s = sThrow`) or that execution can continue
--  normally (`s = sNormal`).
--  - If the command is `skip`, then the state doesn't
--    change and execution of any enclosing loop can
--    continue normally.
--  - If the command is `throw`, the state stays unchanged
--    but we signal a `sThrow`.
--  - If the command is `try`, we begin by executing the
--    first subcommand; if it terminates normally, then the
--    whole `try` terminates in the same state; if it
--    terminates with an exception, we execute the second
--    subcommand instead.
--  - If the command is an assignment, then we update the
--    binding for that variable in the state accordingly and
--    signal that execution can continue normally.
...
--    the execution of whichever branch was taken.
--  - If the command is a sequence `c₁ ; c₂`, we first
--    execute `c₁`. If this yields a `sThrow`, we skip the
--    execution of `c₂` and propagate the `sThrow` signal to
--    the surrounding context; the resulting state is the
--    same as the one obtained by executing `c₁` alone.
--    Otherwise, we execute `c₂` on the state obtained after
--    executing `c₁`, and propagate the signal generated
--    there.
--  - Finally, for a loop of the form `while (b) {c}`, when
--    `b` evaluates to `true`, we execute `c` and check the
--    signal that it raises. If that signal is `sNormal`,
--    the execution proceeds as in the original semantics.
--    Otherwise, we stop the execution of the loop and
--    signal `sThrow`.
--
--  Based on the above description, complete the definition
--  of the `Com.EvalR` relation:
...
--  Now prove the following properties of your definition of
--  `Com.EvalR`:
...
--  update the `Com.EvalR` definition to define the
--  semantics of `for` loops, and add cases for `for` loops
--  as needed so that all the proofs in this file are
--  accepted by Lean.
--
