...
-- Let's start with a very simple example. The following declaration
-- introduces a new type and lists its possible values.
...
-- definitions explicitly list all of their values, one constructor at a time.
-- Here is a more interesting inductive type definition, `Color`, where one of
...
-- built from them — are finite. The natural numbers, on the other hand, form
-- an infinite type, so we'll need to use a slightly richer form of inductive
