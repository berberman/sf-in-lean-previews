...
--      simplified" does not parse, but I'm not sure whether to add "will"
--      or "can" or something else...
...
--      Yipeng said we can pass a theorem, e.g. `dsimp [Nat.mul_zero]`,
--      which would rewrite `Nat.mul_zero` many times and then perform
--      reductions, just like simp `[Nat.mul_zero]`. Also `@[defeq] lemmas`
--      in the `simp` set are always used implicitly.
--
--      Should `dsimp [Nat.mul_zero]` be preferred over `dsimp [Nat.mul]`?
--      An example:
--
...
--        dsimp [Nat.mul_zero]`
--
--      This could be confusing though, because rewriting by `dsimp` only
--      works for *definitional* equalities. The following doesn't work
--
--      `example (n : Nat) : (((0 * n) * n) * n) = 0 := by
--        dsimp [Nat.zero_mul]`
--
--      This is because `Nat.zero_mul` is true by induction, not reduction.
--      This is a bit confusing to explain, and also unfortunate since one
--      may not know why an equality holds. Thus I'd prefer not to include
--      this use here.
