...
-- Note to developers (Mike Hicks @mwhicks1):
--     This is the first time we've seen implicit arguments like `{n : Nat}`,
--     and they're used pervasively from here on (`cons_append`, `head_cons`,
--     `count_nil`, etc.). We should either introduce implicit arguments
--     explicitly here (or earlier, e.g., an exercise in UsingLean) or
--     restructure so their proper explanation — currently in Poly — comes
--     before this chapter.
...
-- our own `append` function, we can register it as the `++` operator within
-- our namespace:
...
-- while `tail` returns everything but the first element (the "tail"). Since
-- the empty list has no first element, we pass a default value to be returned
-- in that case.
...
-- In fact, as the entire proof is just plain computation, it can be done with
...
-- constructors to one another are the *only* possible shapes that elements of
-- an inductively defined set can have.

-- This last fact directly gives rise to a way of reasoning about inductively
-- defined sets: a number is either `0` or else it is `Nat.succ` applied to
-- some *smaller* number; a list is either `[]` or else it is `::` applied to
-- some number and some *smaller* list; etc. Thus, if we have in mind some
...
-- we use `append` to define a list-reversing function `reverse`:
...
-- `involutive_injective`. (But: Don't try to use that exercise directly as a
...
-- This solution is not so good: If `nthBad` returns 42, we don't know whether
-- that value actually appears in the input or whether we gave bad arguments.
-- A better alternative is to change the return type to include an error value
-- as a possible outcome. We call this new type `NatOption`.
