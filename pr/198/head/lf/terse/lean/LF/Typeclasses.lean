...
-- This works, but as before it's tedious: every caller has to
-- supply an element of `α` to default to, even when there's an
-- obvious choice, like `0` for `Nat`.
...
-- The other is giving Lean some information about what to
-- search *for*: a bare `α` argument carries no content, so any
-- value of `α` would do equally well.

-- Considering the second problem first: our solution is to
-- *name* the type-level concept we're actually after — the
-- **default value** of a type. A `structure` (chapter Lists)
-- is a good way to give a type-level concept a name;
-- structures can also bundle together more than one piece of
-- data, which will come in handy later, though we only need a
-- single field here.
...
-- Let's build this in two steps: first the naming, as a plain
-- `structure`; then the marking, by upgrading it to a `class`.
-- Here's the structure — we'll put it in its own namespace so
-- we can reuse the name `DefaultValue` for the class version
-- below:
...
-- A value of `DefaultValue Nat` picks out a particular `Nat`
-- to serve as the type's default: it's built the same way any
-- structure is, by supplying a `Nat` for the `value` field:
...
-- Another difference is in how we provide values of this type.
...
-- `DefaultValue.value` with a `Nat`, Lean selects
-- `instDefaultValueNat`, the instance for `DefaultValue Nat`.
-- We know that it is this instance because we are able to
...
-- form of the `GetElem` standard library function). It's the
-- same pattern behind `==`: writing `a == b` doesn't call a
-- fixed function, it's notation for `BEq.beq`, resolved by
-- instance search for whatever type `a` and `b` have. We're
-- about to do the same thing for indexing notation, resolving
-- `m[a]` to `MyGetElem.getElem m a`, so that it works for any
-- container type with a `MyGetElem` instance, not just
-- `TotalMap`. We see the pattern again at the conclusion of
-- our development of total maps, illustrating custom syntax
...
--
--     Explanation of the infinite loop by Yipeng Liu
--     (berberman): To see why, with
--     `trace.Meta.Tactic.simp.rewrite` enabled and specifying
--     maxSteps to simp, we can track what's going on:
--
...
--         ==>...`
--
--     So I think the issue here is that the LHS m a is too
--     board, causing this lemma repeating itself. First we
--     have `m a => m[a], but m[a]` is defeq to
--     (MyGetElem.getElem m) a, and nevertheless we require m
--     to be a TotalMap, a TotalMap is defeq to α → β. So this
--     lemma applies again --
--     `(MyGetElem.getElem m) a => (MyGetElem.getElem m)[a]`
