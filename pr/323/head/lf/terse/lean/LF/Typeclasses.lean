...
--  implementations of that class are called *instance*;
...
--  Comparing `List.elem_poly_eq` with `List.elem_poly`, we
--  see three differences. First, `List.elem_poly_eq` takes
--  an *explicit* parameter `eq`, whereas `List.elem_poly`
...
--  is used. Second, whereas `List.elem_poly_eq` invokes
--  parameter `eq` to test equality, `List.elem_poly` uses
--  `==` instead. As Lists noted when we first used it, `==`
--  on `Nat` comes from the `BEq` typeclass. Finally,
--  whereas `[0, 1].elem_poly_eq Nat.beq 0` passes the
...
--  error message: `α` was fully generic — so the `==` in
--  its body would have needed to work for *every* type `α`,
--  and no single `BEq` instance can do that. So Lean's
--  search failed.
...
--  list is empty. As with `List.elem_poly_eq` above, here
--  is a version that makes the default value an explicit
--  parameter:
...
--  `List.headOr_ex`'s `defaultValue` argument. We do this
--  by writing `class` in place of `structure`:
...
--  can rewrite `List.headOr_ex` the same way we rewrote
--  `List.elem_poly_eq` into `List.elem_poly` above,
--  replacing the explicit `defaultValue` parameter with an
--  instance implicit:
...
--  type. First, though, let's go back to `List.elem_poly`
...
--  of `BEq` for the type of `a` and `b`, the same way it
...
--  `List.elem_poly` is called on a `List Nat` — no
--  different from Lean choosing `instDefaultValueNat` for
--  `DefaultValue.value` earlier when it was equated with
--  `(1 : Nat)`.

--  ### Exercise (1 star): List.elem_poly_eq_elem_nat ⭐

--  Prove that `List.elem_poly` agrees with `List.elem_nat`
...
--  we preface our tuple with the `@` symbol. When stepping
...
--  writing `a == b` is notation for `BEq.beq`, resolved by
--  instance search for whatever type `a` and `b` have. We
...
--  the map implementation, its keys, and its values.
...
--  extensionality axiom `funext`, discussed in the Logic
--  chapter.)
...
--  our observation that `even_iff_Even` was reflecting a
--  proof between boolean and propositional equality. In
