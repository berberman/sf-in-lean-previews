...
-- We wrote Lean that enforced this pedagogical style using the
...
-- Definitions in `Nat` are not marked `@irreducible`. This
-- lets us use more powerful *automatic simplification* of
-- functions on natural numbers, which is appropriate when
-- their low-level behaviors are not the primary focus of
...
-- For the remainder of the book, we will use Lean's built-in
-- natural numbers everywhere. We will use `dsimp`, `calc`, and
-- in examples and solutions, and encourage their use. We
-- encourage the use of `rw?` and `exact?` to search for lemmas
-- (though they should not appear in finished proofs). With
-- these tools in hand, we can begin to prove properties about
-- more sophisticated forms of data, beginning with `Lists`.
