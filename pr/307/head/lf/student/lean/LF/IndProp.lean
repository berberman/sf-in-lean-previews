...
--  ### Exercise (1 star): EquivGen (Optional, manually graded) ⭐

--  How would you modify the `ReflTransGen` definition above to define the
--  reflexive, symmetric, and transitive closure—in other words, the
--  equivalence closure?
...
--    e.g., `n' = n` in the proof of `Even.succ_succ_even`.
...
--  the form `Ev.ev_succ_succ n' h'`, where `n = n' + 2` and `h'` is
...
--  A proposition parameterized by a number (such as `Even`) can be thought
