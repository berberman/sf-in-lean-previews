...
-- The following lemma proves `even (n + 1)` flips the parity, which gives an
-- alternative characterization of `even (succ n)` that works better with
-- induction.
...
--     I don't understand why this lemma is helping us with induction. Might
--     it be better to just offer the lemma as an exercise, and then refer
--     back to it when you might actually use it (in an exercise), to
--     illustrate?
...
-- representations, if you do `binToNat` then `natToBin`, you don't get back
-- what you started with.

-- Any other number also has many representations, after applying constructors
-- to the multiple representations of zero.
