...
-- Note to developers:
--     HIDE: Assertion-level arith expressions. (BCP: Not sure
--     this is an optimally clear name.)
--
--     One An (meluge): the Rocq development calls this type
--     `Aexp`, in contrast to the object-level `aexp`. In Lean
--     the object-level syntax type is named `Aexp` in Imp, so
--     we use `Aexp'` for the assertion level version.
...
-- We will sometimes need to lift functions to operate on
