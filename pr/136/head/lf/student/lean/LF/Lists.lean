...
-- define list notation, allowing us to write `[1, 2]` instead of
-- `1 :: 2 :: []`. We also have an unexpander that prints `cons 1 (cons 2 [])`
-- to `[1, 2]`.
...
theorem countOddMembers_def (l : NatList) :
    countOddMembers l = (oddMembers l).length := sorry
