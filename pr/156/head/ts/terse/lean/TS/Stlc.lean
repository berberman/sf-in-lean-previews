...
-- *lambda cube* (also called the *Barendregt Cube*), which
...
-- polymorphism gives us the famous Girard-Reynolds calculus,
-- System F.

-- Moving from front to back corresponds to adding *type
-- operators* like `List`.

-- Moving from left to right corresponds to adding *dependent
-- types* like `∀ n, ArrayOfSize n`.

-- The top right corner on the back, which combines all three
-- features, is called the *Calculus of Constructions*. First
-- studied by Coquand and Huet, it forms the foundation of
-- Lean's logic.
...
-- The identity function for booleans, applied to the boolean
-- `true`.
...
-- We need some notation magic to set up the concrete syntax,
-- as we did in the Types chapter...
...
-- all it takes to add a new type construct.
...
-- antiquote in a nested quotation says which grammar it came
-- from. A bare identifier is the one genuinely overlapping
-- case: `Bool` in term position would otherwise quietly become
-- a variable named `Bool`, so that rule rejects it, which also
...
-- substitution below. It binds tighter than application, so
-- `[x:=s] t1 t2` is the application of `[x:=s] t1` to `t2`,
-- and a `λ` or `if` body must be parenthesized:
-- `[x:=s] (λ y : Bool . x)`.
...
-- terms appearing in goals and in `#check` output print as
-- `<{ λ x : Bool . x }>` rather than as a pile of
-- constructors. (Setting `pp.notation false` turns it off,
-- revealing the underlying representation.)
...
-- Note to developers:
--     The Rocq source writes these as `Notation`s rather than
--     `Definition`s, so that `auto` sees through them for
--     free. Our convention (STYLE, "Definitions vs.
--     Abbreviations") is a `def` plus characterizing lemmas,
--     so the places that depend on what a name stands for are
--     visible in the proof rather than left to the elaborator.

-- Note that an abstraction `λ x : T . t` (formally, `Tm.abs`
-- applied to `x`, `T`, and `t`) is always annotated with the
-- type `T` of its parameter, in contrast to Lean (and other
-- functional languages like ML, Haskell, etc.), which use type
-- inference to fill in missing annotations. We're not
...
-- - Next, we define *free variables* and *substitution*. These
--   are used in the reduction rule for application expressions.
...
-- First, for the boolean part of the language, the situation
-- is clear: `true` and `false` are the only values. An `if`
-- expression is never a value.

-- Second, an application is not a value: it represents a
-- function being invoked on some argument, which clearly still
-- has work left to do.
...
--     then lean on `auto`/`eauto` to assemble derivations. We
--     have no counterpart here: the proofs below name their
--     constructors explicitly, in the style of the Types
--     chapter. Lean's `grind` would be the closest analogue if
--     a later pass wants automation.
...
-- Having made the choice not to reduce under abstractions, we
-- don't need to worry about whether variables are values,
-- since we'll always be reducing programs "from the outside
-- in," and that means the `step` relation will always be
...
-- operation is used below to define the operational semantics
-- of function application, where we will need to substitute
-- the argument term for the function parameter in the
...
-- by substituting `false` for the parameter `x` in the body of
-- the function.

-- In general, we need to be able to substitute some given term
-- `s` for occurrences of some variable `x` in another term
-- `t`. Informally, this is written `[x:=s]t` and pronounced
...
-- `true` in `λx:Bool. x` does *not* yield `λx:Bool. true`! The
-- reason for this is that the `x` in the body of `λx:Bool. x`
-- is *bound* by the abstraction: it is a new, local name that
-- just happens to be spelled the same as some global name `x`.
...
-- we consider the case where `s`, the term being substituted
-- for a variable in some other term, may itself contain free
-- variables. We say that `s` is an *open* term.

-- Here is an example. Using the above definition to substitute
-- the open term

--   s = λx:Bool. r

-- (where `r` is a *free* reference to some global resource)
-- for the free variable `z` in the term
...
-- where the free reference to `r` in `s` has been "captured"
...
-- example, if we rename the bound variable in `t`, e.g., let
...
-- which does not behave the same as the substituting in the
-- original `t`:

--   [z:=s]t = λr:Bool. λx:Bool. r

-- That is, renaming a bound variable in `t` would change how
-- `t` behaves under our simple substitution. So substitution
-- gets more complicated in that setting, but fortunately we
-- don't have that problem in our STLC variant.

-- Fortunately, since we are only interested here in defining
-- the `step` relation on *closed* terms (i.e., terms like
-- `λx:Bool. x` that include binders for all of the variables
-- they mention), we can sidestep this extra complexity, but it
-- must be dealt with when formalizing richer languages.
...
-- This is *call by value* reduction: to reduce an application
-- `(t1 t2)`, we
...
-- (Note that this term doesn't actually typecheck; even so, we
-- can ask how it reduces.)
...
--     type a term `x y` if we've just said that we will just
--     look at closed terms as our programs?
...
-- We can read the three-place relation `Γ ⊢ t ⦂ T` as: "under
-- the assumptions in Γ, the term `t` has the type `T`."
...
-- rather than as a `HasType` applied to a chain of map
-- updates.
...
--     `eauto`, having registered the `has_type` constructors
--     in the `core` hint database; it also observes that plain
--     `auto` suffices here because the term contains no
--     application nodes. We have no hint database, so both
--     derivations are given explicitly.
...
-- Note to developers:
--     The Rocq proof gets to the same contradiction through a
--     chain of `inversion`s and then an induction on the
--     offending type; the `LATER` note there asks why
--     `eauto 30` makes no progress on the previous example,
--     and a `NOTATION` note from Ori reports an error with the
--     associativity of the arrow in one of the inversion
