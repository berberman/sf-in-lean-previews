...
-- So far we have discussed **parametric polymorphism**, where in Lean we can
-- declare a type variable

variable (α : Type)

-- without any further information about the details of the type `α`. Here we
-- are able to work with a type like `List α`, and can write functions like
-- `List.reverse` or `List.length` that only operate on the structure of a
-- list independently of its contents. Along with these we have corresponding
-- proofs like `List.length_reverse` that are similarly structural in nature.

-- This is somewhat limiting, as we cannot inspect or non-trivially use any
-- particular `a : α`. What we might want in some situations rather than `α`
-- being completely generic is to partially specify its behavior. In Lean,
-- this happens through a form of "ad hoc polymorphism" called
-- **typeclasses**. This concept originated in the Haskell programming
-- language, and is analogous to features you may be familiar with in other
-- languages such as traits in Rust.
...
-- As discussed previously, a structure is just an inductive type that comes
-- with a single constructor, in this case `HasOneStruct.mk`, along with a few
-- different syntaxes for convenience:
...
-- can use this in proofs such as

theorem nat_hasOneStruct_eq_one : nat_hasOneStruct_mk.one = 1 := rfl

-- to see how this structure provably contains the value that we have placed
-- within it. In Lean, typeclasses work very similarly, and in fact are
-- implemented as structures. The only difference in defining a typeclass is
-- that we instead use the `class` keyword:

class HasOne (α : Type) where
  one : α

-- While the definition is essentially the same, including the constructor
-- `HasOne.mk`, the usage of typeclasses is different, and what allows us to
-- use them to express ad hoc polymorphism. Instead of using `def` to define
-- inhabitants of this type, we will instead use the `instance` keyword:

-- Note to developers:
--     From GitHub (@chenson2018): Part of the point here is that we don't
--     usually refer to instances explicitly, even me providing an explicit
--     name is for pedagogy reasons. I'll make it more explicit in the text
--     that what we're looking for in the infoview is the presence of this
--     instance.

instance instHasOneNat : HasOne Nat where
  one := 1

-- The definition is exactly the same as above (and any of the alternative
-- syntaxes work as well), but instead of being used as a definition that we
-- pass around, Lean will try to infer its usage through a process called
-- **typeclass synthesis** or **typeclass inference**. To help see this, let's
-- define another instance of `HasOne` for `Int`, the type of integers ... -2,
-- -1, 0, 1, 2, ...

instance instHasOneInt : HasOne Int where
  one := -1

-- Instead of explicitly referring to a structure like we did in the proof
-- `nat_hasOneStruct_eq_one`, we can instead use `HasOne.one`, and Lean will
-- use typeclass inference to determine the intended meaning. As an example,
-- you can write

example : HasOne.one = (1 : Nat) := rfl
example : HasOne.one = (-1 : Int) := rfl

-- and based on the type annotations Lean will infer which of these instances
-- you intended to use. So these proofs are equivalent to

example : instHasOneNat.one = (1 : Nat) := rfl
example : instHasOneInt.one = (-1 : Int) := rfl

-- but without needing to explicitly refer to the instances `instHasOneNat`
-- and `instHasOneInt`. Another way that we can inspect what instance is being
-- using with the constructor of a typeclass is the option `pp.all`. For
-- example looking at
...
-- we can see that `@HasOne.one Nat instHasOneNat` and
-- `@HasOne.one Int instHasOneInt` are the respective terms that appear on the
-- right hand side of each equality.

-- Lean also has a command `#synth` that allows you to search for an instance
-- of a given type. So the command

#synth HasOne Nat

-- for instance prints `instHasOneNat`, informing us that this is the instance
-- found that has type `HasOne Nat`. In general, the assumption is that for
-- typeclasses like `HasOne` that include data, as opposed to solely proofs,
-- that these these instances should be unique to avoid ambiguity.

-- Note to developers:
--     @chenson2018: I don't really want to explain diamonds here, is the
--     above white lie hand-waving okay??

-- ## Proof-Carrying Typeclasses

-- Notably, the above examples enforce no conditions on the data that they
-- carry, and we could provide any value that we would like for `HasOne.one`.
-- This could be misleading, for instance if we had instead defined an
-- instance
...
-- we might have some counterintuitive proofs based on the name leading us to
-- believe the class contained the element `1`. In most languages that support
-- typeclasses it is not possible to enforce a notion of "lawfulness" as part
-- of the typeclass, and it falls to the author to ensure that any desired
-- invariants are satisfied.

-- As an example, suppose that we wanted to express the idea that a type has
-- at least two distinct elements. A first attempt might be
...
-- but this does not disallow the case where `one` and `two` are equal. In a
-- proof assistant, however, typeclasses can also carry proofs along with
...
-- to enforce that these are two distinct entries. Declaring instances works
-- in much the same way as before, except that now `HasTwo.one_neq_two`
-- requires a proof:
...
-- Following the pattern of `HasOne` and `HasTwo`, define a class `HasThree`
-- that specifies a type with at least three distinct elements.

-- Note to developers (Claude, NOW):
--     Rendering bug in this exercise and `instHasThree` below. Both use the
--     `-- SOLUTION`/`-- END SOLUTION` comment-marker idiom to hide **some**
--     class fields / instance fields, but the Verso HTML build does not
--     process those markers (only the `solution!` tactic is handled). In
--     **student** and **terse** the class shows only the visible fields while
--     the `instHasThree` instance then reports a spurious
--     `Fields missing: one_neq_three, two_neq_three` error; in **solutions**
--     the hidden fields are shown but the literal `-- SOLUTION` /
--     `-- END SOLUTION` comment lines leak into the displayed code. The
--     generated `.lean` is fine — HTML-only. Fix by expressing the hidden
--     fields with `solution!` rather than the comment markers.
...
-- Another use for typeclasses, which may be familiar from other languages, is
-- to overload a given piece of syntax. To start, consider the
...
-- This function takes a natural number `a` and a list of natural numbers
-- `xs`, then returns a `Bool` indicating if `a` occurs within `xs`. For
-- example:
...
-- If we wanted to construct a polymorphic version of this, how would we
-- proceed? If we try to simply replace `Nat` with a type variable `α`, we get
-- a somewhat mysterious error

-- Note to developers:
--     @dsainati - The Verso compilation is not actually removing this from
--     the generated file despite th e -keep flag. It is, however, stripping
--     the message guard, so this results in an error. Once we figure out how
--     we are handling these cases, uncomment this.
--
--     `/--
...
-- that mentions a typeclass `BEq`. Perhaps a better question is what exactly
-- the `==` notation meant in our original `List.elem_nat`. Let's look at a
-- usage of this notation with natural numbers, but turn off notation:

set_option pp.notation false in
#check 1 == 2

-- This may be a bit surprising, as you may have been expecting to see
-- `Nat.beq`, the function for boolean equality on natural numbers. What we
-- instead find is that `==` is a notation for the typeclass `BEq`:

-- class BEq (α : Type u) where
--   /-- Boolean equality, notated as `a == b`. -/
--   beq : α → α → Bool

-- What is happening in the background when we use the `a == b` notation is
-- that Lean is searching for an *instance* of the `BEq` typeclasses that
-- applies for `Nat`. An example of such an instance would be

instance (priority := low) : BEq Nat where
  beq := Nat.beq

-- where we are specifying that the usage of `==` for natural numbers should
-- correspond to the expected `Nat.beq`. So in order to write a function that
-- uses the notation for boolean equality over a generic type, we need a way
-- to express that there is some instance for `BEq α`. This is done using
-- **instance implicits**, where we place a desired typeclass assumption in
-- square brackets. Our corrected definition is then
...
-- where Lean can now infer the usage of `==` is the one that we have
-- provided.
...
-- them in many places in the coming chapters.

-- We'll define two flavors of maps: total maps, which include a "default"
...
-- provide the assumption that we have an boolean equality (`==`) on our keys
-- that is reflexive and coincides with proposition equality `=`.

-- ### Total Maps

-- Our main job in this chapter will be to build a definition of partial maps
-- that is similar in behavior to the one we saw in the Lists chapter, plus
-- accompanying lemmas about its behavior.

-- This time around, though, we're going to use functions, rather than lists
-- of key-value pairs, to build maps. The advantage of this representation is
-- that it offers a more "extensional" view of maps: two maps that respond to
-- queries in the same way will be represented as exactly the same function,
-- rather than just as "equivalent" list structures. This simplifies proofs
-- that use maps.

-- We build up to partial maps in two steps. First, we define total maps that
-- return a default value when we look up a key that is not present in the
-- map.
...
-- typeclass, which is the standard library's implementation of our `HasOne`
-- example from above:
...
--     This introductory bit seems heavier than necessary. Why not just say
--     that we want to treat TotalMap as an abstract thing, not rely on the
--     fact that it is defined in terms of functions, and then go ahead and do
--     that?

-- To access values in a map (i.e., a function), we can apply the map to a
-- key:
...
-- as a function type, so technically Lean lets us use a function application
...
-- In the ideal, we should be able to substitute a different type with similar
-- behavior without needing to change the public interface. Suppose, just for
-- the sake of argument, that we wanted to define total maps as `List (α × β)`
-- or `Std.HashMap α β`. Neither of these are functions, so the syntax `∅ 1`
-- wouldn't work. If this is the public interface to access elements, it
-- restricts the definition ot `TotalMap`, forcing it to be a function of some
-- kind.
...
-- parameter with a hint to Lean that helps typeclass inference). The
...
-- To use the notation `m[a]` to access elements of a map `m`, we add a
-- `MyGetElem` instance for total maps.
...
-- When proving some of the characterizing lemmas below, we will rewrite using
-- `getElem_def`. However, `getElem_def` exposes the underlying implementation
-- that accessing elements in total maps is a function application, therefore
-- it should only be used sparingly, and only inside the `TotalMap` namespace.
...
-- More interesting is the map-updating function, which (as always) takes a
-- map `m`, a key `a`, and a value `b` and returns a new map that takes `a` to
-- `b` and takes every other key to whatever `m` does. The novelty here is
-- that we achieve this effect by wrapping a new function around the old one.
...
-- takes a function `m` and yields a new function `fun x' ↦ ...` that behaves
...
-- Even if you don't work the following exercises, make sure you thoroughly
-- understand the statements of the lemmas!
...
-- Wouldn't it be nice if we could use a more natural notation for concrete
-- maps like `{ "bar" ↦ true, "foo" ↦ true }`? To accomplish this we define a
-- simple structure that consists of a key and a value along with `↦` notation
-- for it.
...
-- Next, we declare `Insert` and `Singleton` instances which control the `{}`
-- notation in lean.
...
-- The reason we need to explicitly specify the type of the map is that Lean
-- doesn't know what type of collection `{ "foo" ↦ true }` is without type
-- hints, as we can see with `#check`:

#check { "foo" ↦ true }

-- {"foo" ↦ true} : ?m.4

-- The type shows a `?m.4`, which indicates that Lean can't infer the type. A
-- type which can't be inferred doesn't have any type classes like
...
--
--     Niklas Halonen (xhalo32): This is just a technical note that
--     discourages using `PartialMap.inner` over `PartialMap.toTotal`. We
--     don't want both to appear in the public API. Compare with
--     `MeasurableSet'` in
...
--     which doesn't appear in the public API (it uses `MeasurableSet`
--     instead).
...
-- wrapping it in angle brackets, i.e. using the anonymous constructor syntax.
...
-- Next, we lift all of the basic lemmas about total maps to partial maps. To
-- do this we should first prove an extensionality lemma about partial maps.
...
-- finite time; and this, in turn, means (by a standard computability
-- argument) that there is no function in `Nat → Bool` that checks whether a
...
-- halts." The second row in the table follows directly from this essential
-- difference. To evaluate a pattern match (or conditional) on a boolean, we
-- need to know whether the scrutinee evaluates to `true` or `false`; this
-- only works for `Bool`, not `Prop`.

-- The third row highlights an important practical difference: equality
-- functions like `Nat.beq` that return a boolean cannot be used directly to
-- justify rewriting with the rewrite tactic; propositional equality is
-- required for this. Since `Prop` includes both decidable and undecidable
-- properties, we have two options when we want to formalize a property that
-- happens to be decidable: we can express it either as a boolean computation
...
-- In general, both can be useful. Which we choose has to do with the
-- *computational* nature of Lean's core language, which is designed so that
-- every function it expresses is total, and by default computable unless we
...
-- interchangeably. For instance, while our example `eq` failed above while
...
-- Why is this allowed? It is precisely because equality of natural numbers is
-- decidable, and Lean makes use of this fact. If we print this definition
-- with notation unset, we would find that it is using `instDecidableEqNat`
...
-- This is only half the story however: while Lean's core theory enables this
-- computation, Lean is also often used in applications where we don't care
-- about computability, such as pure mathematics. In particular, it is
-- possible to write a function for arbitrary equality:
...
-- `Classical` namespace that we are *not* interested in computation. What is
-- happening in the background is that this allows typeclass synthesis to find
-- the scoped instance `Classical.propDecidable`, which makes use of the axiom
-- of choice to provide a proof that all propositions are *classically*
-- decidable. This sort of definition is suitable for use with proofs, but is
-- not allowed to be used in conjunction with computational features of Lean
-- such as the `decide` tactic or the `#eval` command.
