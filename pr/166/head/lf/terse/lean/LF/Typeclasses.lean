...
-- So far we have discussed **parametric polymorphism**, where
-- in Lean we can declare a type variable

variable (α : Type)

-- without any further information about the details of the
-- type `α`. Here we are able to work with a type like
-- `List α`, and can write functions like `List.reverse` or
-- `List.length` that only operate on the structure of a list
-- independently of its contents. Along with these we have
-- corresponding proofs like `List.length_reverse` that are
-- similarly structural in nature.

-- This is somewhat limiting, as we cannot inspect or
-- non-trivially use any particular `a : α`. What we might want
-- in some situations rather than `α` being completely generic
-- is to partially specify its behavior. In Lean, this happens
-- through a form of "ad hoc polymorphism" called
-- **typeclasses**. This concept originated in the Haskell
-- programming language, and is analogous to features you may
-- be familiar with in other languages such as traits in Rust.
...
-- As discussed previously, a structure is just an inductive
-- type that comes with a single constructor, in this case
-- `HasOneStruct.mk`, along with a few different syntaxes for
-- convenience:
...
-- All of these are the same value, a structure of type
-- `HasOneStruct Nat` that we can thing of as carrying a piece
-- of data (a term that is not a proposition), the number `1`,
-- as evidence that this type is not empty. We can use this in
-- proofs such as

theorem nat_hasOneStruct_eq_one : nat_hasOneStruct_mk.one = 1 := rfl

-- to see how this structure provably contains the value that
-- we have placed within it. In Lean, typeclasses work very
-- similarly, and in fact are implemented as structures. The
-- only difference in defining a typeclass is that we instead
-- use the `class` keyword:

class HasOne (α : Type) where
  one : α

-- While the definition is essentially the same, including the
-- constructor `HasOne.mk`, the usage of typeclasses is
-- different, and what allows us to use them to express ad hoc
-- polymorphism. Instead of using `def` to define inhabitants
-- of this type, we will instead use the `instance` keyword:
...
-- The definition is exactly the same as above (and any of the
-- alternative syntaxes work as well), but instead of being
-- used as a definition that we pass around, Lean will try to
-- infer its usage through a process called **typeclass
-- synthesis** or **typeclass inference**. To help see this,
-- let's define another instance of `HasOne` for `Int`, the
-- type of integers ... -2, -1, 0, 1, 2, ...

instance instHasOneInt : HasOne Int where
  one := -1

-- Instead of explicitly referring to a structure like we did
-- in the proof `nat_hasOneStruct_eq_one`, we can instead use
-- `HasOne.one`, and Lean will use typeclass inference to
-- determine the intended meaning. As an example, you can write

example : HasOne.one = (1 : Nat) := rfl
example : HasOne.one = (-1 : Int) := rfl

-- and based on the type annotations Lean will infer which of
-- these instances you intended to use. So these proofs are
-- equivalent to

example : instHasOneNat.one = (1 : Nat) := rfl
example : instHasOneInt.one = (-1 : Int) := rfl

-- but without needing to explicitly refer to the instances
-- `instHasOneNat` and `instHasOneInt`. Another way that we can
-- inspect what instance is being using with the constructor of
-- a typeclass is the option `pp.all`. For example looking at
...
-- `@HasOne.one Int instHasOneInt` are the respective terms
-- that appear on the right hand side of each equality.

-- Lean also has a command `#synth` that allows you to search
-- for an instance of a given type. So the command

#synth HasOne Nat

-- for instance prints `instHasOneNat`, informing us that this
-- is the instance found that has type `HasOne Nat`. In
-- general, the assumption is that for typeclasses like
-- `HasOne` that include data, as opposed to solely proofs,
-- that these these instances should be unique to avoid
-- ambiguity.
...
-- Another use for typeclasses, which may be familiar from
-- other languages, is to overload a given piece of syntax. To
-- start, consider the (non-polymorphic) definition
...
-- This function takes a natural number `a` and a list of
-- natural numbers `xs`, then returns a `Bool` indicating if
-- `a` occurs within `xs`. For example:

#eval [0, 1].elem_nat 0
#eval [0, 1].elem_nat 1
#eval [0, 1].elem_nat 2

-- If we wanted to construct a polymorphic version of this, how
-- would we proceed? If we try to simply replace `Nat` with a
-- type variable `α`, we get a somewhat mysterious error
...
-- that mentions a typeclass `BEq`. Perhaps a better question
-- is what exactly the `==` notation meant in our original
-- `List.elem_nat`. Let's look at a usage of this notation with
-- natural numbers, but turn off notation:

set_option pp.notation false in
#check 1 == 2

-- This may be a bit surprising, as you may have been expecting
-- to see `Nat.beq`, the function for boolean equality on
-- natural numbers. What we instead find is that `==` is a
-- notation for the typeclass `BEq`:

-- class BEq (α : Type u) where
--   /-- Boolean equality, notated as `a == b`. -/
--   beq : α → α → Bool

-- What is happening in the background when we use the `a == b`
-- notation is that Lean is searching for an *instance* of the
-- `BEq` typeclasses that applies for `Nat`. An example of such
-- an instance would be

instance (priority := low) : BEq Nat where
  beq := Nat.beq

-- where we are specifying that the usage of `==` for natural
-- numbers should correspond to the expected `Nat.beq`. So in
-- order to write a function that uses the notation for boolean
-- equality over a generic type, we need a way to express that
-- there is some instance for `BEq α`. This is done using
-- **instance implicits**, where we place a desired typeclass
-- assumption in square brackets. Our corrected definition is
-- then
...
-- where Lean can now infer the usage of `==` is the one that
-- we have provided.
...
-- Intuitively, a total map over an element type `β` is just a
-- function that can be looked up using a corresponding
-- `a : α`.
...
-- Let's introduce notation for accessing elements using square
-- brackets. We'll call the typeclass `MyGetElem` after
-- `GetElem` from the standard library. The reason we are
-- defining a new typeclass is for simplicity -- `GetElem`
-- contains logic for when keys are valid which we don't need.

-- The typeclass takes three type parameters: the collection,
-- the keys that can be used to index and the elements that it
-- returns. For example, let's say we have declared an instance
...
-- Don't worry about the `outParam`, `macro_rules` or the
-- `app_unexpander`, they are minor technicalities.
...
-- If we update a map `m` at a key `a` with a value `b₁` and
-- then update again with the same key `a` and another value
-- `b₂`, the resulting map behaves the same (gives the same
-- result when applied to any key) as the simpler map obtained
-- by performing just the second `update` on `m`:
...
-- `none`. We use define partial maps as a one-field structure
-- encapsulating the inner total map.
...
--     Explain why we use a one-field structure (instead of a
--     `def` for example)
