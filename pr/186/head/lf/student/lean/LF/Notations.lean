...
-- This file provides an overview of the custom notations introduced across
-- the volumes.
...
-- - The notation is `local` so it's dropped at `end MyBool`. This is on purpose
--   as we start using Lean's booleans right after.
...
-- - Introduces tactics like `inversion` and `apply at` plus the `lemma`
--   command.

-- Note to developers (Niklas Halonen  @xhalo32):
--     - Comments about notations in Basics:
--
--       - We have a mixture of `local` and `scoped` notations. Neither `local` or
--         `scoped` is mentioned in `More on Notation (Optional)`.
--
--         - The scoped notations are inside `NatPlayground.Nat`.
--
--       - The boolean notations have `(priority := high)` which seem to be
--         unnecessary. The file builds without the priority setting.
--
--     - The `≤?` notation in Tactics, `=~` in Automation, and `→ₜ, →ₚ` in
--       Typeclasses should be `scoped`.
--
--     - Should `CustomTactics` be imported by default in every chapter?

-- Note to developers (Niklas Halonen  @xhalo32):
--     The `(priority := high)` in IndProp is in fact necessary which is
--     surprising since increasing the priority isn't necessary in Basics.
--     This is caused by the fact that `≤` has the same type signature in
--     IndProp so it's ambiguous whereas in Basics `+` is defined for a custom
--     `Nat` type so Lean selects the notation that is expected by the types.
--     Lean also seems to favor the scoped `+` notation over the one from the
--     prelude:
--
--     `scoped infixl:65 " + " => add
--
...
-- Note to developers (Niklas Halonen  @xhalo32):
--     It might make sense to introduce a notation type class for big step
--     evaluation, or to use scoped notation to change `⇓b` to `⇓`. The
--     notation type class could be reused in Smallstep.
--
--     Notation type class for `⟶`?
--
--     I think we can simplify typing judgments to just
--     `"<{ " "⊢ " tm " ⦂ " term:max " }>"` and remove the `| `(<{ ⊢ $t ⦂ ~$T
--     }>) => `(Tm.HasType <{ $t }> $T)` branch. The `Tm.HasType` in my
--     opinion gets drowned between all of the syntax boilerplate.
--
--     I find it a bit strange that substitution is `stlcTm` rather than a
--     term. At least I don't think of substitution as a "term" in the
--     internal language.
