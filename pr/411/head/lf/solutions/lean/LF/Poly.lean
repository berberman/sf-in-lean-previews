...
--  *higher-order functions* (treating functions as data). We begin with
--  polymorphism.
...
--  lists, etc. We *could* just define a new inductive datatype for each of
--  these, for example...
...
--  make up different constructor names for each datatype, but — even worse
--  — we would also need to define new versions of all the list
...
--  defined type of lists whose elements are of type `α`.

--  Note to developers (Yipeng Liu @berberman):
--      A trick used below: parenthesizing the declaration makes it a term
--      — Lean elaborates it and prints its inferred function type, instead
--      of the declaration signature: `MyList (α : Type) : Type`.
...
--  now polymorphic constructors. In Lean, the type parameter is *implicit*
--  by default: Lean will infer it from context. For example, `MyList.nil`
--  is the empty list, and Lean figures out the element type from how it is
--  used.
...
--  precisely, when Lean encounters a `_`, it will attempt to *unify* all
...
--  generalize the definition `NatProd` of pairs of natural numbers from
--  last chapter, we get polymorphic pairs, often called *products*:
...
--  second components of the pair. It also has special syntax for creating
--  products:
...
--  Our last polymorphic type for now is *polymorphic options*. Lean's
--  standard library provides `Option α`, with constructors `none` and
--  `some`. (We already saw `NatOption` in the Lists chapter.) Let's
--  briefly look at the definition:
...
--  We can now rewrite the `nth?` function so that it works with any type
--  of list.
...
--  Complete the definition of a polymorphic version of the `head?`
...
--  The argument `f` here is itself a function (from `α` to `α`); the body
--  of `doIt3Times` applies `f` three times to some value `x`.
...
--  Here is a more useful higher-order function, taking a list of `α`s and
--  a *predicate* on `α` (a function from `α` to `Bool`) and "filtering"
--  the list to yield a new list containing just those elements for which
...
--  For example, if we apply `filter` to the predicate `Nat.even` and a
--  list of numbers, it returns a list containing just the even members.
...
--  define the function `isLength1` and give it a name just to be able to
--  pass it as an argument to `filter`, since we will probably never use it
...
--  Use `filter` to write a Lean function `partition` that, given a type
--  `α`, a predicate of type `α → Bool` and a `List α`, should return a
--  pair of lists. The first member of the pair is the sublist of the
--  original list containing the elements that satisfy the test, and the
--  second is the sublist containing those that fail the test. The order of
...
--  the list `[f n1, f n2, f n3, ...]`, where `f` has been applied to each
--  element of `l` in turn. For example:

example : map (· + 3) [2, 0, 2] = [5, 3, 5] := by rfl

--  The element types of the input and output lists need not be the same,
--  since `map` takes *two* type arguments, `α` and `β`; it can thus be
--  applied to a list of numbers and a function from numbers to booleans to
...
--  The function `map` maps a `List α` to a `List β` using a function of
...
--  Lists are not the only inductive type for which `map` makes sense. Here
--  is a `map` for the `Option` type:
...
--  in many places. Replace the curly braces around the implicit arguments
--  with explicit parentheses, and then fill in explicit type parameters
--  where necessary and use Lean to check that you've done so correctly.
--  (This exercise is not to be turned in; it is probably easiest to do it
--  on a *copy* of this file that you can throw away afterwards.)

--  ### Fold

--  An even more powerful higher-order function is `fold`. It is the
--  inspiration for the "reduce" operation that lies at the heart of
...
--  takes an `α` and a `β` and returns a `β`. The examples above show one
--  instance where it is useful for `α` and `β` to be different. Can you
--  think of any others?
...
--  *returning* functions as the results of other functions. To begin, here
--  is a function that takes a value `x` (drawn from some type `α`) and
--  returns a function from `Nat` to `α` that yields `x` whenever it is
--  called, ignoring its `Nat` argument.
...
--  more arguments: a list and a starting value.
...
--  For example, here is an alternative definition of `List.length`:
...
--      Even though `foldMap` is not autograded, we mark it as a hole just
--      in case in the future something depended on it.
...
--  arguments, one of type `α` and another of type `β`, and return an
--  output of type `γ`. Recall from our discussion of partial application
--  that this type is written `α → (β → γ)` when fully parenthesized. That
--  is, if we have `f : α → β → γ`, and we give `f` an input of type `α`,
--  it will give us as output a function of type `β → γ`. If we then give
--  that function an input of type `β`, it will return an output of type
...
--  As an exercise, define its inverse, `prodUncurry`. Then prove the
--  theorems below to show that the two are really inverses.
...
--  Thought exercise: before looking at the output of the following
--  commands, can you calculate the types of `prodCurry` and `prodUncurry`?
...
--    hypothesis tells us that `l'.length = n' → nth? l' n' = none`, for
--    any `n'`.
--
...
--    So it's enough to show `nth? l' l'.length = none`, which follows
--    directly from the induction hypothesis, picking `l'.length` for `n'`.

--  ### Church Numerals (Advanced)

--  The following exercises explore an alternative way of defining natural
--  numbers using the *Church numerals*, which are named after their
...
--  times"? The answer is actually simple: just return the argument
--  untouched.
...
--  informally notate that as `fun α f x => f^n x`, with the convention
--  that `f^0 x` is just `x`. Note how the `doIt3Times` function we've
...
--  If we passed in `Nat.succ` as `succ` and `0` as `zero`, we'd even get
--  the Peano naturals as a result:
...
--  `fun X f x => f^(n+1) x` as output. In other words, do it `n` times,
...
--  means right now, but know that this is Lean's way of preventing a
--  paradox in which a type contains itself. So leave the type argument
--  unchanged.
