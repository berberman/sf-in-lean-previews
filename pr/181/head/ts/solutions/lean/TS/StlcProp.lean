...
--     The document-level `set_option maxHeartbeats 1000000` above is needed
--     by `substitution_preserves_typing`, and it has to be
--     **document-level**: the cost is not in the proof itself (the same proof
--     elaborates well inside budget as plain Lean) but in Verso's InlineLean
--     highlighting pass, which re-drives elaboration to attach type and hover
--     information. That pass honors only document-level options — a
--     `set_option maxHeartbeats ... in` on the theorem was verified not to
--     help here, and neither does splitting the code block, since the budget
--     is per-declaration. `LF/IndPropRegexpVerso.lean` carries the same
--     override and documents the mechanism at length.
--
--     The way to remove this would be to break the substitution lemma's
--     variable and abstraction cases out as named helper lemmas, so each is
--     highlighted against its own fresh budget — the pattern `IndPropRegexp`
--     uses for `star_app_aux`. That is a pedagogy call (it changes how the
--     central proof of the chapter reads), so it is left for review rather
--     than done here.

-- Note to developers (Benjamin Pierce  @bcpierce00, before next release, 2022):
--     In Wadler's "PLF in Agda", he defines an "animator" for STLC terms
--     using the proof terms for progress + preservation. This would be a
--     FANTASTIC example (or, perhaps better, exercise!) for this chapter.
...
-- We pick up where the Stlc chapter left off, so everything below lives in
-- the same namespace as the definitions it is about.

namespace Stlc

-- ## Canonical Forms

-- As we saw for the very simple language in the Types chapter, the first step
-- in establishing basic properties of reduction and types is to identify the
-- possible *canonical forms* (i.e., well-typed values) belonging to each
-- type. For `Bool`, these are again the boolean values `true` and `false`;
-- for arrow types, they are lambda-abstractions.

-- Formally, we will need these lemmas only for terms that are not only well
-- typed but *closed* — i.e., well typed in the empty context.
...
-- step. The proof is a relatively straightforward extension of the progress
-- proof we saw in the Types chapter. We give the proof in English first, then
-- the formal version.
...
-- - The last rule of the derivation cannot be `HasType.var`, since a variable
--   is never well typed in an empty context.

-- - The `HasType.tru`, `HasType.fls`, and `HasType.abs` cases are trivial,
--   since in each of these cases we can see by inspecting the rule that `t` is
--   a value.

-- - If the last rule of the derivation is `HasType.app`, then `t` has the form
--   `t₁ t₂` for some `t₁` and `t₂`, where `⊢ t₁ ⦂ T₂ → T` and `⊢ t₂ ⦂ T₂` for
--   some type `T₂`. The induction hypothesis for the first subderivation says
--   that either `t₁` is a value or else it can take a reduction step.

--   - If `t₁` is a value, then consider `t₂`, which by the induction hypothesis
--     for the second subderivation must also either be a value or take a step.

--     - Suppose `t₂` is a value. Since `t₁` is a value with an arrow type, it must
--       be a lambda abstraction; hence `t₁ t₂` can take a step by `Step.appAbs`.
...
--   `t = if t₁ then t₂ else t₃`, where `t₁` has type `Bool`. The first IH says
--   that `t₁` either is a value or takes a step.

--   - If `t₁` is a value, then since it has type `Bool` it must be either `true`
--     or `false`. If it is `true`, then `t` steps to `t₂`; otherwise it steps to
--     `t₃`.
...
-- The other half of the type soundness property is the preservation of types
-- during reduction. For this part, we'll need to develop some technical
-- machinery for reasoning about variables and substitution. Working from top
-- to bottom (from the high-level property we are actually interested in to
-- the lowest-level technical lemmas that are needed by various cases of the
-- more interesting proofs), the story goes like this:

-- - The *preservation theorem* is proved by induction on a typing derivation
--   and case analysis on the step relation, pretty much as we did in the Types
--   chapter. The one case that is significantly different is the one for the
--   `Step.appAbs` rule, whose definition uses the substitution operation. To
--   see that this step preserves typing, we need to know that the substitution
--   itself does. So we prove a...

-- - *substitution lemma*, stating that substituting a (closed, well-typed) term
--   `s` for a variable `x` in a term `t` preserves the type of `t`. The proof
--   goes by induction on the form of `t` and requires looking at all the
--   different cases in the definition of substitition. This time, for the
--   variables case, we discover that we need to deduce from the fact that a
--   term `s` has type S in the empty context the fact that `s` has type S in
--   every context. For this we prove a...

-- - *weakening* lemma, showing that typing is preserved under "extensions" to
--   the context `Γ`.

-- To make Lean happy, though, we need to formalize the story in the opposite
-- order, starting with weakening...

-- ### The Weakening Lemma

-- First, we show that typing is preserved under "extensions" to the context
-- `Γ`. (Recall map inclusion, `Γ ⊆ Γ'`, from the `Typeclasses` chapter.)
...
-- Now we come to the conceptual heart of the proof that reduction preserves
-- types — namely, the observation that *substitution* preserves types.

-- Formally, the so-called *substitution lemma* says this: Suppose we have a
-- term `t` with a free variable `x`, and suppose we've assigned a type `T` to
-- `t` under the assumption that `x` has some type `U`. Also, suppose that we
-- have some other term `v` and that we've shown that `v` has type `U`. Then,
-- since `v` satisfies the assumption we made about `x` when typing `t`, we
-- can substitute `v` for each of the occurrences of `x` in `t` and obtain a
-- new term that still has type `T`.
...
-- The substitution lemma can be viewed as a kind of "commutation property."
-- Intuitively, it says that substitution and typing can be done in either
-- order: we can either assign types to the terms `t` and `v` separately
-- (under suitable contexts) and then combine them using substitution, or we
-- can substitute first and then assign a type to ` `x:=v` t`; the result is
-- the same either way.

-- *Proof*: We show, by induction on `t`, that for all `T` and `Γ`, if
-- `x ↦ U; Γ ⊢ t ⦂ T` and `⊢ v ⦂ U`, then `Γ ⊢ [x:=v]t ⦂ T`.

-- - If `t` is a variable there are two cases to consider, depending on whether
--   `t` is `x` or some other variable.

--   - If `t = x`, then from the fact that `x ↦ U; Γ ⊢ x ⦂ T` we conclude that
--     `U = T`. We must show that `[x:=v]x = v` has type `T` under `Γ`, given the
--     assumption that `v` has type `U = T` under the empty context. This follows
--     from the weakening lemma.

--   - If `t` is some variable `y` that is not equal to `x`, then we need only
--     note that `y` has the same type under `x ↦ U; Γ` as under `Γ`.

-- - If `t` is an abstraction `λy:S. t₀`, then `T = S→T₁` and the IH tells us,
--   for all `Γ'` and `T₀`, that if `x ↦ U; Γ' ⊢ t₀ ⦂ T₀`, then
--   `Γ' ⊢ [x:=v]t₀ ⦂ T₀`. Moreover, by inspecting the typing rules we see it
--   must be the case that `y ↦ S; x ↦ U; Γ ⊢ t₀ ⦂ T₁`.

--   The substitution in the conclusion behaves differently depending on whether
--   `x` and `y` are the same variable.

--   First, suppose `x = y`. Then, by the definition of substitution,
--   `[x:=v]t = t`, so we just need to show `Γ ⊢ t ⦂ T`. Using `HasType.abs`, we
--   need to show that `y ↦ S; Γ ⊢ t₀ ⦂ T₁`. But we know
--   `y ↦ S; x ↦ U; Γ ⊢ t₀ ⦂ T₁`, and the claim follows since `x = y`.

--   Second, suppose `x <> y`. Again, using `HasType.abs`, we need to show that
--   `y ↦ S; Γ ⊢ [x:=v]t₀ ⦂ T₁`. Since `x <> y`, we have
--   `y ↦ S; x ↦ U; Γ = x ↦ U; y ↦ S; Γ`. So we have
--   `x ↦ U; y ↦ S; Γ ⊢ t₀ ⦂ T₁`. Then, the the IH applies (taking
--   `Γ' = y ↦ S; Γ`), giving us `y ↦ S; Γ ⊢ [x:=v]t₀ ⦂ T₁`, as required.

-- - If `t` is an application `t₁ t₂`, the result follows straightforwardly from
...
-- assume `v` has type `U` in the *empty* context — in other words, we assume
-- `v` is closed. (Since we are using a simple definition of substition that
-- is not capture-avoiding, it doesn't make sense to substitute non-closed
-- terms into other terms. Fortunately, closed terms are all we need!)

-- ### Exercise (3 stars): substitution_preserves_typing_from_typing_ind (Advanced) ⭐⭐⭐

-- Show that substitution*preserves*typing can also be proved by induction on
-- typing derivations instead of induction on terms.
...
-- also a closed term with type `T`. In other words, the small-step reduction
...
-- - We can immediately rule out `HasType.var`, `HasType.abs`, `HasType.tru`,
--   and `HasType.fls` as final rules in the derivation, since in each of these
--   cases `t` cannot take a step.

-- - If the last rule in the derivation is `HasType.app`, then `t = t₁ t₂`, and
--   there are subderivations showing that `⊢ t₁ ⦂ T₂→T` and `⊢ t₂ ⦂ T₂` plus
--   two induction hypotheses: (1) `t₁ ⟶ t₁'` implies `⊢ t₁' ⦂ T₂→T` and (2)
--   `t₂ ⟶ t₂'` implies `⊢ t₂' ⦂ T₂`. There are now three subcases to consider,
--   one for each rule that could be used to show that `t₁ t₂` takes a step to
--   `t'`.

--   - If `t₁ t₂` takes a step by `Step.app1`, with `t₁` stepping to `t₁'`, then,
--     by the first IH, `t₁'` has the same type as `t₁` (`⊢ t₁' ⦂ T₂→T`), and
--     hence by `HasType.app` `t₁' t₂` has type `T`.

--   - The `Step.app2` case is similar, using the second IH.

--   - If `t₁ t₂` takes a step by `Step.appAbs`, then `t₁ = λx:T₀. t₀` and `t₁ t₂`
--     steps to `[x0:=t₂]t₀`; the desired result now follows from the substitution
--     lemma.

-- - If the last rule in the derivation is `HasType.ite`, then
--   `t = if t₁ then t₂ else t₃`, with `⊢ t₁ ⦂ Bool`, `⊢ t₂ ⦂ T₁`, and
--   `⊢ t₃ ⦂ T₁`, and with three induction hypotheses: (1) `t₁ ⟶ t₁'` implies
--   `⊢ t₁' ⦂ Bool`, (2) `t₂ ⟶ t₂'` implies `⊢ t₂' ⦂ T₁`, and (3) `t₃ ⟶ t₃'`
--   implies `⊢ t₃' ⦂ T₁`.

--   There are again three subcases to consider, depending on how `t` steps.

--   - If `t` steps to `t₂` or `t₃` by `Step.ifTrue` or `Step.ifFalse`, the result
--     is immediate, since `t₂` and `t₃` have the same type as `t`.

--   - Otherwise, `t` steps by `Step.ifStep`, and the desired conclusion follows
--     directly from the first induction hypothesis.
...
-- This property did not hold for that language, and it also fails for STLC.
-- That is, it is not always the case that, if `t ⟶ t'` and `empty ⊢ t' ⦂ T`,
-- then `empty ⊢ t ⦂ T`. Show this by giving a counter-example that does *not
-- involve conditionals*.

-- COMMENT:
-- `untagged (* .. *) comment at prose position in the Rocq source — probably an error there; decide whether it is book prose or an author note`
-- For example, `((\a:Bool→Bool, λy:Bool. y) true)` is ill typed, but it
-- evaluates to the well-typed term `λy:Bool. y`,
...
-- Put progress and preservation together and show that a well-typed term can
...
-- Another nice property of the STLC is that types are unique: a given term
-- (in a given context) has at most one type.
...
-- *context invariance*. It states that typing is preserved under "inessential
-- changes" to the context `Γ` — in particular, changes that do not affect any
-- of the free variables of the term. In this section, we establish this
-- property for our system, introducing some other standard terminology on the
-- way.

-- First, we need to define the *free variables* in a term — i.e., variables
-- that are used in the term in positions that are *not* in the scope of an
-- enclosing function abstraction binding a variable of the same name.

-- More technically, a variable `x` *appears free in* a term *t* if `t`
-- contains some occurrence of `x` that is not under an abstraction labeled
-- `x`. For example:
...
-- The *free variables* of a term are just the variables that appear free in
-- it. This gives us another way to define *closed* terms — arguably a better
-- one, since it applies even to ill-typed terms. Indeed, this is the standard
-- definition of the term "closed."

def Tm.Closed (t : Tm) : Prop := ∀ x, ¬ x ∈ᶠ t

-- Conversely, an *open* term is one that may contain free variables. (I.e.,
-- every term is an open term; the closed terms are a subset of the open ones.
-- "Open" precisely means "possibly containing free variables.")

-- ### Exercise (1 star): afi ⭐

-- (Officially optional, but strongly recommended!) In the space below, write
-- out the rules of the `∈ᶠ` relation in informal inference-rule notation.
-- (Use whatever notational conventions you like — the point of the exercise
-- is just for you to think a bit about the meaning of each rule.) Although
-- this is a rather low-level, technical definition, understanding it is
-- crucial to understanding substitution and its properties, which are really
-- the crux of the lambda-calculus.

-- COMMENT:
-- `untagged (* .. *) comment at prose position in the Rocq source — probably an error there; decide whether it is book prose or an author note`
-- (no solution yet) -/ /- LATER: Fill in an official solution

-- Next, we show that if a variable `x` appears free in a term `t`, and if we
-- know `t` is well typed in context `Γ`, then it must be the case that `Γ`
-- assigns a type to `x`.
...
-- *Proof*: We show, by induction on the proof that `x` appears free in `t`,
-- that, for all contexts `Γ`, if `t` is well typed under `Γ`, then `Γ`
-- assigns some type to `x`.

-- - If the last rule used is `AppearsFreeIn.var`, then `t = x`, and from the
--   assumption that `t` is well typed under `Γ` we have immediately that `Γ`
--   assigns a type to `x`.

-- - If the last rule used is `AppearsFreeIn.app1`, then `t = t₁ t₂` and `x`
--   appears free in `t₁`. Since `t` is well typed under `Γ`, we can see from
--   the typing rules that `t₁` must also be, and the IH then tells us that `Γ`
--   assigns `x` a type.

-- - Almost all the other cases are similar: `x` appears free in a subterm of
--   `t`, and since `t` is well typed under `Γ`, we know the subterm of `t` in
--   which `x` appears is well typed under `Γ` as well, and the IH gives us
--   exactly the conclusion we want.

-- - The only remaining case is `AppearsFreeIn.abs`. In this case
--   `t = λy:T₁. t₁` and `x` appears free in `t₁`, and we also know that `x` is
--   different from `y`. The difference from the previous cases is that, whereas
--   `t` is well typed under `Γ`, its body `t₁` is well typed under `y ↦ T₁; Γ`,
--   so the IH allows us to conclude that `x` is assigned some type by the
--   extended context `y ↦ T₁; Γ`. To conclude that `Γ` assigns a type to `x`,
--   we appeal to lemma `PartialMap.update_neq`, noting that `x` and `y` are
--   different variables.
...
-- From the `free_in_context` lemma, it immediately follows that any term `t`
-- that is well typed in the empty context is closed (it has no free
-- variables).
...
-- Finally, we establish *context invariance*. It is useful in cases when we
-- have a proof of some typing relation `Γ ⊢ t ⦂ T`, and we need to replace
-- `Γ` by a different context `Γ'`. When is it safe to do this? Intuitively,
-- it must at least be the case that `Γ'` assigns the same types as `Γ` to all
-- the variables that appear free in `t`. In fact, this is the only condition
-- that is needed.
...
--   `Γ x = T`. By assumption, `Γ' x = T` as well, and hence `Γ' ⊢ t ⦂ T` by
--   `HasType.var`.

-- - If the last rule was `HasType.abs`, then `t = λy:T₂. t₁`, with
--   `T = T₂ → T₁` and `y ↦ T₂; Γ ⊢ t₁ ⦂ T₁`. The induction hypothesis states
--   that for any context `Γ''`, if `y ↦ T₂; Γ` and `Γ''` assign the same types
--   to all the free variables in `t₁`, then `t₁` has type `T₁` under `Γ''`. Let
--   `Γ'` be a context which agrees with `Γ` on the free variables in `t`; we
--   must show `Γ' ⊢ λy:T₂. t₁ ⦂ T₂ → T₁`.

--   By `HasType.abs`, it suffices to show that `y ↦ T₂; Γ' ⊢ t₁ ⦂ T₁`. By the
--   IH (setting `Γ'' = y ↦ T₂;Γ'`), it suffices to show that `y ↦ T₂;Γ` and
--   `y ↦ T₂;Γ'` agree on all the variables that appear free in `t₁`.

--   Any variable occurring free in `t₁` must be either `y` or some other
--   variable. `y ↦ T₂; Γ` and `y ↦ T₂; Γ'` clearly agree on `y`. Otherwise,
--   note that any variable other than `y` that occurs free in `t₁` also occurs
--   free in `t = λy:T₂. t₁`, and by assumption `Γ` and `Γ'` agree on all such
--   variables; hence so do `y ↦ T₂; Γ` and `y ↦ T₂; Γ'`.

-- - If the last rule was `HasType.app`, then `t = t₁ t₂`, with
--   `Γ ⊢ t₁ ⦂ T₂ → T` and `Γ ⊢ t₂ ⦂ T₂`. One induction hypothesis states that
--   for all contexts `Γ'`, if `Γ'` agrees with `Γ` on the free variables in
--   `t₁`, then `t₁` has type `T₂ → T` under `Γ'`; there is a similar IH for
--   `t₂`. We must show that `t₁ t₂` also has type `T` under `Γ'`, given the
--   assumption that `Γ'` agrees with `Γ` on all the free variables in `t₁ t₂`.
--   By `HasType.app`, it suffices to show that `t₁` and `t₂` each have the same
--   type under `Γ'` as under `Γ`. But all free variables in `t₁` are also free
--   in `t₁ t₂`, and similarly for `t₂`; hence the desired result follows from
...
-- The context invariance lemma can actually be used in place of the weakening
-- lemma to prove the crucial substitution lemma stated earlier.

-- ## Additional Exercises

-- ### Exercise (1 star): progress_preservation_statement ⭐

-- (Officially optional, but strongly recommended!) Without peeking at their
-- statements above, write down the progress and preservation theorems for the
-- simply typed lambda-calculus (as Lean theorems). You can write `sorry` for
-- the proofs.

-- Note to developers (before next release):
--     At least one person was confused by what to name these. We could
--     simplify life by giving the names explicitly and just omitting the
--     bodies. Indeed, once we do that we could autograde this by demanding
--     that what they write be identical to what we wrote above! Maybe a
--     better way to solve this would be to have the following template. BCP
--     21: Yes, do this!!
...
-- Which of the following properties of the STLC remain true in the presence
-- of these rules? For each property, write either "remains true" or "becomes
-- false." If a property becomes false, give a counterexample.

-- - Determinism of `step`

-- - Becomes false. For instance `(if true then false else true) ⟶ false` and
--   `(if true then false else true) ⟶ zap`.
...
-- Suppose instead that we add a new term `foo` with the following reduction
-- rules:
...
-- Which of the following properties of the STLC remain true in the presence
-- of this rule? For each one, write either "remains true" or else "becomes
-- false." If a property becomes false, give a counterexample.
...
-- - Remains true. We are only adding to the step relation, and this can never
--   damage progress.
...
-- relation. Which of the following properties of the STLC remain true in the
...
-- - Becomes false. For example,
--   `((\x:Bool→Bool, \y:Bool→Bool, x) (λz:Bool. z)) (λz:Bool. z)` is well
--   typed, but stuck.
...
-- - Becomes false, for instance: `(if true then false else false) ⟶ false` and
...
-- - Remains true. We are only adding to the typing relation, and this can never
--   damage determinism of `step`.
...
-- - Becomes false. For instance, `true true` is a term that becomes typable (at
--   type `Bool`), but which is stuck.

-- - Preservation

-- - Remains true. There are 3 ways `t₁ t₂` can reduce. For `Step.app1` and
--   `Step.app2` we can still apply the induction hypothesis. To reduce `t₁ t₂`
--   using `Step.appAbs` `t₁` would need to be a function, but functions don't
--   have type `Bool`.
...
-- predecessor, and zero-test operations of the Types chapter. What is new is
-- the setting: those operations now live in a language that also has
-- variables, abstraction, and application, so an arithmetic computation can
-- be packaged up as a function and passed around as a value.

namespace StlcArith

-- To types, we add a base type of natural numbers (and remove booleans, for
-- brevity).
...
-- `StlcArith` is a **different** language from the STLC of this chapter, not
-- an extension of it, so it needs its own concrete syntax. Rather than invent
-- a new one, we reuse the grammars set up in the Stlc chapter — the syntax
-- categories `stlcTy`, `stlcTm`, and `stlcVar` — and give them a new meaning
-- here. Terms and types of this language are therefore written inside the
-- same `<{ … }>` brackets, with the same `~e` escape back to Lean.
...
-- unchanged. Only the `macro_rules` are new, and they differ from the STLC's
-- in just two places — the identifier `Nat` names this language's base type,
-- and the arrow builds this language's `StlcArith.Ty.arrow`.
...
-- Multiplication binds looser than application and tighter than `λ`, so
-- `x * y z` multiplies `x` by the application `y z`; it associates to the
-- right, so `x * y * z` is `x * (y * z)`.

-- `succ` and `pred` get no production of their own. Making them keywords
-- would reserve those words globally — and we would then be unable to write
-- `succ` as a case name in a proof, including for Lean's own `Nat`. Instead
-- they are written as though they were functions applied to an argument,
-- `succ t`, and the application rule below recognizes them. `if0` **is** a
-- keyword, since `then` and `else` leave no other option; that is why the
-- constructor above is called `StlcArith.Tm.ite0` rather than `if0`, just as
-- the STLC's conditional is `Stlc.Tm.ite`.
...
-- As in the Stlc chapter, a delaborator runs the grammar backwards, so that
-- goals mentioning these terms and types read in the concrete syntax. The
-- parenthesizers registered there are for the whole syntax category, so they
...
--     The Rocq source builds this grammar with custom entries, and a few
--     pieces of that have no Lean counterpart: the `<{{ x }}>` quotation for
--     shifting into type-parsing mode (Lean works out from context which of
--     the two brackets is meant), the `$( t )` escape into arbitrary Rocq
--     `constr` notation (our `~e` already does that, in both grammars), and
--     the two coercions `tm_var : string >-> tm` and `tm_const : nat >-> tm`,
--     which let a bare string or numeral stand for a term. Here a bare
--     identifier and a bare numeral are productions of the term grammar
--     itself, so no coercions are needed.

-- In this extended exercise, your job is to finish formalizing the definition
-- and properties of the STLC extended with arithmetic. Specifically:
...
-- theorems we provide. You will need to define and prove helper lemmas, as
-- before.
...
-- Substitution is defined exactly as it was for the STLC, with one clause per
-- new constructor.

-- _Details:_ Why the definition is wrapped in a section

-- Substitution is written using its own `[x := s] t` notation, which is being
-- defined at the same time, so — as in the Stlc chapter — the rule is first
-- declared `local`, with hygiene off so that the `subst` in its expansion
-- refers to the function being defined, and then declared again for real once
-- the section closes.
...
-- saying how your `subst` behaves on that constructor, in the style of the
-- Stlc chapter — the substitution lemma below is proved by rewriting with
-- them rather than by unfolding the definition. Two of the constructors need
-- two lemmas apiece, since substitution treats a bound name differently
...
-- Note to developers (before next release):
--     The reduction example above ought to be joined by a bigger one —
--     something to replace the factorial example that used to live here.
...
-- The context grammar `stlcCtx` is reused as well; only the map it denotes is
-- new, since the types it stores are this language's. As with `subst`, the
-- judgment rule is introduced twice: `local` and hygiene-free while the
...
-- STLC's. The remaining four are the typing rules for arithmetic expressions.
...
-- declared again, hygienically, for every later use, and a pair of
...
-- The two helper lemmas that weakening is for are also proved just as they
-- were for the STLC.
...
-- *Hint*: you will need to define and prove the same helper lemmas we used
-- before.
...
--     `STLCArith.subst` and 6 each for `weakening`, `preservation`, and
--     `progress`. The three theorems carry those weights directly; the 10
--     points for the definitions are split between the two examples that
--     exercise them, `Nat_step_example` and `Nat_typing_example`, since a
--     definition has no autogradable statement of its own. Names are
--     qualified (`StlcArith.progress`) so the grader can tell them apart from
--     this chapter's own `progress` and `preservation`.
