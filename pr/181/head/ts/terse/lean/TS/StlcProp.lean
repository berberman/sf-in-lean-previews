...
--     The document-level `set_option maxHeartbeats 1000000`
--     above is needed by `substitution_preserves_typing`, and
--     it has to be **document-level**: the cost is not in the
--     proof itself (the same proof elaborates well inside
--     budget as plain Lean) but in Verso's InlineLean
--     highlighting pass, which re-drives elaboration to attach
--     type and hover information. That pass honors only
--     document-level options — a
--     `set_option maxHeartbeats ... in` on the theorem was
--     verified not to help here, and neither does splitting
--     the code block, since the budget is per-declaration.
--     `LF/IndPropRegexpVerso.lean` carries the same override
--     and documents the mechanism at length.
--
--     The way to remove this would be to break the
--     substitution lemma's variable and abstraction cases out
--     as named helper lemmas, so each is highlighted against
--     its own fresh budget — the pattern `IndPropRegexp` uses
--     for `star_app_aux`. That is a pedagogy call (it changes
--     how the central proof of the chapter reads), so it is
--     left for review rather than done here.
...
--     In Wadler's "PLF in Agda", he defines an "animator" for
--     STLC terms using the proof terms for progress +
--     preservation. This would be a FANTASTIC example (or,
--     perhaps better, exercise!) for this chapter.

-- In this chapter, we develop the fundamental theory of the
-- Simply Typed Lambda Calculus — in particular, the type
-- safety theorem.

-- We pick up where the Stlc chapter left off, so everything
-- below lives in the same namespace as the definitions it is
-- about.
...
-- Formally, we will need these lemmas only for terms that are
-- not only well typed but *closed* — i.e., well typed in the
-- empty context.
...
--   typing derivation and case analysis on the step relation,
...
-- In this case, we need to deduce from the fact that a term
-- `s` has type S in the empty context the fact that `s` has
-- type S in every context.
...
-- First, we show that typing is preserved under "extensions"
-- to the context `Γ`. (Recall map inclusion, `Γ ⊆ Γ'`, from
...
--   suppose we've been able to assign a type `T` to `t` under
--   the assumption that `x` has some type `U`.

-- - Also, suppose that we have some other term `v` and that
--   we've shown that `v` has type `U`.
...
-- We now have the ingredients we need to prove preservation:
-- if a closed, well-typed term `t` has type `T` and takes a
-- step to `t'`, then `t'` is also a closed term with type `T`.
-- In other words, the small-step reduction relation preserves
-- types.
...
-- The type grammar needs no new productions: `Nat` is a bare
-- identifier, which the template already accepts, and arrows
-- and parentheses are unchanged. Only the `macro_rules` are
-- new, and they differ from the STLC's in just two places —
-- the identifier `Nat` names this language's base type, and
-- the arrow builds this language's `StlcArith.Ty.arrow`.
...
-- Terms do need new productions: a numeral, an infix `*`, and
-- the zero test. Multiplication binds looser than application
-- and tighter than `λ`, so `x * y z` multiplies `x` by the
-- application `y z`; it associates to the right, so
-- `x * y * z` is `x * (y * z)`.

-- `succ` and `pred` get no production of their own. Making
-- them keywords would reserve those words globally — and we
-- would then be unable to write `succ` as a case name in a
-- proof, including for Lean's own `Nat`. Instead they are
-- written as though they were functions applied to an
-- argument, `succ t`, and the application rule below
-- recognizes them. `if0` **is** a keyword, since `then` and
-- `else` leave no other option; that is why the constructor
-- above is called `StlcArith.Tm.ite0` rather than `if0`, just
-- as the STLC's conditional is `Stlc.Tm.ite`.
...
-- backwards, so that goals mentioning these terms and types
-- read in the concrete syntax. The parenthesizers registered
-- there are for the whole syntax category, so they serve this
-- language too and are not repeated.
...
--     The Rocq source builds this grammar with custom entries,
--     and a few pieces of that have no Lean counterpart: the
--     `<{{ x }}>` quotation for shifting into type-parsing
--     mode (Lean works out from context which of the two
--     brackets is meant), the `$( t )` escape into arbitrary
--     Rocq `constr` notation (our `~e` already does that, in
--     both grammars), and the two coercions
--     `tm_var : string >-> tm` and `tm_const : nat >-> tm`,
--     which let a bare string or numeral stand for a term.
--     Here a bare identifier and a bare numeral are
--     productions of the term grammar itself, so no coercions
--     are needed.

end StlcArith

-- Note to developers (Claude):
--     The source's grading file weights this exercise at 28
--     points — 10 for `STLCArith.subst` and 6 each for
--     `weakening`, `preservation`, and `progress`. The three
--     theorems carry those weights directly; the 10 points for
--     the definitions are split between the two examples that
--     exercise them, `Nat_step_example` and
--     `Nat_typing_example`, since a definition has no
--     autogradable statement of its own. Names are qualified
--     (`StlcArith.progress`) so the grader can tell them apart
--     from this chapter's own `progress` and `preservation`.
