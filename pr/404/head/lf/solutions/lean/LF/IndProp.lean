...
--      inline prose throughout this chapter can use `n`, `n'`, `m`, `m'`,
--      `k`, `α`, `x`, `y`, `l`, `l₁`, `l₂`, and `l₃` without repeating
--      their type annotations, but the same problem described in Logic
...
--  — i.e., we can *introduce* and *eliminate* `Even` evidence, in the
--  sense of Logic.
--
...
--      None of this quiz's options matches its solution. In the
--      `succ_succ` case the hypothesis is `n + 2 = 1`, and `contradiction`
--      alone does not close it (checked); `injection` is needed first, as
--      the solution shows. So (C) is not sufficient as stated and (D) is
--      the literally correct answer, which is presumably not intended.
--      Options: add `injection` to option (C) or as a new option, or
--      change the hypothesis to one that `cases` and `contradiction`
--      refute directly.
...
--  an inversion lemma for every `inductive` definition we make. (The
--  details of how `inversion` is implemented are beyond the scope of this
--  course. [Lean provides *metaprogramming*
--  facilities](https://leanprover-community.github.io/lean4-metaprogramming-book/main/01_intro.html)
--  that its users can employ to write their own tactics, and these
--  capabilities are powerful enough that just about any algorithmic
...
--  Recall from the Logic chapter that equality (`Eq`) is itself an
--  inductively defined proposition, so `inversion` can also be used on
...
--  Here is a useful way to think about `inversion`. For an inductively
...
--  cases, it solves these equations to force some expressions or
--  substitute some variables. If an equation cannot be solved, `inversion`
--  leaves it in the context and we can use it in the rest of the proof.
...
--
--  We could try to proceed by `cases` or `induction` on `n`. But since
...
--      behavior of uninhabited types, which I don't think have been
...
--  Recall the `Le` relation from earlier in this chapter. Here are a
--  number of facts about the `≤`, `<`, and `≥` relations, and about `Le`'s
--  relationship to the boolean function `Nat.ble`, that we are going to
--  need later in the course; the proofs make good practice for the
--  case-analysis and induction techniques from the last few sections.
...
--  `k = m + n`. We can prove this invariant as follows:
...
--     evidence for `R (m + 1) (n + 1) (k + 2)` is by applying `c2` and
--     `c3` (in either order) to evidence for `R m n k`, so the latter must
--     already hold. (This can be proved by induction, although the proof
--     is surprisingly tedious.)
--
--  We can prove `c4` and `c5` are redundant by redefining `R'` with only
--  `c1`, `c2`, and `c3`, and proving `R'` is equivalent to `R`.
--
...
--  proofs by Daniel Schepler and N. Raghavendra. Unlike the proof above,
--  it never needs to decide whether an element belongs to a list, so it
