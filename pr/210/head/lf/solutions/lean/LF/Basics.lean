...
-- it is considered good practice not to access the fields of an object
-- directly, but instead to use getter and setter methods. This helps to
-- encapsulate the object's definition, so that, if its fields or
