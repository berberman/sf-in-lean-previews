...
--     save a mismatch in the `dec_minimum` exercise in Hoare₂?
--
...
-- up in the state with the map-indexing notation `st[x]` from the Typeclasses
-- chapter in the Logical Foundations book. For the notation to work, we used
...
-- to what we did for `Aexp.EvalR` in the Slang chapter.
...
-- Old HP Calculators, programming languages like Forth and Postscript, and
...
-- and evaluated like this (where we show the program being evaluated on the
-- right and the contents of the stack on the left):
...
-- as input a state, a stack represented as a list of numbers (top stack item
-- is the head of the list), and a program represented as a list of
-- instructions, and it should return the stack after executing the program.
...
-- elements. In a sense, it is immaterial what we do, since a correct compiler
-- will never emit such a malformed program. But for sake of later exercises,
...
-- Execution can be decomposed in the following sense: executing stack program
-- `p₁ ++ p₂` is the same as executing `p₁`, taking the resulting stack, and
...
-- evaluates to `false`, then the entire `and` expression evaluates to `false`
-- immediately, without evaluating `b₂`. Otherwise, `b₂` is evaluated to
-- determine the result of the `and` expression.
...
-- `BExp.eval`. (N.b. This is only true because expression evaluation in Imp
-- is rather simple. In a bigger language where evaluating an expression might
-- diverge, the short-circuiting `and` would *not* be equivalent to the
-- original, since it would make more programs terminate.)

def Bexp.evalSC (st : State) (b : Bexp) : Bool := (
  match b with
  | .bool b      =>  b
  | .eq   a₁ a₂  =>  a₁.eval st == a₂.eval st
  | .neq  a₁ a₂  =>  a₁.eval st != a₂.eval st
...
-- consider how to add `break` to Imp. First, we need to enrich the language
-- of commands with an additional case. Because `break` is a reserved keyword
-- in Lean, we will abbreviate it as `brk`.
...
-- Next, we need to define the behavior of `brk`. Informally, whenever `brk`
-- is executed in a sequence of commands, it stops the execution of that
-- sequence and signals that the innermost enclosing loop should terminate.
-- (If there aren't any enclosing loops, then the whole program simply
-- terminates.) The final state should be the same as the one in which the
-- `brk` statement was executed.

-- One important point is what to do when there are multiple loops enclosing a
-- given `brk`. In those cases, `brk` should only terminate the *innermost*
...
-- evaluation relation that specifies whether evaluation of a command executes
-- a `brk` statement:
...
-- started in state `st`, then it terminates in state `st'` and either signals
-- that the innermost surrounding loop (or the whole program) should exit
-- immediately (`s = sBreak`) or that execution should continue normally
-- (`s = sContinue`).

-- The definition of the `st =[ c ]=> st' // s` relation is very similar to
-- the one we gave above for the regular evaluation relation
-- (`st =[ c ]=> st'`) -- we just need to handle the termination signals
-- appropriately:

-- - If the command is `skip`, then the state doesn't change and execution of
...
-- - If the command is of the form `if (b) {c₁} {c₂}`, then the state is updated
--   as in the original semantics of Imp, except that we also propagate the
--   signal from the execution of whichever branch was taken.

-- - If the command is a sequence `c₁ ; c₂`, we first execute `c₁`. If this
--   yields a `sBreak`, we skip the execution of `c₂` and propagate the `sBreak`
--   signal to the surrounding context; the resulting state is the same as the
--   one obtained by executing `c₁` alone. Otherwise, we execute `c₂` on the
--   state obtained after executing `c₁`, and propagate the signal generated
--   there.

-- - Finally, for a loop of the form `while (b) {c}`, the semantics is almost
--   the same as before. The only difference is that, when `b` evaluates to
--   `true`, we execute `c` and check the signal that it raises. If that signal
--   is `sContinue`, then the execution proceeds as in the original semantics.
--   Otherwise, we stop the execution of the loop, and the resulting state is
--   the same as the one resulting from the execution of the current iteration.
--   In either case, since `break` only terminates the innermost loop, `while`
--   signals `sContinue`.

-- Based on the above description, complete the definition of the `Com.EvalR`
-- relation:
