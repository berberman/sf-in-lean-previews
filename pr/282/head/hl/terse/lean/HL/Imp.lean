...
-- After you've defined `sCompile`, prove the following to test
-- that it works.
...
-- implementation of `sExecute` or `sCompile` could be
-- simplified.
...
-- Add C-style `for` loops to the language of commands, update
-- the `ceval` definition to define the semantics of `for`
-- loops, and add cases for `for` loops as needed so that all
-- the proofs in this file are accepted by Rocq.

-- A `for` loop should be parameterized by (a) a statement
-- executed initially, (b) a test that is run on each iteration
-- of the loop to determine whether the loop should continue,
-- (c) a statement executed at the end of each loop iteration,
-- and (d) a statement that makes up the body of the loop. (You
-- don't need to worry about making up a concrete Notation for
-- `for` loops, but feel free to play with this too if you
-- like.)
