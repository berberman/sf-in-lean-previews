...
-- `[x:=s] t₁ t₂` is the application of `[x:=s] t₁` to `t₂`,
...
-- definition it names, so it is introduced in two steps: the
-- rule below is declared `local` with hygiene off, so the
-- `HasType` in its expansion resolves to the relation being
-- declared, and after the `section` closes it is declared
-- again for real use.
