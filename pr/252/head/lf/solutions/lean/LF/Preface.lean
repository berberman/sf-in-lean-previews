...
-- Lean, and almost all of the exercises are designed to be worked using Lean.

-- This book, *Logical Foundations in Lean*, lays groundwork for the others,
-- introducing the reader to the basic ideas of functional programming, formal
-- logic, and Lean itself.
...
-- challenges with a host of techniques for improving software reliability,
...
-- model-view-controller, publish-subscribe, etc.) and whole programming
...
-- - basic tools from *logic* for making and justifying precise claims about
--   programs;

-- - the use of *provers* (or *proof assistants*) to construct rigorous logical
--   arguments;

-- - *functional programming*, both as a programming method that simplifies
...
-- course on discrete math or analysis of algorithms, but in this book we will
-- examine them more deeply than you have probably done so far.

-- ### Proof Assistants

-- The flow of ideas between logic and computer science over the years has run
-- in both directions, with CS also making contributions to logic. One of
-- these has been the development of software tools for helping construct and
-- validate proofs of logical statements. These tools fall into two broad
-- categories:
...
--   limited, they have matured tremendously in recent decades and are used now
--   in a multitude of settings. Examples of such tools include SAT solvers, SMT
--   solvers, and model checkers.

-- - *Proof assistants* — or just *provers* — are hybrid tools that automate the
--   more routine aspects of creating proofs while depending on human guidance
--   for more difficult aspects. Widely used proof assistants include Isabelle,
--   Agda, Twelf, ACL2, PVS, F*, HOL4, Rocq, and Lean, among many others.

-- This course is based around Lean, a proof assistant that has been under
-- development since 2013 and has attracted a large and active community of
-- users in both research and at companies like DeepMind, OpenAI, Anthropic,
-- MSR, and AWS.

-- Lean provides a rich environment for interactive development of
-- machine-checked formal reasoning. The kernel of the Lean system is a simple
-- proof-checker, which guarantees that only correct deduction steps are ever
-- performed. On top of this kernel, the Lean environment provides high-level
-- facilities for proof development, including a large library of common
-- definitions and lemmas, powerful tactics for constructing complex proofs
...
-- - As a *platform for modeling programming languages*, they have become
--   standard tools for researchers who need to describe and reason about
...
--   common criteria certification, and for formal specifications of the x86 and
...
-- roots go back to Church's lambda-calculus from the 1930s, well *before* the
-- first electronic computers! But since the early '90s it has enjoyed a surge
-- of interest among both software engineers and language designers.

-- The most basic tenet of functional programming is that, whenever possible,
-- computation should be *pure*, in the sense that the only effect of
-- execution should be to produce a result: it should be free from *side
...
-- a pure sorting function would take the original list and return a fresh
...
-- critical in concurrent systems, where every piece of mutable state shared
-- between threads is a potential source of pernicious bugs.

-- Another reason for the popularity of functional programming is related to
-- the first: functional programs are often much easier to parallelize and
-- physically distribute than their imperative counterparts. If running a
-- computation has no effect other than producing a result, then it does not
-- matter *where* it is run. Similarly, if a data structure is never modified
-- destructively, it can be copied freely, across cores or across the network.
-- Indeed, the "Map-Reduce" idiom, which lies at the heart of massively
-- distributed query processors like Hadoop and is used by Google to index the
...
-- science. Indeed, Lean itself can be viewed as a combination of a small but
-- extremely expressive functional programming language and a set of tools for
-- stating and proving logical assertions. Moreover, when we come to look more
...
--     These steps send the reader to the **sources** repo, but the files a
--     reader is meant to work in are the ones the build generates:
--     `make lf-student` writes a standalone Lake project (its own
--     `lakefile.toml` and `lean-toolchain`) to `_out/lf/student/lean`, and
--     that is where the exercises with `sorry`s live. The `LF/*.lean` files
--     in the repo are Verso documents — prose plus code blocks — and building
--     the repo builds Verso and all three volumes, which is a good deal more
--     than "a few minutes".
--
--     There is also a chicken-and-egg problem in the ordering: running `make`
--     at all needs `elan`/`lake` already installed, but the step above gets
--     Lean via the VS Code extension, which only offers it once a Lean
--     project is open.
--
--     Both go away once there is a released archive of the student `.lean`
