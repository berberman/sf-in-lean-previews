...
-- implications (`a → b`), and quantified propositions (`∀ x, a`). In this
...
-- Note to developers (Jonathan Chan  @ionathanch):
--     Actually it quantifies over `Sort`, where `Prop = Sort 0` and
--     `Type u = Sort (u + 1)`. Not something that needs teaching right at
--     this moment, but they'll see `Sort` when hovering.
...
-- disjunction `a ∨ b`, we must separately discharge two proof obligations,
-- each showing that the conclusion holds under a different assumption - `a`
...
-- *Proof*: Suppose some proposition `a` holds. We must show `¬ ¬ a` - i.e.,
-- `¬ a → False`, so suppose `¬ a` as well and try to derive `False`. Then we
-- have both `a` and `¬ a` (i.e., `a → False`) from which we can indeed derive
-- `False`. So `¬ ¬ a` holds.
...
-- *Proof*: Suppose, for some `a`, that `a ∧ ¬ a` holds. Recall that `¬ a` is
-- defined as `a → False`. Given `a` and `a → False`, we can prove `False`, so
-- `(a ∧ ¬ a) → False`, i.e. `¬ (a ∧ ¬ a)`.
...
-- directions. `a ↔ b` is notation for `Iff a b`.

-- In Lean, `Iff` is a structure packaging two fields and a constructor, which
-- allow you to access its component implications. Given an `Iff` hypothesis,
-- you can access the "forward direction" implication via the `Iff.mp` (short
-- for *modus ponens*, the Latin name for reasoning by implication) field, and
-- the "reverse direction" via the `Iff.mpr` (*modus ponens reverse*) field.

-- If your goal is an `Iff`, you can convert it into two goals, one for each
-- direction of the implication, via the `Iff.intro` constructor. Or you can
-- just use the `constructor` tactic.
...
-- say that there is some `x` of type `α` such that some property `a` holds of
-- `x`, we write `∃ x : α, a`. This is notation for the `Exists` connective,
-- and is defined as `Exists (fun (x : α) => a)`. As with `∀ x : α`, the type
-- annotation `: α` can be omitted if Lean is able to infer from the context
-- what the type of `x` should be.

-- To prove a statement of the form `∃ x, a`, we must show that `a` holds for
-- some specific choice for `x`, known as the *witness* of the existential.
-- This is done in two steps: First, we explicitly tell Lean which witness `y`
-- we have in mind by invoking the tactic `exists y`. Then we prove that `a`
-- holds after all occurrences of `x` are replaced by `y`. The `exists` tactic
-- tries to close the proof with simple tactics such as `rfl` or
-- `contradiction`, so we may not have to prove `a` explicitly.
...
def All {α : Type} (a : α → Prop) (xs : List α) : Prop := (
  match xs with
  | [] => True
  | x :: xs' => a x ∧ All a xs')

theorem All_nil {α} (a : α → Prop) : All a [] = True := (rfl)

theorem All_cons {α} (a : α → Prop) x xs : All a (x :: xs) = (a x ∧ All a xs) := (rfl)
...
-- property `a` such that `a n` is equivalent to `Podd n` when `n` is odd and
...
-- This section gives just a taste.
...
-- `Nat.add`). Leaving off the colon and the type, Lean prints these types in
-- the infoview for us.
...
-- that we can fix them by applying `Nat.add_comm` to the arguments we want it
-- to be instantiated with, in much the same way as we apply a polymorphic
-- function to a type argument. Then the rewrite is forced to happen exactly
-- where we want it.
...
-- (closed) expression of type `Bool` can be simplified in a finite number of
-- steps to either `true` or `false` -- i.e., there is a terminating
...
-- Beyond the fact that non-computable properties are possible in general to
-- phrase as boolean computations, even many *computable* properties are
-- easier to express using `Prop` than `Bool`, since recursive function
-- definitions are subject to significant restrictions. For instance, the next
-- chapter shows how to define the property that a regular expression matches
-- a given string using `Prop`. Doing the same with `Bool` would amount to
-- writing a regular expression matching algorithm, which would be more
-- complicated, harder to understand, and harder to reason about than a simple
...
-- `101 = Nat.double n` and then somehow reasoning to a contradiction -- would
-- be rather complicated.

-- But if we convert it to a claim about the boolean `Nat.even` function, we
...
-- booleans and general propositions. Being able to cross back and forth
-- between the boolean and propositional worlds will often be convenient in
-- later chapters.
...
def beqList {α : Type} (beq : α → α → Bool) (xs1 xs2 : List α) : Bool := (
  match xs1, xs2 with
  | [], [] => true
...
theorem beqList_cons_cons {α} (beq : α → α → Bool) x1 x2 xs1 xs2 :
    ...
    (beq x1 x2 && beqList beq xs1 xs2) := (rfl)

theorem beqList_nil_cons {α} (beq : α → α → Bool) x xs :
    beqList beq [] (x :: xs) = false := (rfl)
...
-- equality operator is polymorphic, we can use it at *any* type
...
-- don't compute to the same term, and we cannot proceed by cases on `a` or
-- `b`, as they are not inductive.
...
-- able to close goals of the form `a ↔ a` by reflexivity.
...
-- performed (i.e., we don't have to execute `++` after the recursive call); a
-- decent compiler will generate very efficient code in this case.
...
-- `a` holds while defining a Lean function. You may be surprised to learn
...
-- *Claim*: There exist irrational numbers `n` and `m` such that `n ^ m` (`n`
-- to the power `m`) is rational.

-- *Proof*: It is not difficult to show that `sqrt 2` is irrational. So if
-- `sqrt 2 ^ sqrt 2` is rational, it suffices to take `n = m = sqrt 2` and we
...
-- the proof knowing that such `n` and `m` exist, but not being sure of their
-- actual values.
