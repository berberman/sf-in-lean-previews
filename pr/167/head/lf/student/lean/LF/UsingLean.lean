...
-- well, but they are not the same as the `Nat` we defined in `Basics.lean`.
...
-- want to have the tactics like `add_comm` and `add_assoc` "guess" which
-- subterms to rewrite.
...
-- we just want to simplify the function (here `add`) automatically when we
-- can.
