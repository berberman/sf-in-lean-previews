...
-- To make element-getting lighter weight, we can define notation so we can
...
-- concept of getting an element as its own typeclass, called `MyGetElem`, and
-- define notation for instances of that typeclass. We do this to illustrate a
-- common pattern in Lean (indeed, `MyGetElem` is a simpler form of the
...
-- defined custom notation before (e.g. `::` and `[...]` for lists, or
-- `+`/`*`/`==` for arithmetic), but always with `infixl`/`infixr` or
...
-- `GetElem`, we only need to define the macro.

-- Note to developers (Benjamin Pierce  @bcpierce00):
--     What does "the macro" mean? And didn't we say we were not going to
--     explain the macro stuff? I feel like this section is falling in an
--     uncomfortable middle ground between completely skating over the
--     technicalities and actually explaining.

-- It's scoped since we don't want to override the default `GetElem`
-- everywhere, but only when `open scoped MyGetElem` is in force.
...
-- Note to developers (Benjamin Pierce  @bcpierce00):
--     ... as opposed to what (let's be explicit!)? And why do we make this
--     choice? Just to show both ways, or for some principled reason?
...
-- Next, we declare `Insert` and `Singleton` instances that control the `{}`
-- notation in Lean.

-- Note to developers (Benjamin Pierce  @bcpierce00):
--     Do readers know what `Insert` and `Singleton` are? Should we link to
--     their docs?
