...
elab_rules : term
  | `(aexp { $x:ident }) => do
    ...
    if (← withNewMCtxDepth <| isDefEqGuarded type (Lean.mkConst ``Aexp)) then
      pure e
    else if (← withNewMCtxDepth <| isDefEqGuarded type (Lean.mkConst ``String)) then
      pure <| Lean.mkApp (Lean.mkConst ``Aexp.id) e
    else
      throwErrorAt x "expected an Imp identifier or arithmetic expression"
