...
-- In VS Code, a lightbulb icon appears on the left when a code
-- action is available at your cursor.

-- Let's look at an example code action using `induction`. For
-- example, suppose we start with the following incomplete
-- proof:
...
-- Put your cursor on `induction n` and open the code action
-- menu. You should see "Generate an explicit pattern match for
-- 'induction'." in the list. If you choose this action, Lean
-- adds an explicit branch for each constructor:
...
-- This gives us basic structure of the proof without requiring
-- us to write each branch by hand. We can then focus on
-- proving each case.
...
-- `induction`, `case`, and `match` branches while working with
-- this book.

-- By default, `rewrite` and `rw` rewrite left to right, i.e.,
-- they transform the goal (or a hypothesis) from the form on
-- the left side of the equality to the right side. To rewrite
-- from right to left, use `rewrite [← h]` or `rw [← h]`, where
-- `←` is entered as `\l` or `\<-`.
