...
--      strings of `succ` applied to `0` (which also
--      introduces subtleties that are really not the main
--      point here). But the present formulation is not a
--      big problem either.
--
--      LATER: Harper's lecture from the Milner symposium
--      would make a good support for this lecture.
--
--      LATER: There are a bunch of slides from earlier
--      offerings of CIS500 that might be wonderful
...
--  Note to developers (Benjamin Pierce @bcpierce00):
--      This next is not using the new conventions for
--      `grade` blocks, which I thought `to_verso.py` was
--      now enforcing. Is that because this file was
--      converted a while back, before these improvements?
--      (I suspect yes because the indentation is also wonky
--      and I improved that too.) Anyway, the `grade` block
--      headers should be fixed, throughout (and maybe in
--      Smallstep and Imp?)... `dev` block headers too, if
--      we want to be really consistent.
