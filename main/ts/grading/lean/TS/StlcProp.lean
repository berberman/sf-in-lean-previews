...
attribute [autogradedHole] Stlc.progress'

attribute [autogradedProof 3] Stlc.progress'
...
attribute [autogradedHole] Stlc.substitution_preserves_typing_from_typing_ind

attribute [autogradedProof 3] Stlc.substitution_preserves_typing_from_typing_ind
...
--    `(if true then (λx:Bool. x) else (λx:Bool. x)) ⟶ true` but it's not
--    the case that `⊢ true ⦂ Bool → Bool`.
...
--    `(λx:Bool. λy:Bool. x) true ⟶ λy:Bool. true` but it's not the case
--    that `⊢ λy:Bool. true ⦂ Bool`
...
--  - Becomes false. For instance, `true true` is a term that becomes
--    typable (at type `Bool`), but which is stuck.

--  - Preservation

--  - Remains true. There are 3 ways `t₁ t₂` can reduce. For `Step.app1`
--    and `Step.app2` we can still apply the induction hypothesis. To
--    reduce `t₁ t₂` using `Step.appAbs` `t₁` would need to be a function,
--    but functions don't have type `Bool`.
...
--      pieces of that have no Lean counterpart: the `<{{ x }}>` quotation
--      for shifting into type-parsing mode (Lean works out from context
--      which of the two brackets is meant), the `$( t )` escape into
--      arbitrary Rocq `constr` notation (our `~e` already does that, in
--      both grammars), and the two coercions `tm_var : string >-> tm` and
--      `tm_const : nat >-> tm`, which let a bare string or numeral stand
--      for a term. Here a bare identifier and a bare numeral are
...
attribute [autogradedHole] StlcArith.Tm.IsValue
...
attribute [autogradedHole] StlcArith.Step
...
attribute [autogradedHole] StlcArith.Nat_step_example

attribute [autogradedProof 5] StlcArith.Nat_step_example
...
attribute [autogradedHole] StlcArith.Nat_typing_example

attribute [autogradedProof 5] StlcArith.Nat_typing_example
...
attribute [autogradedHole] StlcArith.weakening

attribute [autogradedProof 6] StlcArith.weakening
...
attribute [autogradedHole] StlcArith.preservation

attribute [autogradedProof 6] StlcArith.preservation
...
attribute [autogradedHole] StlcArith.progress

attribute [autogradedProof 6] StlcArith.progress

end StlcArith

--  Note to developers (Claude):
--      The source's grading file weights this exercise at 28 points — 10
--      for `STLCArith.subst` and 6 each for `weakening`, `preservation`,
--      and `progress`. The three theorems carry those weights directly;
--      the 10 points for the definitions are split between the two
--      examples that exercise them, `Nat_step_example` and
--      `Nat_typing_example`, since a definition has no autogradable
--      statement of its own. Names are qualified (`StlcArith.progress`) so
