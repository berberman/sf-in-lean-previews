...
-- The `rw` tactic is used to apply a known equality (a hypothesis from the
-- context or a previously proved lemma) to modify the goal, replacing all
-- occurrences of one side by the other.

-- The `apply` tactic uses a known implication (a hypothesis from the context,
-- a previously proved lemma, or a constructor) to replace a goal that matches
-- the conclusion of the implication with subgoals, one for each premise of
-- the implication.
...
attribute [autogradedProof 2] beq_eq
...
attribute [autogradedProof 3] plus_n_n_injective
...
attribute [autogradedProof 3] nth_error_after_last
...
attribute [autogradedProof 2] bool_fn_applied_thrice
...
attribute [autogradedProof 3] beq_symm
...
attribute [autogradedProof 3] filter_exercise
...
example : forallb (Nat.odd) [1,3,5,7,9] = true := (by rfl)
example : forallb not [false,false] = true := (by rfl)
example : forallb (Nat.even) [0,2,4,5] = false := (by rfl)
...
example : existsb (Nat.odd) [1,0,0,0,0,3] = true := (by rfl)
example : existsb (Nat.even) ([] : List Nat) = false := (by rfl)
...
attribute [autogradedProof 6] existsb_existsb'
