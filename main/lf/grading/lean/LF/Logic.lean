...
--  i.e., `¬ a → False`, so suppose `¬ a` as well and try to derive
--  `False`. Then we have both `a` and `¬ a` (i.e., `a → False`) from which
...
--  *Proof*: Suppose, for some `a`, that `a ∧ ¬ a` holds. Recall that `¬ a`
--  is defined as `a → False`. Given `a` and `a → False`, we can prove
--  `False`, so `(a ∧ ¬ a) → False`, i.e. `¬ (a ∧ ¬ a)`.
