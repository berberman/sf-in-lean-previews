...
--  complexity of modern systems, the number of people involved, and the
--  range of demands placed on them make it challenging to build software
--  that is even more-or-less correct, much less 100% correct. At the same
...
--  Volumes have been written about the central role of logic in computer
--  science. Manna and Waldinger (1971) called it "the calculus of computer
--  science," while Halpern *et al.* (2001)'s paper *On the Unusual
--  Effectiveness of Logic in Computer Science* catalogs scores of ways in
--  which logic offers critical tools and insights. Indeed, they observe
--  that, "As a matter of fact, logic has turned out to be significantly
--  more effective in computer science than it has been in mathematics.
...
--  The flow of ideas between logic and computer science over the years has
--  run in both directions, with CS also making key contributions to logic.
...
--  provides high-level facilities for proof development, including a large
--  library of common definitions and lemmas, powerful tactics for
...
--      The individual references above should be merged into the
--      categories below. In particular, the first three should go in the
--      bullet about math, and the last one should go in the bullet about
--      modeling programming languages. (I see that that first bullet could
--      also use citations for some of the points it already makes, but we
--      can leave that for later...)

--  - As a *platform for modeling programming languages*, proof assistants
--    have become standard tools for researchers who need to describe and
--    reason about complex language definitions. They have been used, for
--    example, to check the security of the JavaCard platform, obtaining
--    the highest level of common criteria certification, and for formal
--    specifications of the x86 and LLVM instruction sets and programming
--    languages such as C.

--  - As *environments for developing formally certified software and
--    hardware*, they have been used, for example, to build CompCert (Leroy
--    *et al.*, 2016), a fully-verified optimizing compiler for C,
--    [Cedar](https://github.com/cedar-policy/cedar-spec) (Disselkoen *et
--    al.*, 2024), a formally-specified policy language, and CertiKOS (Gu
--    *et al.*, 2016), a fully verified hypervisor, and for proving the
--    correctness of subtle algorithms involving floating point numbers,
--    and as the basis for CertiCrypt, FCF, and SSProve, which are
...
--    theorem, which had previously been controversial among mathematicians
--    because the argument required checking a large number of
...
--  *Functional programming* refers both to a collection of powerful coding
--  idioms that can be used in almost any programming language and to a
...
--  from *side effects* such as I/O, assignments to mutable variables,
--  redirecting pointers, etc. For example, whereas an *imperative* sorting
...
--  break an invariant relied on by another part of the program. This is
...
--  counterparts. If running a computation has no effect other than
--  producing a result, then it does not matter *where* it is run.
--  Likewise, if a data structure is never modified destructively, it can
--  be copied freely, across cores or across the network. Indeed, the
...
--  extremely expressive functional programming language and a set of tools
--  for stating and proving logical assertions. Moreover, when we come to
--  look more closely, we find that these two sides of Lean are actually
--  aspects of the very same underlying machinery -- i.e., *proofs are
--  programs*.
...
--  - Clone the [SF-in-Lean](https://github.com/plclub/sf-in-lean) git repo
...
--      These steps send the reader to the **sources** repo, but the files
--      a reader is meant to work in are the ones the build generates:
--      `make lf-student` writes a standalone Lake project (its own
--      `lakefile.toml` and `lean-toolchain`) to `_out/lf/student/lean`,
--      and that is where the exercises with `sorry`s live. The `LF/*.lean`
--      files in the repo are Verso documents — prose plus code blocks —
--      and building the repo builds Verso and all three volumes, which is
--      a good deal more than "a few minutes".
--
--      There is also a chicken-and-egg problem in the ordering: running
--      `make` at all needs `elan`/`lake` already installed, but the step
--      above gets Lean via the VS Code extension, which only offers it
--      once a Lean project is open.
--
--      Both go away once there is a released archive of the student
--      `.lean` files: the steps become "download and unpack, open the
--      folder in VS Code, accept the install prompt". Until that exists,
--      this section should probably say plainly that readers get the files
--      from their instructor or by following `ALPHA-TESTERS.md`.
...
--  Optional exercises provide a bit of extra practice with key concepts
--  and introduce secondary themes that may be of interest to some readers.
--  Advanced exercises offer an extra challenge and a deeper cut at the
--  ideas. Doing just the non-optional, non-advanced exercises should
--  provide good coverage of the core material.
