...
-- Up to now, we've used the manual part of Lean's tactic facilities. In this
-- chapter, we'll learn more about some of Lean's powerful automation
-- features, including tactic combinators like `try` and `repeat`, decision
-- procedures like `lia`, and automatic simplification using `simp`. Using
-- these features together with Lean's metaprogramming facilities will enable
-- us to make some of our proofs startlingly short! Used properly, they can
-- also make proofs more maintainable and robust to changes in underlying
-- definitions.

-- Our motivating example will be the following proof, repeated with just a
-- few small changes from the IndProp chapter. We will simplify this proof in
-- several stages.
...
-- Note to developers (@rogerburtonpatel):
...
--   and multiplication by constants (this is what makes it Presburger
--   arithmetic),
...
-- then invoking `lia` will either solve the goal or fail, meaning that the
-- goal is actually false. If the goal is *not* of this form, `lia` will fail.
-- Note that when failing, `lia`, may mention another tactic, called `grind`.
-- This is another, more powerful tactic that implements `lia`, but we will
-- not use it here.
...
-- In Induction, we saw how to use the `<;>` combinator in order to apply the
-- same tactic to every subgoal in a proof. As a reminder, consider this
-- example, where `cases` on `b` and `c` each leaves two subgoals that are
-- discharged identically:
...
-- letting us discharge many similar subgoals at once. Getting used to them
-- takes a little energy, but it lets us scale up to more complex definitions
-- and more interesting properties without drowning in boring, repetitive
-- detail.
...
-- The first such combinator we'll discuss is `try`. If `t` is a tactic, then
-- `try t` is a tactic that is just like `t` except that, if `t` fails,
-- `try t` *successfully* does nothing at all (rather than failing).
...
-- Here, we can use the `lia` tactic to close some of these goals, but not all
-- of them. So, a more compact way to write this proof would be:
...
-- Note that `try lia <;> try rw [...] <;> lia` *doesn't* work, because the
...
-- The `repeat` combinator takes another tactic or parenthesized sequence of
-- tactics and keeps applying it until it fails.
...
-- original goal, then repeat *succeeds* without changing the goal at all
...
-- The tactic `repeat t` does not have any upper bound on the number of times
-- it applies `t`. If `t` is a tactic that *always* succeeds (and makes
-- progress), then `repeat t` will loop forever.
...
-- guide Lean in constructing proofs; if the construction process diverges
-- (i.e., it does not terminate), this simply means that we have failed to
-- construct a proof at all, not that we have constructed a bad proof.
...
-- Neither `rfl` nor `left` succeed on this goal, but `lia` does, so `first`
-- stops after `lia` and never tries `induction`. As with `try`, `first` is
-- most useful in combination with other combinators. For example, we can
-- rewrite our previous examples that used `repeat` and `try` like so:
...
-- The `first` tactic here will attempt to close the goal with an application
-- of `List.mem_cons_self`, if it can, and otherwise
-- `apply List.mem_cons_of_mem` to proceed to checking the next element in the
...
-- `List.mem_cons_self` like before, we would instead first try
-- `apply List.mem_cons_of_mem`, which would also succeed. This leaves us with
-- the goal `10 ∈ []`, which is of course false.
...
-- The `simp` tactic is Lean's *simplifier*, and it is one of the most
-- powerful tools in the language. Given a set of lemmas ─ some built-in, some
-- user-provided ─ `simp` attempts to reduce a goal or hypothesis by rewriting
-- with those lemmas as much as possible.

-- Indeed, the characterizing lemmas we've been writing for our definitions
-- all throughout this book are examples of these *simplification lemmas*, or
-- *`simp` lemmas* as they're called by Lean programmers.
...
-- Instead of manually rewriting by the characterizing lemmas in the example
-- below, `simp` does it automatically.

theorem add_succ_nested (n m : Nat) :
    n + (m + 1 + 1) = (n + m + 1)  + 1 := by
  simp

-- If you know what theorems you want `simp` to use for your goal proof, you
-- can write `simp [<theorems>]`. If you want `simp` to *only* use those, you
-- can use `simp only [<theorems>]`. Like with `dsimp`, you can also supply a
...
-- In the InfoView, Lean will show you what `simp` is doing. You can click the
-- `[apply]` button to replace `simp?` with the suggested replacement. You
-- should always do this for your final proof scripts: `simp?` is helpful for
...
-- Like `apply` and `rw`, there's also a version of `simp` that can simplify
-- in hypotheses, rather than the goal. Invoking `simp` as
-- `simp [<lemmas>] at h` runs the simplifier with at hypothesis `h`.
...
-- simplifies in all hypotheses and in the goal at the same time. Rewriting
-- the example above:
...
-- Because `simp` is such a powerful tactic, the Lean community has developed
-- a number of conventions surrounding appropriate usage. One such convention
-- is around *terminal* `simp` usage.

-- A call to `simp` is considered terminal either when it is the last tactic
-- used to close a goal or when it is followed only by other automatic (also
-- called "flexible") tactics like `simp` or `lia`. In idiomatic Lean, all
-- non-terminal uses of `simp` should use the `only` qualifier and specify
-- exactly which lemmas are being used to simplify. Use of `simp` without
-- `only` should only occur in terminal positions.

-- In our example from before, the use of `simp` is terminal (and therefore
-- okay) because it is followed only by other `simp`s and `lia`:
...
-- On the other hand, if we instead decline to use `lia` and solve the goal
-- manually, this example uses `simp` in a non-terminal position, and is
...
-- Using `simp` this way is brittle because if we add new `simp` lemmas to our
-- library, this can change the way that our hypotheses and goals are
-- simplified. Because our proof after the `simp`s relies on the precise
-- structure of the goals and hypotheses, these changes could cause the proof
-- to break as the structure of the development evolves.

-- We can fix the style of this proof by changing the `simp`s to specify which
-- theorems they are using to simplify:
...
-- All of the theorems marked with the `@[simp]` attribute in a Lean library
-- compose the *simp set* for that library, and the result of simplifying an
-- expression iteratively using all of the theorems in the simp set is the
-- *simp normal form* of that expression.

-- It's important for the stability of proofs using `simp` that all the
-- theorems in the simp set progress towards this normal form. Accordingly,
-- library designers often first consider what they want that normal form to
-- look like, and then structure their theorem definitions accordingly. As a
-- simple example, the simp normal form for lists prefers to use the `++`
-- notation instead of `List.append`, so there is a `simp` theorem
-- `List.append_eq` whose type is
-- `List.append_eq {α : Type u} {as bs : List α} : as.append bs = as ++ bs`.

-- In this case, the `simp` normal form appears on the right, while the
-- expression in need of simplification appears on the left. We can thus think
-- of this theorem as simplifying from left to right. Not every `simp` lemma
-- in the standard library has a `simp` normal form on its right-hand side,
-- but all make progress towards `simp` normal form when applied.

-- For our purposes, in this textbook and in later ones, we will take care to
-- define our `simp` lemmas such that they respect this left-to-right
-- simplification behavior.

-- ## The `trivial` Tactic

-- A final automated tactic to have in your toolkit is `trivial`, which tries
-- a number of different simple tactics (such as `rfl` or `contradiction`) to
-- try to close the current goal. Some examples:
...
-- As a culminating exercise for this book and as practice using the
-- automation techniques we discussed above on a real proof, we examine the
-- theory of regular expressions, eventually working up to a proof of the
-- pumping lemma.

-- ### Definitions

-- Regular expressions are a formal language for describing sets of strings.
...
-- `RegExp α` describe strings with characters drawn from `α` ─ which in this
-- exercise we represent as *lists* with elements from `α`.

-- (Technical aside: We depart slightly from standard practice in that we do
-- not require the type `α` to be finite. This results in a somewhat different
...
--     not to carry Prop-valued typeclass assumptions, enforcing this only at
...
-- - If `re₁` matches `s₁`, and `re₂` matches `s₂`, then `App re₁ re₂` matches
--   `s₁ ++ s₂`.
...
--   of strings `s = s₁ ++ ... ++ sₖ`, and the expression `re` matches each one
--   of the strings `sᵢ`, then `Star re` matches `s`.

--   In particular, the sequence of strings may be empty, so `Star re` always
...
-- ... is not explicitly reflected in the above definition. Do we need to add
-- something?

-- (A) Yes, we should add a rule for this. (B) No, one of the other rules
-- already covers this case. (C) No, the *lack* of a rule actually gives us
-- the behavior we want.

-- Notice that these rules are not *quite* the same as the intuition that we
-- gave at the beginning of the section. First, we don't need to include a
-- rule explicitly stating that no string is matched by `EmptySet`; indeed,
-- the syntax of inductive definitions doesn't even *allow* us to give such a
-- "negative rule." We just don't happen to include any rule that would have
-- the effect of `EmptySet` matching some string.

-- Second, the intuition we gave for `Union` and `Star` correspond to two
-- constructors each: `mUnionL` / `mUnionR`, and `mStar0` / `mStarApp`. The
-- result is logically equivalent to the original intuition but more
-- convenient to use in Lean, since the recursive occurrences of `ExpMatch`
-- are given as direct arguments to the constructors, making it easier to
-- perform induction on evidence. (The exercises below ask you to prove that
-- the constructors given in the inductive declaration and the ones that would
-- arise from a more literal transcription of the intuition is indeed
-- equivalent.)
...
-- Notice how the last example applies `mApp` to the string `[1]` directly.
-- Since the goal mentions `[1, 2]` instead of `[1] ++ [2]`, Lean wouldn't be
-- able to figure out how to split the string on its own.

-- Using `inversion`, we can also show that certain strings do *not* match a
-- regular expression:
...
-- `reg_exp_of_list` function constructs a regular expression that matches
-- exactly the string that it receives as an argument:
...
-- As a quick exercise, prove that every list matches `reg_exp_of_list` of
-- itself:
...
-- The following lemmas show that the intuition about matching given at the
-- beginning of the section can be obtained from the formal inductive
-- definition.
...
-- State and prove that this `EmptyStr'` definition matches exactly the same
-- strings as the `EmptyStr` constructor.
...
-- For example, suppose we want to prove the following intuitive fact: If a
-- string `s` is matched by a regular expression `re`, then all elements of
-- `s` must occur as character literals somewhere in `re`.
...
-- The problem here is that `induction` over a `Prop` hypothesis only works
-- properly with hypotheses that are "fully general," i.e., ones in which all
-- the arguments are just variables, as opposed to more specific expressions
-- like `Star re`.

-- A possible, but awkward, way to solve this problem is "manually
-- generalizing" over the problematic expressions by adding explicit equality
...
-- occurrences of the expression `e` by the variable `x`, and (2) add an
-- equation `h : x = e` to the context. Here's how we can use it to show the
-- above result:
...
-- exercise above), shows that our definition of `ExpMatch` for `Star` is
...
-- expressions is the so-called *pumping lemma*, which states, informally,
-- that any sufficiently long string `s` matching a regular expression `re`
-- can be "pumped" by repeating some middle section of `s` an arbitrary number
-- of times to produce a new string also matching `re`. For the sake of
-- simplicity, this exercise considers a slightly weaker theorem than is
-- usually stated in courses on automata theory ─ hence the name
-- `weak_pumping`. The stronger one can be found below.

-- To get started, we need to define "sufficiently long." Since we are working
-- in a constructive logic, we actually need to be able to *calculate*, for
-- each regular expression `re`, a minimum length for strings `s` to guarantee
-- "pumpability."
...
-- You may find these lemmas about the pumping constant useful when proving
-- the pumping lemma below.
...
-- (appends it to itself) some number of times. Note how we define `simp`
-- lemmas for `napp` to go with its definition.
...
-- These auxiliary lemmas might also be useful in your proof of the pumping
-- lemma.
...
-- The (weak) pumping lemma itself says that, if `s =~ re` and if the length
-- of `s` is at least the pumping constant of `re`, then `s` can be split into
-- three substrings `s₁ ++ s₂ ++ s₃` in such a way that `s₂` can be repeated
-- any number of times and the result, when combined with `s₁` and `s₃`, will
-- still match `re`. Since `s₂` is also guaranteed not to be the empty string,
-- this gives us a (constructive!) way to generate strings matching `re` that
-- are as long as we like.

-- This proof is quite long, so to make it more tractable we've broken it up
-- into a number of sub-proofs, which we then assemble to prove the main
-- lemma.
...
-- optional exercise earlier in the IndProp chapter may be useful here ─ in
...
-- Note to developers (Benjamin Pierce @bcpierce00):
--     I've made it optional, following the original, but we should think/talk
--     about it. I also reduced the rating from 10 to 5 (which I think is the
--     maximum?).
...
-- claim that `s₁.length + s₂.length ≤ pumpingConstant re`.
