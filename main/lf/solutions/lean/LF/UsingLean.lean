...
--     That last sentence is not very clear. "that definitions be fully
--     simplified" does not parse, but I'm not sure whether to add "will" or
...
-- or prove these theorems ourselves, we may not know (or remember) all the
-- available theorems.

-- Lean provides a few ways to search through the standard library to find
-- theorems that may be useful during a particular proof. The first way is the
-- `exact?` tactic. This tactic searches the standard library for a theorem
-- that can be applied, along with the hypotheses in the context, to exactly
...
-- idiomatic Lean should not contain `exact?` tactics (or any other `?`
-- tactics) in the finished versions of proofs.
...
-- one. Fortunately, there are other tactics that can help in these cases.
...
--     Yipeng said we can pass a theorem, e.g. `dsimp [Nat.mul_zero]`, which
--     would rewrite `Nat.mul_zero` many times and then perform reductions,
--     just like simp `[Nat.mul_zero]`. Also `@[defeq] lemmas` in the `simp`
--     set are always used implicitly.
--
--     Should `dsimp [Nat.mul_zero]` be preferred over `dsimp [Nat.mul]`? An
--     example:
--
...
--       dsimp [Nat.mul_zero]`
--
--     This could be confusing though, because rewriting by `dsimp` only works
--     for *definitional* equalities. The following doesn't work
--
--     `example (n : Nat) : (((0 * n) * n) * n) = 0 := by
--       dsimp [Nat.zero_mul]`
--
--     This is because `Nat.zero_mul` is true by induction, not reduction.
--     This is a bit confusing to explain, and also unfortunate since one may
--     not know why an equality holds. Thus I'd prefer not to include this use
--     here.
