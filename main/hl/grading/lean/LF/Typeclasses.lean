...
--  `a : α`.
--
--  Sometimes, though, we want less freedom: rather than leaving `α`
--  completely generic, we want to partially specify its behavior. In Lean,
--  this is done through a form of "ad hoc polymorphism" called
--  **typeclasses**. The concept originated in Haskell and is analogous to
--  features you may know from other languages, such as traits in Rust.
...
--
--  Typeclasses automate this — instead of the programmer passing the
--  function explicitly, Lean searches for one and provides it on its own.
...
--  `BEq` comes from Lean's standard library. Let's define a typeclass of
--  our own, to see the mechanism — classes, instances, and synthesis —
--  that made `==` resolve automatically above.
--
...
--  on the type of the things in the list, like `0` for `Nat`.
--
--  Getting Lean to fill in `defaultValue` automatically takes two things.
--  One is marking the parameter as "searchable," rather than something the
--  caller always supplies explicitly. The other is giving Lean some
--  information about what it should search *for*.
--
--  Considering the second problem first: the way to provide this
--  information is to *name* the data we're after — the **default value**
--  of a type. In particular, a `structure` (chapter Lists) is a good way
--  to give this information a name; structures can also bundle together
--  more than one piece of data, which will come in handy later, though we
--  only need a single field here.
--
--  To address the first problem, we need to mark this particular structure
--  as one Lean should search for automatically — not every
--  `structure`-typed argument should be.
--
--  Let's build up to what we want in two steps: first the naming, as a
--  plain `structure`; then the marking, by upgrading it to a `class`.
--  Here's the structure — we'll put it in its own namespace so we can
--  reuse the name `DefaultValue` for the class version below:
...
--  A value of type `DefaultValue Nat` picks out a particular `Nat` to
--  serve as the type's default: it's built the same way any structure is,
--  by supplying a `Nat` for the `value` field:
...
--  We then provide values of this type a bit differently. Instead of
--  `def`, we use `instance`:
...
--  Notice that we refer to `DefaultValue.value` alone, with no instance
--  named. Because the expression equates `DefaultValue.value` with the
--  `Nat` `0`, Lean selects `instDefaultValueNat`, the instance for
--  `DefaultValue Nat`. We know this because we are able to prove that
...
--  We can also create instances for polymorphic types, like `Option α`,
--  whose default is `none`, by giving the instance declaration a
--  parameter:
...
--  expect at most one instance per type, so this search has a unique
--  answer.
--
...
attribute [autogradedProof 1] List.elemPoly_eq_elemNat

--  ## Proof-Carrying Typeclasses

--  The above examples enforce no conditions on the data an instance may
--  carry — any value of the right type will do. But sometimes enforcing
--  constraints on data is useful. For example, suppose we want to specify
--  that a type has not just a single element, but two. Here is a first
--  attempt:
...
--  Unfortunately, this specification isn't precise because it allows `one`
--  and `two` to refer to the same term. Fortunately, Lean's typeclasses
--  can carry proofs along with data, so we can write the following to
--  enforce that `one` and `two` are distinct.
...
--  Declaring instances works in much the same way as before, except that
--  now the `HasTwo.one_neq_two` field requires a proof:
...
--  and give an instance of it for `Nat`.
...
--  define mathematical structures that carry both operators and laws about
...
--  notation for it. The `Monoid` typeclass "inherits" the fields of
--  `OpSet`, similar to how a class would in an object-oriented language.
--
--  As one might expect, the `+` operator over `Nat`s forms a monoid, where
--  `0` is the identity element. Note that we don't have to define `Nat`'s
--  `OpSet` instance separately, we can define a single instance that
...
--  element of a monoid is unique. That is, if we have two monoids over the
--  same set with the same operator, their identity elements must also be
--  the same:
...
--  We can extend the definition of a `Monoid` to capture this new feature:
...
--  Two groups defined with the same operation over the same set must have
--  the same inverse as well.
...
--  ordinary programming and in the theory of programming languages; we're
--  going to need them in many places in later volumes.
--
--  We'll define two flavors of maps: *total maps*, which include a
--  "default" element to be returned when a key being looked up doesn't
--  exist, and *partial maps*, which instead return an option to indicate
...
--  "equivalent" list structures. This simplifies proofs that use maps.
--
...
--
--  The function `TotalMap.empty` yields an empty total map, given a
...
--  we would prefer not to expose this fact in its public interface.
...
--  In the above example, we use `rewrite` and `rfl` instead of the usual
--  `rw` to highlight something interesting. After the rewrites in this
--  proof, we end up with a goal that looks like
--  `{ inner := fun x => 0 }.inner n = 0`, which we can solve with `rfl`.
...
--  `MyGetElem.getElem m a` for any type with a `MyGetElem` instance, not
--  just `TotalMap`.
--
...
--  you do wish to learn more, Chapter 5 and 6 of [Metaprogramming in Lean
...
--  `GetElem`, we only need to define the `macro_rules`, not the `notation`
...
--  For example, we can build a map taking `String` to `Bool`, where
...
--  Here `|>` is Lean's **pipe** notation: `x |>.f y` means `x.f y`,
...
--  references `TotalMap.update` directly. Unlike indexing, `update`
--  doesn't need to work generically across container types (there's no
--  standard-library operation like `GetElem` that we're mirroring here),
--  so the simpler, direct route suffices.
...
--  (`funext`) from the Logic chapter: two maps are equal when they agree
--  at every key. Recording it once, for maps, and tagging it `@[ext]` lets
--  the `ext` tactic reduce a goal `m₁ = m₂` to the pointwise one in the
--  proofs below.
--
--  The fact that `TotalMap` is a structure complicates things slightly. We
--  need to use injectivity of its constructor `mk` which Lean
--  automatically provides for us as `mk.injEq`. It lets us prove `m₁ = m₂`
--  from `m₁.inner = m₂.inner` or vice versa.
...
--  Given keys `a₁` and `a₂`, the tactic `by_cases` `h : a₁ = a₂` splits
--  the proof into the case where they are equal — where `subst h` then
--  replaces one by the other — and the case where they are not, which is
--  what `update_neq` wants. Use it to prove the following theorem, which
--  states that if we update a map to assign key `a` the same value as it
...
--  Wouldn't it be nice if we could use a more natural notation for
--  concrete maps like `{ "bar" ↦ true, "foo" ↦ true }`? To accomplish this
--  we define a simple structure that consists of a key and a value along
--  with `↦` notation for it.
...
--  standard-library typeclasses behind the `{x, y, ...}` and `{x}`
--  collection-literal notation that `List`, `Finset`, and other stdlib
...
--  Lean doesn't know what type of collection `{ "foo" ↦ true }` is without
--  type hints, as we can see with `#check`:
...
--  This also holds by definition (`rfl`), since all the rewrites in the
--  above proof do the computation step-by-step.

example : (2 →ₚ 3)[2] = some 3 := by rfl

--  Next, we lift all of the basic lemmas about total maps to partial maps.
--  To do this we should first prove an extensionality lemma about partial
--  maps. To prove extensionality, we employ injectivity of `PartialMap`'s
...
--  One last thing: for partial maps, it's convenient to introduce a notion
--  of map inclusion, stating that all the entries in one map are also
--  present in another. Lean already has notation for this — `m₁ ⊆ m₂` —
--  which we get by supplying a `HasSubset` instance.
...
--  will see in *Type Systems*, where maps are used to keep track of which
--  program variables are defined in a given scope.
...
--  about natural numbers that we discussed and proved in previous
--  chapters; copy your solutions to those problems here:
...
--  number of steps to either `true` or `false` — i.e., there is a
...
--  argument) that there is no function in `Nat → Bool` that checks whether
--  a given number is the code of a terminating Turing machine.
--
...
--  halts." The second row in the table follows directly from this
--  essential difference. To evaluate a pattern match (or conditional) on a
--  boolean, we need to know whether the scrutinee evaluates to `true` or
--  `false`; this only works for `Bool`, not `Prop`.
--
--  The third row highlights an important practical difference: equality
--  functions like `Nat.beq` that return a boolean cannot be used directly
--  to justify rewriting with the rewrite tactic; propositional equality is
--  required for this. Since `Prop` includes both decidable and undecidable
--  properties, we have two options when we want to formalize a property
--  that happens to be decidable: we can express it either as a boolean
...
attribute [autogradedProof 3] Reflection.Nat.even_double_exists
...
--  *computational* nature of Lean's core language, which is designed so
--  that every function it expresses is total, and by default computable
--  unless we explicit indicate otherwise. As an example, consider trying
--  to write a function `α → α → Bool` checking for equality on an
--  arbitrary type:

--  Note to developers:
--      dsainati12 days ago We use regular if for the first time here. It
--      is probably necessary to explain at this point what if is and how
--      it differs from bif.
--
--      👍 1 berberman1 day ago Should we clarify the difference between =
--      and ==? Observably if and bif can possibly accept both as the
--      condition because of Coe or DecidableEq instances, which IMO could
--      be confusing.
--
--      Probably we can talk a bit about the coercion system in this file
--      as well, since Coe could be an example of typeclasses, so long as
--      if we ignore the outParam thing...
--
--      chenson20181 day ago I definitely intended for this to cover =
--      versus ==. Maps uses LawfulBEq (which says = and == coincide). If
--      that will now appear here it's a good place to give some more
--      detail?
--
--      rogerburtonpatel1 day ago I think right after this part on
--      decidability is good. It's a hefty chunk of information already, so
--      keeping distinct ideas distinct is more likely than not a good
--      call.

--  Note to developers:
--      @dsainati - commenting this out for the same reason as above
--
--      @ionathanch - this needs to be put back in, right? `nat_eq`
--      continues on referring to this
--
...
--  interchangeably. For instance, while our example `eq` failed above
--  while trying to use propositional equality `=` in the condition of an
--  `if` statement, we are allowed to write
...
--  synthesis to find the scoped instance `Classical.propDecidable`, which
--  makes use of the axiom of choice to provide a proof that all
--  propositions are *classically* decidable. This sort of definition is
...
--      if that's what you want. Some the the proofs can be a bit advanced
